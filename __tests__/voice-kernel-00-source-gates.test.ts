...
describe('KERNEL-00 · VPIO-02B — witness preparation: the fourth subject row lives in the instrument only; the organism is frozen at ac12dedf4; no device act', () => {
  ...
  it('corpus partition (founder ruling 2026-09-15; SID custody extension 2026-09-16): every tracked journal carries H (declared subject) and C (physical start-trace signature); vpio-02-sid remains its own custody subject while intentionally sharing the vpio-02 physical trace; disagreement fails closed naming file · header · signature; sets are vpio-02 61 · vpio-02-sid 90 (ENTRY 75 + SOURCE 15: partial 5 + complete 10) · vpio-01 30 · engine 468; the frozen classifier reproduces every produced ledger under its declared classifier subject; cross-subject reads are SUBJECT-MISMATCH; no directory is named', () => {
    ...
    const vpio02sidEntry = vpio02sid.filter((p) => p.includes('/VPIO-02-SID-ENTRY'));
    const vpio02sidSource = vpio02sid.filter((p) => p.includes('/VPIO-02-SID-SOURCE'));
    const engine = all.filter((p) => physicalForDeclared(carriers.get(p)!.h?.subject) === 'engine' && carriers.get(p)!.c === 'engine');
    expect([vpio02.length, vpio02sid.length, vpio02sidEntry.length, vpio02sidSource.length, vpio01.length, engine.length]).toEqual([61, 90, 75, 15, 30, 468]);
    ...
    expect(c02sid.filter((c) => c === 'gen-1 listen').length).toBe(89);
    ...
    expect(classes('vpio-01', vpio02sid)).toEqual(Array(90).fill('SUBJECT-MISMATCH'));
    ...
    expect(byDir.size).toBe(22);                                                                    // + SID ENTRY-BATCH-05 as its own custody population; archive remains headerless
    const floods: string[] = [];
  ...
  it('K00-05/06 Option C: the output reader is evidence-only — closed verdict vocabulary, ratified 100 ms and the 0.90 witness criterion as named constants, no subprocess/device/xcodebuild, no journal write, no import of the entry classifier; --selftest 33/33 naming the founder’s twelve offline cases; every real VPIO journal yields no PASS and no FAIL', () => {
    ...
    const sidEntryDirs = outputLedgers.filter((p) => exactSubject(p) === 'vpio-02-sid' && p.includes('/VPIO-02-SID-ENTRY')).map((p) => p.replace(/ledger\.md$/, 'journals'));
...
describe('KERNEL-00 · SOURCE-ID-02 — seven-bin Goertzel at the consumed seam (evidence only) · gated 997 Hz fixture · evidence-only source reader · SID custody declarations; implementation is code/offline only', () => {
  ...
  it('the source reader is evidence-only: closed vocabulary, the amplitude-domain coefficients and the tail bound pinned, geometry fail-closed, frameReset before any attribution and the own-control law (SOURCE-ID-02A), self-test 24/24 naming every path; every tracked VPIO-02 journal reads UNMEASURED-SRC: no_source_evidence (never PASS/FAIL)', () => {
    ...
    const sidEntryDirs = sourceLedgers.filter((p) => exactSourceSubject(p) === 'vpio-02-sid' && p.includes('/VPIO-02-SID-ENTRY'));
  ...
  it('SID SOURCE-03 completed N=10 under zero-harness custody but source attribution is honestly UNMEASURED on all ten rows', () => {
    const d = join(process.cwd(), 'docs/programme/VOICE-2026/driver-ledger/VPIO-02-SID-SOURCE-20260916T151408Z');
    const journals = readdirSync(join(d, 'journals')).filter((n) => n.endsWith('.jsonl'));
    expect(journals.length).toBe(10);
    const src = readFileSync(join(d, 'source-ledger.md'), 'utf8');
    expect((src.match(/SOURCE-VALIDITY/g) ?? []).length).toBe(10);
    expect((src.match(/SOURCE-ROW/g) ?? []).length).toBe(10);
    expect((src.match(/\*\*UNMEASURED-SRC\*\*/g) ?? []).length).toBe(20);
    expect((src.match(/stimulus_not_visible/g) ?? []).length).toBe(10);
    expect(src).not.toMatch(/NEAR-END-SURVIVES|NEAR-END-SUPPRESSED/);
    const out = readFileSync(join(d, 'output-ledger.md'), 'utf8');
    expect((out.match(/\*\*PASS-06\*\*/g) ?? []).length).toBe(9);
    expect((out.match(/\*\*UNMEASURED-06\*\*/g) ?? []).length).toBe(2); // K00-06 + COUPLING for sample 8
    expect(out).not.toMatch(/\*\*FAIL-06\*\*/);
    for (let n = 1; n <= 10; n++) {
      expect(readFileSync(join(d, `sample-${n}-source-preplay-harness-state.txt`), 'utf8')).toContain(`sample ${n} preplay harnesses=0`);
      expect(readFileSync(join(d, `sample-${n}-source-jit-harness-state.txt`), 'utf8')).toContain(`sample ${n} jit harnesses=0`);
      expect(readFileSync(join(d, `stimulus-sample-${n}.tsv`), 'utf8')).toContain('custody\tVALID');
    }
    const adj = readFileSync(join(process.cwd(), 'docs/programme/VOICE-2026/driver-ledger/VPIO-02-SID-SOURCE-03-ADJUDICATION-20260916T151408Z/ADJUDICATION.txt'), 'utf8');
    expect(adj).toContain('RESULT COMPLETE N=10 · SOURCE ATTRIBUTION UNMEASURED');
    expect(adj).toContain('K00-06 constitutional standing UNCHANGED');
    expect(adj).toContain('KERNEL-00 acceptance NOT MET');
  });
...
describe('KERNEL-00 · SID ENTRY observer-liveness verifier (founder ruling 2026-09-15, tooling-only) — a validity input for [G], never an ENTRY class', () => {
  ...
  it('SID SOURCE successor 02 preserves guarded S-b physiology while using fresh one-shot identities after the spent 01 wrapper fall-through', () => {
    const pre = readFileSync(join(process.cwd(), 'docs/programme/VOICE-2026/SID_SOURCE-PREFLIGHT-02_PIN_DRAFT_2026-09-16.sh'), 'utf8');
    const batch = readFileSync(join(process.cwd(), 'docs/programme/VOICE-2026/SID_SOURCE-BATCH-02_PIN_DRAFT_2026-09-16.sh'), 'utf8');
    ...
    expect(pre).toContain('PTR=/private/tmp/sid-source-preflight-02-current.txt');
    expect(batch).toContain('ACT_MARK=/private/tmp/sid-source-batch-02-invoked.txt');
    expect(batch).toContain('PTR=/private/tmp/sid-source-preflight-02-current.txt');
    expect(batch).toContain('test "$AGE" -le 300');
  ...
  it('SID SOURCE harness-clear is a separate exact-set 3→2→1→0 native process act that preserves all installed subjects and contains no launch/install/XCTest path', () => {
    const p = readFileSync(join(process.cwd(), 'docs/programme/VOICE-2026/SID_SOURCE-HARNESS-CLEAR-01_PIN_DRAFT_2026-09-16.sh'), 'utf8');
    for (const c of ['0B07D423-97E7-4196-BC1C-C69C96F994BE','6A2E406B-D1B8-43A4-92F3-29D50333AF19','E3B88028-A10F-46B1-AB27-CF0A1F83FB78','85948DBD-BA8F-4679-950D-31767B1C24E5']) expect(p).toContain(c);
    ...
    expect(p).toContain('readset mid-k00 "$V1_CONTAINER" "$V2_CONTAINER"');
    expect(p).toContain('readset mid-vpio01 "$V2_CONTAINER"');
  ...
  it('SID SOURCE-03 post-prewarm wrapper observes DAS first, requires external authorities, and delegates only to pinned CLEAR/PREFLIGHT/BATCH acts', () => {
    const p = readFileSync(join(process.cwd(), 'docs/programme/VOICE-2026/SID_SOURCE-03_AFTER-PREWARM_RUN_DRAFT_2026-09-16.sh'), 'utf8');
    expect(p).toContain('idevicesyslog -u "$UDID" -p dasd --no-colors');
    expect(p).toContain('Rescheduling repeating task com.apple.appResume.prewarm');
    expect(p).toContain('CLEAR_AUTH=/private/tmp/sid-source-harness-clear-02-authority.txt');
    expect(p).toContain('BATCH_AUTH=/private/tmp/sid-source-batch-03-authority.txt');
    expect(p).toContain('bash "$CLEAR"');
    expect(p).toContain('bash "$PRE"');
    expect(p).toContain('bash "$BATCH"');
    expect(p.indexOf('grep -q \'Rescheduling repeating task com.apple.appResume.prewarm\'')).toBeLessThan(p.indexOf('bash "$CLEAR"'));
    expect(p.indexOf('bash "$CLEAR"')).toBeLessThan(p.indexOf('bash "$PRE"'));
    expect(p.indexOf('bash "$PRE"')).toBeLessThan(p.indexOf('bash "$BATCH"'));
