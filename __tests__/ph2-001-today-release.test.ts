...
// The tolerant scan is inline on both tiers (CTR-001 shape). Mirror it exactly and
// assert below that the shipped source matches this shape on BOTH tiers.
function pairTurns(turns: any): Array<{ userMessage: string; maiaResponse: string; timestamp: unknown }> {
  const pairs: any[] = [];
  ...
  for (let i = 0; i < turns.length - 1; i++) {
    ...
    pairs.push({ userMessage: userTurn.content, maiaResponse: assistantTurn.content, timestamp: userTurn.createdAt });
    i++;
  }
  return pairs;
}
const pairCrossSessionTurns = pairTurns;
...
check('consecutive user turns still yield a pair (CTR-001 scan)', () => {
  const p = pairCrossSessionTurns([U('early'), U('actual'), A('a1')]);
  assert(p.length === 1, `expected 1 pair, got ${p.length}`);
  assert(p[0].userMessage === 'actual', `CTR-001 pairs the adjacent user turn; got ${p[0].userMessage}`);
});
...
check('the fixed i+=2 stride is gone from BOTH call sites (code, not comments)', () => {
  assert(!/for \(let i = 0; i < crossSessionTurns\.length - 1; i \+= 2\)/.test(svc),
    'a fixed-stride pairing loop still exists');
  const tolerant = (svc.match(/i\+\+; \/\/ consume the assistant turn/g) ?? []).length;
  assert(tolerant === 2, `expected tolerant scan on both tiers, found ${tolerant}`);
});

// ── ITEM 2 — zero usable pairs is visible, not silent ────────────────────────
console.log('\n[2] absence is logged');

check('CORE emits zero-pair visibility', () => {
  assert(/Cross-Session Recall CORE\] Zero pairs from/.test(svc), 'CORE zero-pair log missing');
});
check('DEEP emits zero-pair visibility (same shape)', () => {
  assert(/Cross-Session Recall DEEP\] Zero pairs from/.test(svc), 'DEEP zero-pair log missing');
});
check('the absence log is NOT inside the pairs.length > 0 branch', () => {
  ...
  for (const tier of ['CORE', 'DEEP']) {
    const idx = svc.indexOf(`Cross-Session Recall ${tier}] Zero pairs from`);
    assert(idx > -1, `${tier} zero-pair log missing`);
    // CTR-001 carries a long rationale comment between `} else {` and the emission,
    // so the lookback must exceed it. Widened from 400 → 1400 chars.
    const before = svc.slice(Math.max(0, idx - 1400), idx);
    assert(/\}\s*else\s*\{/.test(before), `${tier} zero-pair log is not in the else branch`);
  }
});
