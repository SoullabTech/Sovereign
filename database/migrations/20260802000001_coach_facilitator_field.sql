...
-- (20260114000001, 20260116000001, 20260118_stellium): whichever migrates first wins
-- and the others silently no-op, so its live shape is not knowable from the repo.
-- Per the founder ruling, ambiguous legacy data is LEFT IN PLACE and this migration
-- neither reads, rewrites, nor reinterprets it. `coach_*` is a clean spine with an
-- explicit adapter seam (§ADAPTER below). Nothing here drops or alters a legacy table.
--
...
-- ═════════════════════════════════════════════════════════════════════════════
-- 1. RELATIONSHIP — the bounded container for everything below
-- ═════════════════════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS coach_client_relationships (
  ...
  client_member_id       UUID NOT NULL REFERENCES members(id) ON DELETE RESTRICT,

  -- The field this relationship lives in (joins practice_fields.field_slug).
  -- Carries the invitation's authorization context.
  field_slug VARCHAR(64),

  -- Status describes the RELATIONSHIP, never the person.
  ...
  -- relationship may still have a lawful basis to retain process records, and a
  -- lapsed basis must not be inferred from status alone.
  retention_basis TEXT NOT NULL DEFAULT 'active_engagement'
    CHECK (retention_basis IN ('active_engagement','continuity_of_care','legal_obligation','client_request','expired')),
  retention_reviewed_at TIMESTAMPTZ,

  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),

  UNIQUE (practitioner_member_id, client_member_id),
  CHECK (practitioner_member_id <> client_member_id)
);

CREATE INDEX IF NOT EXISTS idx_ccr_practitioner ON coach_client_relationships(practitioner_member_id, status);
CREATE INDEX IF NOT EXISTS idx_ccr_client       ON coach_client_relationships(client_member_id, status);

COMMENT ON TABLE coach_client_relationships IS
  'The practitioner-client relationship: the bounded container for every process record below. '
  'NOT a dossier about the person — a record of the relationship and the processes supported within it. '
  'status describes the process; retention_basis is deliberately a separate axis (founder ruling 2026-08-02).';
COMMENT ON COLUMN coach_client_relationships.retention_basis IS
  'Why records are still held. Modelled separately from status so that "completed" never silently '
  'implies "retain forever" and "former" never silently implies "delete". A future retention policy '
  'may redact the client-identifying link while preserving program-level aggregate facts only where lawful.';
...
-- ═════════════════════════════════════════════════════════════════════════════
-- 1. practitioner_clients (3 competing DDLs) — legacy relationship data is LEFT IN
--    PLACE. `lib/coachField/adapters.ts` holds read-only migration CANDIDATES; no
--    automatic backfill runs, because the live shape is not knowable from the repo.
--
