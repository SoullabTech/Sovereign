...
window.JFW_FIXTURES = {
  ...
  programme_state: {
    ...
    // F1-R1 (founder ruling): a consumer must be able to determine the projection population.
    // complete=false ⇒ every total derived from this projection must be scoped ("in this snapshot").
    population: {
      definition: 'a programme = a governing question with a charter or founder ruling in docs/programme, or an ops lane with a record in docs/ops, or an externally governed dependency named by founder ruling',
      source: 'F0 census §2.8 lane index + CLAUDE.md priority thread (bullets dated 2026-09-03…2026-09-23), read manually at b4f73ac4',
      examined: 26,
      emitted: 13,
      selection: 'programmes with a LATEST standing in the CLAUDE.md priority thread on or after 2026-09-13, plus the two external/ops dependencies the F0 adjudication named; older or closed lanes were examined and not emitted',
      not_emitted: ['JARVIS-KP-01', 'JARVIS-GOVERNED-KNOWLEDGE-FLOW-01', 'JARVIS-PROVIDER-01', 'JARVIS-PROVIDER-02', 'JARVIS-NEMOTRON-C3', 'JARVIS-CANONICAL-PROVIDER-EXECUTION-01', 'JARVIS-DESKTOP-OPERATOR-FLOW-02', 'JARVIS-RD-HUMAN-EXPERIENCE-ARCHITECTURE-01', 'JARVIS-PUBLIC-ACCOUNTED-FOR-01', 'S3 (closed in production)', 'CONTINUITY-REMAINDER-01', 'TEMPORAL-MEMORY-RECONCILIATION-01 (closed)', 'CMT-01'],
      unreadable: [],
      unclassified: ['JOP-05 / JOP-05A (planned, not bound)', 'JARVIS-CAPACITY-SENTINEL-01 (named, not opened)'],
      complete: false,
      why_not_complete: 'the population was assembled by hand from two prose sources; no deterministic projector has enumerated docs/programme, so exhaustiveness is not mechanically established'
    },
    programmes: [
  ...
  // Monitor — local-first (D-04). Each row: subject · axis · value · instrument · observed_at · freshness · evidence_state.
  // freshness (F1-R2): 'current' = observed on the snapshot date · 'historical' = observed earlier, so its present
  // applicability is unknown (a past PASS is a past PASS) · 'none' = never observed for this snapshot. Freshness and
  // value are separate axes; a historical good value never counts as "observed and fine now".
  monitor: [
    { group: 'This workspace', subject: 'Checkout', axis: 'branch / head / clean', value: 'claude/sharp-cannon-cyrdeb · 065b9c66 · clean', plain: 'Your working copy is clean and on the Founder Workspace branch.', level: 'good', instrument: 'git rev-parse / git status --porcelain (Desktop status probe, main.js)', observed_at: '2026-09-23', freshness: 'current', evidence_state: 'OBSERVED (this checkout)' },
    { group: 'This workspace', subject: 'Canonical freshness', axis: 'origin/clean-main-no-secrets vs census', value: 'b4f73ac4 = census SHA', plain: 'Canonical has not moved since the F0 census, so nothing here is stale on that account.', level: 'good', instrument: 'git fetch + rev-parse origin/clean-main-no-secrets', observed_at: '2026-09-23', freshness: 'current', evidence_state: 'OBSERVED' },
    { group: 'This workspace', subject: 'Work engine (Builder OS)', axis: 'session.mjs status', value: 'not observed — the console was not launched during F0', plain: 'JARVIS did not check its own work engine for this snapshot.', level: 'unobserved', instrument: 'node scripts/builder/session.mjs status --json', observed_at: null, freshness: 'none', evidence_state: 'UNOBSERVED' },
    { group: 'This workspace', subject: 'Local model worker', axis: 'Ollama 127.0.0.1:11434/api/tags', value: 'not observed', plain: 'Whether the local model is running was not checked for this snapshot.', level: 'unobserved', instrument: 'HTTP GET 127.0.0.1:11434/api/tags (main.js)', observed_at: null, freshness: 'none', evidence_state: 'UNOBSERVED' },
    { group: 'This workspace', subject: '"Run locally with JARVIS" (C1 lane)', axis: 'static integrity', value: 'FAILED — uses an undeclared name (REPO_ROOT) at main.js:1180-1192', plain: 'This button cannot work right now. The repair belongs to the lane that owns C1, not to this workspace.', level: 'failed', instrument: 'source inspection (F0 §6.1); runtime effect inferred, app not run', observed_at: '2026-09-23', freshness: 'current', evidence_state: 'OBSERVED (static) · INFERRED (runtime)' },
    { group: 'This workspace', subject: 'Local-native work lane', axis: 'label vs mechanism', value: 'labelled "read-only"; mechanism requires bounded worktree write', plain: 'The description of this lane is out of date. It is not reachable from the console today.', level: 'warn', instrument: 'source inspection: main.js:535 vs jarvis-runtime-pipeline.mjs:129', observed_at: '2026-09-23', freshness: 'current', evidence_state: 'OBSERVED (static)' },
    { group: 'This workspace', subject: 'External reasoning (frontier model)', axis: 'authority', value: 'manual only · requires external_ok=true · automatic C3 not authorized', plain: 'JARVIS may ask an outside model only when you say so, each time.', level: 'unauthorized', instrument: 'FRONTIER.validate (frontier-worker.js) · router law', observed_at: '2026-09-23', freshness: 'current', evidence_state: 'OBSERVED (source)' },
    { group: 'This workspace', subject: 'Recall prior work (continuity)', axis: 'script + database + python present', value: 'not observed on this machine', plain: 'Whether recall is ready on the Mac Studio was not checked for this snapshot.', level: 'unobserved', instrument: 'CONTINUITY.status (file existence: jarvis-recall.py, ~/.jarvis/continuity/continuity.sqlite3, python3)', observed_at: null, freshness: 'none', evidence_state: 'UNOBSERVED' },
    { group: 'This workspace', subject: 'Memory / Postgres', axis: 'connection', value: 'UNCONFIGURED by design', plain: 'The console never connects to a database. Memory state is read from the host, not from here.', level: 'unobserved', instrument: 'declared in main.js:518 (no probe by law)', observed_at: '2026-09-23', freshness: 'current', evidence_state: 'OBSERVED (source: deliberately not probed)' },

    { group: 'Mac Studio', subject: 'Internal disk', axis: 'free space', value: '76 GB free (84% used)', plain: 'The workstation has room again. Free space here doubles as memory headroom on this machine.', level: 'good', instrument: 'scripts/ops/workstation-storage-census.sh', observed_at: '2026-09-22', freshness: 'historical', evidence_state: 'OBSERVED (recorded in docs/ops/WORKSTATION_STORAGE_RELIEF_2026-09-22.md)' },
    { group: 'Mac Studio', subject: 'Swap / compressor', axis: 'GB', value: 'swap 10.4 · compressor 14.4', plain: 'Memory pressure eased after the stale servers were stopped.', level: 'good', instrument: 'workstation-storage-census.sh', observed_at: '2026-09-22', freshness: 'historical', evidence_state: 'OBSERVED (recorded)' },
    { group: 'Mac Studio', subject: 'Git worktrees', axis: 'count', value: '269 (0 removable left internally)', plain: 'Worktree cleanup is finished. Do not run the second reclaim act again.', level: 'good', instrument: 'scripts/ops/worktree-census.sh', observed_at: '2026-09-22', freshness: 'historical', evidence_state: 'OBSERVED (recorded)' },
    { group: 'Mac Studio', subject: 'Docker Desktop', axis: 'reserved', value: '12.25 GB RAM · 10 CPUs', plain: 'A quarter of unified memory is reserved for Docker and is invisible to ps.', level: 'warn', instrument: 'recorded in WORKSTATION_STORAGE_RELIEF_2026-09-22.md', observed_at: '2026-09-22', freshness: 'historical', evidence_state: 'OBSERVED (recorded)' },
    { group: 'Mac Studio', subject: 'Backups', axis: 'scheduled?', value: 'four backup scripts present; scheduling not observed', plain: 'JARVIS cannot see whether backups actually run. There are four overlapping scripts.', level: 'unobserved', instrument: 'scripts/backup-postgres.sh · backup-database.sh · backup-db.sh · maia-auto-backup.sh · setup-backup-cron.sh', observed_at: '2026-09-23', freshness: 'current', evidence_state: 'OBSERVED (files) · UNOBSERVED (scheduling)' },

    { group: 'Repository health', subject: 'Type errors', axis: 'count vs baseline', value: '239 baseline (2026-07-30); later lanes reported 229–230', plain: 'Known type debt, not a gate failure. It only fails if it gets worse.', level: 'warn', instrument: 'npm run typecheck:full vs typecheck-baseline.json', observed_at: '2026-07-30', freshness: 'historical', evidence_state: 'OBSERVED (recorded, 55 days old)' },
    { group: 'Repository health', subject: 'GitHub dependency alerts', axis: 'default branch', value: '764 (31 critical · 383 high · 289 moderate · 61 low)', plain: 'GitHub is reporting a large backlog of vulnerable dependencies. Nobody has instrumented it beyond this message.', level: 'warn', instrument: 'GitHub remote message during git push (Dependabot)', observed_at: '2026-09-23', freshness: 'current', evidence_state: 'OBSERVED (push output)' },

    { group: 'Production (soullab.life)', subject: 'Production', axis: 'any', value: 'not observed from this workspace · additional authority required', plain: 'This workspace does not look at production. Seeing it would need its own authority act naming transport, target, credentials, allowed observations, disclosure boundary, cadence and stop conditions.', level: 'unauthorized', instrument: 'none permitted (founder ruling D-04; main.js:519 "NOT PROBED")', observed_at: null, freshness: 'none', evidence_state: 'DELIBERATELY NOT OBSERVED' },
    { group: 'Production (soullab.life)', subject: 'Last known runtime', axis: 'GIT_COMMIT', value: '98542ff06 (recorded 2026-09-16)', plain: 'The most recent runtime SHA written into the record. It is a week old and may be stale.', level: 'unobserved', instrument: 'recorded in docs/programme/TEMPORAL-MEMORY-CUT1-TRACEABILITY-01_PRODUCTION_WITNESS_AND_CLOSURE_2026-09-16.md', observed_at: '2026-09-16', freshness: 'historical', evidence_state: 'OBSERVED (recorded, 7 days old)' },
    { group: 'Production (soullab.life)', subject: 'Co-Lab release gate', axis: 'passed / failed / warned', value: '33 · 0 · 0 on runtime ca5fdff44', plain: 'The last recorded run of the member-safety gate passed. It is 17 days old.', level: 'good', instrument: 'scripts/verify-constitution-colab.ts (in container)', observed_at: '2026-09-06', freshness: 'historical', evidence_state: 'OBSERVED (recorded, 17 days old)' },
    { group: 'Production (soullab.life)', subject: 'Completed deploys', axis: 'ledger', value: 'no durable ledger exists', plain: 'JARVIS cannot tell you when the last deploy happened, because the deploy lane keeps no append-only record (finding 2026-09-07).', level: 'unobserved', instrument: 'scripts/deploy-lock.sh writes holder metadata into the lockfile, overwritten on each acquisition', observed_at: '2026-09-07', freshness: 'historical', evidence_state: 'NOT FOUND (instrument absent)' },

    { group: 'Costs', subject: 'Provider spend', axis: 'any', value: 'Cost monitoring is not instrumented yet.', plain: 'There is no cost instrument, so no number is shown (founder ruling D-05).', level: 'unobserved', instrument: 'none', observed_at: null, freshness: 'none', evidence_state: 'NOT FOUND (instrument absent)' }
  ],
  ...
  graph: {
    nodes: [
      ...
      { id: 'br.fw', kind: 'branch', label: 'Founder Workspace branch', sub: 'claude/sharp-cannon-cyrdeb' },
      { id: 'c.b4f7', kind: 'commit', label: 'Canonical trunk at census', sub: 'b4f73ac4' },
      { id: 'c.8b85', kind: 'commit', label: 'F0 census record', sub: 'commit 8b8592d9' },
      ...
      { id: 'c.5189', kind: 'commit', label: 'O4 router implementation', sub: 'commit 51890bc0 · unwitnessed' },
      ...
      { id: 'sys.console', kind: 'system', label: 'JARVIS console', sub: 'the desktop app' },
      { id: 'sys.builder', kind: 'system', label: 'Work engine', sub: 'Builder runtime' },
