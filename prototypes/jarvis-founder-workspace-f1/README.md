...
## Open it
Double-click `index.html` (works from `file://`; no build, no server, no dependencies). It makes **no network request of any kind** — typography uses system faces only (F1-R5).

## Files
| File | Role |
|---|---|
| `index.html` | the page: all presentation and interaction; no data of its own |
| `fixtures.js` | every recorded value shown, each with `instrument`, `observed_at`, `freshness`, `evidence_state` and `source`. Values marked `ILLUSTRATIVE` are fixtures with no recorded observation and are labelled as such on screen. `programme_state.population` states the projection population and whether it is complete (it is not) |
