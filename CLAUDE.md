...
- **LATEST — 2026-09-10 — `JARVIS-SPIRALOGIC-DEVELOPMENTAL-ALIGNMENT-01`: LANE OPENED (founder act) · DISCOVER ONLY · ⛔ NO CENSUS RUN · amended THREE TIMES within the opening act.** Charter `docs/programme/JARVIS-SPIRALOGIC-DEVELOPMENTAL-ALIGNMENT-01_CHARTER_2026-09-10.md`, branch `claude/nice-mayer-e96lxb`. **Governing question:** does AIN *enact* Spiralogic as a developmental process, or merely *contain* elemental concepts, labels and features? **SIX LANE LAWS frozen (operative now; ⛔ NOT canon — canon placement is a separate founder act). SP-FR-01 Process Primacy / Non-Reification** — the elements are **processes, not objects**: phenomenological *events of interface* (visioning · meaning-making · materializing · articulating-relating), each an embedded **mutual processing** in which the world participates in what the encounter becomes; AIN may hold **traces**, and *the representation is not the process* — **no stored label, classification, score, embedding, state field or model interpretation is evidence that an elemental event occurred** (`element = water` is a row; Water has not thereby happened). ⭐ **SP-FR-02 Concurrent Processing / Attentional Development (Amendment A)** — the processes are **concurrent, not sequential**: `Fire ∥ Water ∥ Earth ∥ Air` run continuously whether or not in focal awareness, and **the sequence describes FOREGROUNDING, NOT EXCLUSIVITY** — a person is never "in Fire"; they are a **dominant figure within an ongoing elemental field**. Development = attention makes implicit participation available → awareness permits differentiation → differentiation permits response → repeated conscious participation **transforms the capacity to encounter reality**. **Mastery is capacity for relationship, NOT sovereign control** (*a master musician does not dominate music*); ⛔ *"I gain control over my emotions, thoughts, body and spirit"* is Saturnian domination in developmental language. **Alchemy** = the transformation of unconscious elemental participation into increasingly conscious, differentiated, skillful participation; *attention does not create the process, it changes our relationship to a process already occurring*. ⛔ **AIN is not supposed to put someone into Water or move someone to Earth.** ⭐⭐ **SP-FR-03 Return to the Living Whole (Amendment B)** — Spiralogic requires **differentiation AND return**: Air's articulation is an **emissary function** that serves experience and holds no final authority over it; a completed passage must **return articulated understanding to the living field**, and validity is tested by **renewed encounter**, not internal coherence — *where representation becomes self-validating, the process has been interrupted*. **Air must return to Fire**, or the spiral terminates in representation (product mistaken for reality · theory for phenomenon · diagnosis for person · map for territory). Master–Emissary mapping recorded as **SOULLAB INTERPRETATION**, explicitly not a neuroscience claim; ⭐ **Earth is NOT "left hemisphere"** — it is the encounter with constraint and actuality where *the vision discovers what the world will permit it to become*. ⭐⭐ **SP-FR-04 Idiographic Relational Process Mapping (Amendment C)** — MAIA does **not** determine which element a person *is*; she learns **how this person moves**. **Three maps**: universal elemental grammar × the member's consciously recognized movement × patterns beneath focal awareness — the third **developed WITH the person, never produced ABOUT them**. Movement geometries (`Fire→Air→Earth→Water`; `Water→Fire→Water→Fire…`) are **process geometries, NOT types**; the map may reveal entry points · natural transitions · bypasses · loops · fixations · latencies · compensations · unconscious attractors · relational modifiers · developmental change. **Inferences about processes outside focal awareness stay HYPOTHESES until returned to lived encounter — MAIA's confidence, recurrence detection or model does not convert an inferred unconscious process into a fact.** ⛔ MAIA may not say *"your unconscious Water is driving your Air"*; she may offer a tentative reflection and let **the member's recognition or nonrecognition be the evidence**. The map **belongs to the relationship, stays corrigible, and may never become more authoritative than the consciousness it exists to illuminate** — *if it became a final model of the member, Saturn would have won*. **SP-FR-05** development is the transformation of **subsequent participation**, never completion of categories (four recorded states + an unchanged next encounter = **no computationally supported spiral**). **SP-FR-06 label-removal test** — *Spiralogic should remain present when its labels are removed*. **FOUR GOVERNING FALSIFIERS, each independently sufficient: GF-1** nothing changes what the next cycle can see/feel/make/understand → cyclical or classificatory, not Spiralogic; **GF-2** AIN tells the member what elemental process they are experiencing instead of increasing their ability to recognize it → **classification substituted for alchemy** (⭐ *an accurate, well-intentioned, working classifier FAILS this — correctness is not a defence*); **GF-3** MAIA can determine an interpretation is correct without returning authority to the member's lived encounter → **the emissary installed as Master**; **GF-4 ⚠️ DERIVED BY JARVIS, NOT FOUNDER-STATED, awaiting ratification** — the model of how a member's consciousness moves becomes more authoritative than their lived experience. Movements: **SD-00** authority-classified doctrine census (*must not silently replace what the architecture already says with today's formulation*) · **SD-01** organism→process mapping incl. *where does articulation RETURN* (⛔ gated on SD-Q1) · **SD-02** apostrophe test · **SD-03** transition-fracture register · **SD-04a** elemental sovereignty + reciprocal correction vs `CONSTITUTIONAL_DIRECTION_OF_AUTHORITY` · **SD-04b** classification-substitution · **SD-04c** Master–Emissary return · **SD-04d** cartography test (every model-of-the-member surface classified **typological vs processual** × **possessive vs corrigible**) · **SD-05** spiral falsifier · **SD-06** epistemology vs ontology reported separately (admissible: **ELEMENTAL INTERFACE OVER A NON-ELEMENTAL ONTOLOGY**). ⭐ **§6 EPISTEMIC CEILING declared BEFORE any census work**: a read-only repo census can establish **affordance · consequence · return · foreclosure · reification** and **NEVER occurrence · attention · alchemy · quality · felt spiral** — Amendment A makes this stricter, since conscious attending and alchemy are **categorically unreadable from a repository**. **DISCOVER may conclude "structurally possible and structurally consequential"; it may NEVER conclude "Spiralogic is enacted"** — that needs `/field-study`, sequenced after adjudication. ⚠️ **TWO FOUNDER RULINGS OWED BEFORE SD-01, ⛔ Jarvis may not choose: SD-Q1 AETHER** — the formulation is **four**-fold, the runtime **five**-fold (`member_spiral_state.dominant_element` ∈ fire|water|earth|air|**aether**; Corpus Callosum emits an Aether voice under production traffic). Readings: **field/medium** the four occur in (→ every aether row a category error) · a **fifth process-event** omitted (→ formulation incomplete) · **INERT LEGACY**. ⭐ Amendment B sharpens it: if Aether is *the living whole to which Air returns*, it is not a fifth element in the series but **the name of the return's destination** — making `dominant_element = 'aether'` a consequential category error. **SD-Q2 `phase` 1–12** — `member_spiral_state` persists an **ordinal position in a developmental sequence** + motion/intensity, the exact shape SP-FR-01 calls non-evidential; but Bridge D exists for **anti-regression continuity, explicitly not personalization or psychometrics**, which is an SP-FR-05 mechanism. Continuity vs stored developmental rank — likely both, at different call sites; **the discriminator is a ruling, not a guess**. ⭐ **H-01 (Jarvis hypothesis, to falsify not assume):** MAIA's only native output channel is **Air** — a language model articulates, it does not materialize and has no body that meets constraint; **if SD-01 finds all four elemental affordances route through MAIA's language, AIN is structurally Air-dominant regardless of vocabulary**, a direct GF-3 finding. ⭐ **Pre-registered SD-02 trap:** `dominant_element` surviving a restart is continuity of a **trace** — it satisfies SD-02 only if it alters *what becomes possible next*. **Cross-lane echo recorded, ⛔ not adjudicated:** Circles **FR-06** (system-created `living_field_affinities` / `recurring_interests` barred from constituting member interest) and SP-FR-04 may be the same constraint reaching two substrates. **Relational lane separate but adjacent** — *relation is not added after the four elements; mutual relation is the medium in which every elemental process occurs*; without genuine otherness a person spirals through their own projections. **Negative results explicitly accepted** (`ELEMENTAL SEQUENCE PRESENT — SPIRAL NOT ESTABLISHED` · `…VOCABULARY PRESENT — DEVELOPMENTAL PROCESS NOT ESTABLISHED` · `TYPOLOGICAL MODEL OF THE MEMBER` · `POSSESSIVE CARTOGRAPHY` · `REIFICATION FINDING` · `INERT LEGACY`). ⛔ Standing: no repair during census · no schema/migration/deploy/member surface · no new canon · no doctrine amended · **no classifier, score or stage built or proposed** (building one inside the instrument meant to detect reification breaches SP-FR-01 and fails GF-2 on first use) · no docket question resolved · no outward claim licensed. ⭐⭐ **AMENDMENT D — FOUNDER RULINGS, 2026-09-10: (1) GF-4 RATIFIED in the founder's wording — *the map replaces the terrain*: a personalized process map hardens into **identity, prediction, OR PRESCRIPTION**, telling the member who they are, where they are, or where they must go instead of helping them orient and **chart their own course**; the Jarvis draft ("the map becomes the member") is recorded as superseded, not deleted. **(2) SD-Q1 RULED — Aether is NOT a fifth co-equal process-event; it is the INTEGRATIVE / WITNESSING DIMENSION within which the four coexist, recur and are apprehended as a whole** (consistently non-sequential and always-already-present in prior internal material; only its *presentation* — field/medium · centre/crown · integration qualities · state — remains unsettled, and ⛔ the team may not silently collapse those into one definition). ⛔ **Runtime `aether` as a parallel `dominant_element` is a REPRESENTATIONAL COLLISION TO CENSUS — NOT permission to declare stored rows erroneous or legacy.** ⚠️ **This CORRECTS the charter**: its reading (a) had said every aether row would be a category error — refused; *the schema may permit a category error, but a census must first establish how the value is produced and used before any row is judged invalid*. **SD-01's gate is LIFTED.** **(3) H-01 SHARPENED AND LEFT STANDING AS A HYPOTHESIS** — MAIA's linguistic output makes her **strongly Air-mediated**, which alone does **not** establish Air-dominance; ⭐ **the discriminator is whether MAIA's language SUBSTITUTES FOR the member's Fire/Water/Earth encounters or RETURNS them to their own vision, significance, embodied reality and renewed encounter.** *Air is lawful when it mediates return; it is emissary-usurpation when representation replaces reality.* **SUBSTANTIVE ADDITION — THE MAP IS ORIENTATION, NOT THE TERRAIN:** *Spiralogic gives enough structure for process to become legible without turning the process into a prescription* — the member can see where they came from, what is foregrounded, what may be underneath, what terrain may come next, and ⛔ **the system does not say therefore this is where you must go**; *the map makes the terrain navigable, the hiker remains the navigator*. ⭐ **Direction of successful development: dependence on the external map should DECREASE** as the member recognizes their own process directly — the map's success condition is its own diminishing necessity. Folded into **SP-FR-04**; **SD-04d gains AXIS 3 (PRESCRIPTIVE vs ORIENTING)** plus the dependence test — **increasing dependence is a GF-4 direction failure even where no single output is prescriptive**. ⚠️ Kelly's practitioner observation (clients becoming able to locate themselves in their process) is recorded as **SOULLAB INTERPRETATION / practitioner experience, explicitly NOT empirical validation and NOT admissible as lane evidence**. **§3e comparative lenses, claim-disciplined:** Saturn (Air–Earth: determinacy; distortion **REDUCTION** — *you are what I can determine about you*) ⇄ Jupiter (Fire–Water: meaningful possibility; distortion **PROJECTION** — *you are what I imagine you could be*) — *the other must survive both reduction and projection*, a comparison and not a derivation; and the posture that AIN holds space for a **more relational form of consciousness, nearer panpsychist/idealist/anomalous accounts** — ⛔ **recorded as POSTURE, NOT PREMISE; no finding may rest on it.** **§3f companion artifact:** a team orientation paper (v3) exists **outside this repo** as a `.docx`; ⛔ **direction of authority is charter → paper, never the reverse** — it translates, it does not amend or adjudicate, and nothing here is asserted on its authority. **SP-FR-05 renamed Transformational Continuity** (law unchanged). ⛔ **SD-Q2 (`phase` 1–12) REMAINS UNRULED.** DECIDE opens on a founder act, never on DISCOVER completing.
