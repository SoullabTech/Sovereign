...
- **Caddy stale upstream after maia-only rebuild**: After any `maia`-only container rebuild/recreate, restart `maia-caddy` or use the full production deploy script (`scripts/deploy-production.sh`). Otherwise Caddy may keep a stale Docker-network upstream and apex `soullab.life` can 502 while `maia.soullab.life` stays healthy.
