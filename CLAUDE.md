...
- **LATEST — 2026-09-06 — JARVIS-RD-HUMAN-EXPERIENCE-ARCHITECTURE-01: foundational challenge pass R8–R12 CLOSED / COMPLETE (founder act); Synthesis v0.2 ACCEPTED as current programme synthesis (not doctrine; P1–P13 CANDIDATE; empirical postures provisional; v0.1 historical); reconciliation RUN (`docs/programme/…_CLAIM_RECONCILIATION_2026-09-06.md`); merge ruling on #1239 pending as a founder stop. **PIVOT (founder, same day): cockpit of record is `docs/programme/JARVIS-HUMAN-EXPERIENCE-MASTER-RUN-v1.md` — Phase 1 whole-organism map CURRENT; manifesto expansion FROZEN; research/new theory STOPPED; Phase 1 EXITED on `573d5268` (founder adjudication, `docs/programme/MAIA_WHOLE_ORGANISM_MAP/00_RANKED_MAP.md` §9); **Phase 2 OPEN** — register `docs/programme/HUMAN_EXPERIENCE_GAP_REGISTER.md`: Plane A constitutional decisions (A1 no OpenAI voice egress outside R15 · A2 ephemeral = no write, Sanctuary governs cognition · A3 inference ≠ member-authored fact · A4 claim discipline · A5 X1a · A6 X9a product offer) sequenced before any experiment; Plane B ranking 1–10 (correction instrument first; plurality E5 → E11 → E6, never inject four interpretations first). Read-only production censuses AUTHORIZED (spec `docs/programme/PHASE2_READONLY_CENSUS_SPEC_2026-09-06.md`; run from minisforum). Shadows sequential: **E1 correction-candidate shadow BUILT on the Phase-1 branch** (`lib/maia/canonical-turn/correctionShadow.ts`, `/list` log-only block after cognition, kill-switch `MAIA_CORRECTION_SHADOW=0`, refusal R32) — **UNDEPLOYED, deploy = founder act**; next in order E5 offline → E7 shadow → E11/E6 → E12. Human witnesses design-only. **Phase 2 instruments COMPLETE and accepted (2026-09-06): E1 deploy candidate `0780f368` (isolated); pp-2 closed (R30 GREEN); read-only census C1–C18 accepted (`scripts/witness/phase2-readonly-census.sql` + `-logs.sh`; C3 observation-only; C2/C5 PROXY, C15/C18 NOT OBSERVABLE, C19 not authorized); E5 v2 accepted, synthetic coding authorized, external rater outstanding. FOUNDER DIRECTION: stop designing, collect evidence — no further instrument in this lane until observations exist to design against. **An observation never triggers a repair:** every result returns to `HUMAN_EXPERIENCE_GAP_REGISTER.md` as an evidence-state change (observed · downgraded · falsified · still unknown · instrument-inadequate), and the next founder act decides what deserves intervention. Sequence: E1 deploy → census (C5 early) → E5 synthetic.** Cat 4 = quarantine, current not exhaustive. Governing sentence: *MAIA's next problem is not becoming more intelligent; it is making the sources, limits, corrections, and consequences of its intelligence genuinely relational.*** **Lane split 2026-09-06:** `/accounted-for` + PR #1239 (branch `claude/maia-human-experience-arch-12g5r6`, contained at `a7b42f29`) now belong to `JARVIS-PUBLIC-ACCOUNTED-FOR-01`; R&D continues on `claude/maia-human-experience-phase1-census` (cut at `a7b42f29`) — merge and deploy NOT AUTHORIZED; MAIA runtime and memory runtime unchanged. Record: `docs/programme/JARVIS-RD-HUMAN-EXPERIENCE-ARCHITECTURE-01_CHARTER_2026-09-06.md` §22; spine under `docs/research/human-experience/`. **Episodic Phase 2 spec inputs added by the R12 ruling (Cat 1, not authorized):** the four-type memory typology (episodic · semantic · pattern · state-responsive, v0.1 source [22] — located, vendor-authored, conceptual, override risk-gated) is WITHDRAWN as a supported decomposition; working decomposition is **Authority × Time** (`docs/research/human-experience/frameworks/memory/AUTHORITY_X_TIME_2026-09-06.md`) with verbatim beneath derived; R7b precise — *member statement overrides as present self-report (does not rewrite history); MAIA impression asks* (= detect → ask → record with an evidence basis); five directions: preserve qualification · change-sensitive retrieval outranks similarity only under temporal conflict (the F2 retrieval cut becomes a safety question) · implicit contradiction → question, not transition · derived stays visibly derived · memory is never leverage. Brain-training rule is programme doctrine: *a gain on the practiced task is never, by itself, evidence of a gain in life.*
