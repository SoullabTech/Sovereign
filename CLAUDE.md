...
- **LATEST — 2026-09-07 — `JARVIS-CIRCLES-01 · CIRCLE-04 REPAIR`: R1+R2+R3 VERIFIED ON CANDIDATE; R4 IMPLEMENTED, NOT VERIFIED — one failure left.** Branch `claude/jarvis-circles-programme-reouzc`; rulings `…_FOUNDER_RULINGS_2026-09-06.md` (**FR-01…FR-13** + cross-lane principle; open **CA-01…CA-15**); records `…_REPAIR_R1_VERIFIED…`, `…_REPAIR_R2_2026-09-06.md`, `…_REPAIR_R2_VERIFIED_R3_2026-09-06.md`, `…_REPAIR_R3_VERIFIED_R4_2026-09-07.md`. **Verification ladder, all founder-run against DISPOSABLE SHADOW DATABASES restored from production schema (R2 migration applied to shadow ONLY; shadow + worktree deleted after; production received NO writes and is NOT migrated):** R1 `ca1c4736` → `18 passed · 5 failed`; R2 `8f31abc8` → `28 passed · 2 failed` (C7, C8, **T3a–T3i all PASS**); **R3 `d1742472` → `34 passed · 1 failed · 0 warned · 0 skipped`** — **S4a 1→FORMING · S4b 2→FORMING · S4c 3→ACTIVE · S4d 3→2→FORMING · S4e 2→3→ACTIVE, all PASS; C14 PASS**. ⭐ **CUSTODY: `R1 = R2 = R3 = VERIFIED ON CANDIDATE / PRODUCTION CLOSURE PENDING eventual deploy`.** *Verified ≠ deployed.* Production still `4 circles · 4 memberships · 0 shares · 0 inquiries · 0 responses`, `circle_membership_removals` **ABSENT**. **Only C6 remained → R4 (implemented this act).** **R4 — participation count REMOVED, not hidden.** Census matched exactly three sites, all gone: the correlated `COUNT(*)` in `listInquiries()`, `response_count` in its return type, and the stale client-type declaration in `FieldMemory.tsx`. **Nothing consumed it** — `FieldMemory.tsx` declared the field and never rendered it — so there was no internal metric to preserve while suppressing presentation; it was simply unnecessary member-facing data. ⛔ **Not substituted with** `has_responses` · percentages · progress · participation ratios · completed-member counts · badges · any qualitative proxy from participation quantity. *The repair is about not turning participation into a social signal in the first place* — reasoning recorded **at the site** so it is not reintroduced as a convenience. **SCOPE:** ⛔ do **not** infer that PostgreSQL may never count rows — **FR-08.7 concerns Circle SOCIAL SURFACES and member-facing status mechanics**; counts stay legitimate for constitutional derivation, authorization, integrity checks, verification, operations. `constitutionState.ts` counting active memberships to derive FORMING/ACTIVE **is constitutional derivation, not social ranking**. **C6 made DESTINATION-AWARE** — asks two destinations (the member-facing inquiry listing; Circle surface components) for `response_count`/`responseCount`/`participation_count`/`participationCount`, strips comments before matching, and ⛔ **deliberately does NOT sweep Circle code for `COUNT(*)`**, which would flag the legitimate derivation. **C14 KEPT (founder ruling)** — a dependency-boundary falsifier, useful though it cannot make TypeScript's structural typing safe; **CA-14 stays OPEN** (identical string values still permit mix-ups outside those two modules); **C14 is containment, not resolution**; FieldPhase not renamed or refactored. ⭐ **PRINCIPLE RECORDED (founder, cross-lane, NOT ratified as Circle law): *Prefer derived facts over declared system state when the authoritative fact already exists and the derivation is deterministic.*** Narrower and safer than *all state should be derived* — **FACT DERIVATION** (the system reads what is already true: a Circle's plurality) vs **MEANING DECLARATION** (a person authors what it means: whether a Circle is complete, a relationship repaired, something new being born). The second class **requires human acts of meaning and authority and must never be derived** — this names why FR-13 works and why CA-04's richer lifecycle stays separate. ⛔ **R4 NOT VERIFIED — remote sessions have no `DATABASE_URL`/`node_modules`.** If correct, VERIFY reaches its **first full candidate `0 failed`**; no required total manufactured. **⛔ ON A FULL PASS, DO NOT PROCEED TO INVOKE — `R5 · CLASS-B RE-CENSUS` comes first:** re-read every remaining Class-B item against the repaired candidate (containment-plan reference · inquiry-response withdrawal · inert `visibility`/`invite_enabled` · pulse defence-in-depth · `integrating` one-way state · refusal-screen navigation · facilitator-assignment gap · any new contradiction from R1–R4) and classify each **STILL DEFECT · RESOLVED INDIRECTLY · SUPERSEDED BY CONSTITUTION · DEFER TO INVOKE · DEFER TO OBSERVATION · DOCUMENTATION/PROVENANCE ONLY**; ⛔ **no repair during that census.** **Standing: R5 NOT STARTED · DEPLOY NOT AUTHORIZED · INVOKE NOT AUTHORIZED · COHORT NOT AUTHORIZED · R2 migration not applied to production · no facilitator assignment (CA-15) · founder UI gate untouched · doctrine byte-identical · no number ratified.**
