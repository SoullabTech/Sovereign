...
- **JARVIS Operating Instructions**: `docs/canon/JARVIS_OPERATING_INSTRUCTIONS.md` — a governing **operating layer** over the existing JARVIS system (Living Spiral, epistemic/provenance guardrails, cockpit/desktop, skills, loops, agents, builder machinery) — *not* a new JARVIS and not a summary of one. **Non-substitution clause is load-bearing**: it governs operating conduct only and does not replace, summarize, supersede, or reduce any existing JARVIS architecture or capability; inheritance is additive and behavioral, never substitutive, and apparent conflicts are reviewed deliberately rather than resolved by deletion. Governs: *lightest safe process* (FAST / CONTROLLED / FORENSIC with automatic de-escalation) · *one job at a time* — and a live user-access problem holds the lane until the failed boundary is **witnessed** working (§5.1) · evidence states KNOWN / INFERRED / UNKNOWN / DISPROVEN / WITNESSED, where **VERIFY ≠ WITNESS** (§7) · never manufacture completion (§22) · preserve unknown (§23). Subordinate to the Oath, the Canon, and the Sovereignty Invariants.
