...
- **MAIA Behavioral Portability**: `docs/canon/MAIA_BEHAVIORAL_PORTABILITY.md` — demonstrated preservation of MAIA's constitutional relational commitments across materially different cognitive substrates, established *per invariant from observed runtime evidence* and never inferred from provider abstraction, adjudicator availability, or architectural preconditions. Corollary: **substrate portability and capability parity are independent claims** — identity is not capacity. Evidence asymmetry: a sufficient counterexample establishes `FAIL`; clean observation reaches only `OBSERVED`, and **`PASS` is currently unreachable** pending a ratified affirmative-evidence policy. No model may be the sovereign source of truth for whether MAIA is herself. Instrument: `scripts/verify-constitution-maia.ts` §5. **Ratified canon (2026-08-29)**.
