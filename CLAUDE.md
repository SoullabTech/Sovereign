...
- **LATEST — 2026-09-14 — ⭐⭐ `WRITER'S STUDIO EDITORIAL PROGRAMME`: the proposal/write membrane is CLOSED and WITNESSED; the current proposal panel is FROZEN INSTRUMENTATION, not the product.** Branch `claude/s3-implementation`, tip `ff4100636`. **`JARVIS-WRITERS-STUDIO-EDITORIAL-01`** (`docs/programme/JARVIS-WRITERS-STUDIO-EDITORIAL-01_CHARTER_2026-09-14.md`) now governs the path to the conversational MAIA editing experience; its criterion is `docs/canon/WRITERS_STUDIO_EDITORIAL_OBJECTIVE.md`. **Next engineering priority: locus-scoped authored proposal succession**, with any schema/migration in its own authorized lane/branch (the 2026-09-07 branch-gate defect is controlling). ⭐ **An editorial ruling may govern multiple independent locus succession chains but is NOT executable authority.** In parallel, continue the UX/research track — ⛔ do not freeze the final surface before succession exists. **Materials remains the Work Continuity object** — *bringing establishes belonging; disclosure establishes readability; neither implies the other* — and ⛔ MAIA's readings of material relationships remain **separately authored insights, never member facts**; **material disclosure must reuse the S3 disclosure/authorization grammar rather than introduce a second mechanism.** **Immersive writing is a LAYOUT STANCE over the same mounted writing session**, using existing region withdrawal (`page.tsx:512-566`) and Held Focus (`useHeldFocus`); ⛔ entering or leaving it must not remount the writing surface, request new write authority, or cause a manuscript write — *a change of attentional stance is not an edit.* **Witnessed today:** EW-F2 step 2 CLOSED under custody `0bd2b6578` (`docs/programme/EW-F2_STEP2_WITNESS_RESULT_2026-09-14.md`), EW-F1a structurally witnessed in `maia_focus_witness` (`mrp_inspection_only_never_accepted` and the immutability trigger each refused by name), panel frozen at `e5b363995`. ⛔ **Production UNTOUCHED; v35/v36 remain `system acceptance REAL · authorial ratification UNRESOLVED`, outside this programme; `972adae6` is a permanently stale base-34 record.** **The six distinctions the next session is most likely to collapse: proposal ≠ authorization · ruling ≠ wording · member declaration ≠ MAIA insight · belonging ≠ disclosure · focus ≠ immersive stance · attentional change ≠ manuscript write.**
- **LATEST — 2026-09-10 — ⭐⭐ `S3 · P1` WRITER-FACING ASK: **STEP 7 CLOSED · `60 passed · 0 failed`.** Founder's machine, from a **clean detached worktree of `785dab0de1ef274cee55c632e6c3fab1fee4a9ab`** (`git status --porcelain` empty), real `next dev` on a qualified database (`526 migrations · 0 failed · VERDICT: QUALIFIED`), real development credential (`credential value NOT RECORDED`). **W0 green** — the earlier run's sole failure was untracked scratch in the main checkout, and the honest repair was to re-run from an isolated worktree rather than delete unrelated files or argue with the check. **Walk 13 green, all nine**: `BODY_AUTHORIZED` · disclosed/withheld scopes as actually used · zero unverified evidence · **answer provenance from the canonical model path** · MAIA's own words, not an echo · **the same thread**, grown by both turns · crossings `crossed`. Walks 1–12 and R1 green in the same run; geometry `7084595c…` identical at open and close. ⛔ **Both `unreachable` failures had ONE cause and it was never the protocol**: `.env.local` acquired a **second** `ANTHROPIC_API_KEY` line each time the append ran twice, and `grep | cut -d= -f2-` glued them into a 217-char value containing a newline — curl refused it (error 43) and the SDK failed identically. Dedupe to one 108-char line · restart the server so it re-reads the file · `head -1` in the export. **Standing: S3 server CLOSED · migration acceptance CLOSED · writer-facing UI BUILT · recognition CLOSED (F1·F2·F3) · R1 PASS · authority/protocol PASS · terminal cognition PASS · STEP 7 CLOSED · writer-facing P1 ACCEPTED.** ⭐ **Next: D9/S3 integration acceptance → D9-PHENOMENOLOGY-WITNESS-01 (UNSPENT) → production preflight (NOT STARTED).** ⛔ **production migration NOT AUTHORIZED · production UNTOUCHED.** ⚠️ **TWO FINDINGS REMAIN OPEN, NEITHER REPAIRED: (1) the last boundary cannot be diagnosed** — `lib/ai/structured/router.ts` captures the provider's real error into `detail`, `lib/manuscript/ask/developmentalAskReader.ts:207` discards it under a bare `catch` with no logging; both of today's failures surfaced as the single word `unreachable` with no cause, and hours of delay are directly attributable to that; **(2) walk 12's fault injection is not crash-safe** — it renames `runtime_consent_state` and restores it in a `finally`; an interrupted run leaves it renamed, and it wedged a live dev database once today. It must point at a disposable database or restore on startup. ⚠️ **Environment defects to fix before the next witness**: `scripts/witness/step7-walk-db-qualification.js` shells out to `psql` and reads `PG*`, **not `DATABASE_URL`** (workaround: export `PGHOST/PGPORT/PGUSER/PGDATABASE`) · `read -p` is bash, macOS runs zsh (`read -s "K?prompt: "`). **Record: `docs/programme/S3_STEP7_WALK_RESULT_2026-09-10.md` (Addendum 4).**
- **PRIOR — 2026-09-10 — the run before the clean re-run — `59 passed · 1 failed`.** Walk 13 ran against a real `next dev` on a qualified database (`526 migrations · 0 failed · VERDICT: QUALIFIED`) with a real development credential (`credential value NOT RECORDED`), and met **every** obligation: `BODY_AUTHORIZED` · disclosed/withheld scopes as actually used · zero unverified evidence · **answer provenance from the canonical model path** · MAIA's own words, not an echo · **the same thread**, grown by both turns · crossings recorded `crossed`. Walks 1–12 and R1 passed in the same run. ⚠️ **The single failure is `W0 the working tree is clean`, and its content matters**: every reported entry carries git's `??` prefix — `.jarvis/`, `data/vault/`, `maia-jest-cache/`, a stray `55`, two loose scripts, one doc — **all untracked; NO tracked file differs from `785dab0de`**, so the application that ran IS that commit. The check is right to fail (it cannot distinguish untracked scratch from modified source, and one that guessed would be worse); recorded as hygiene, and the SHA attribution rests on the tracked-file fact, not on the check. ⛔ **Three environment defects cost the afternoon, NONE in the protocol**: two `ANTHROPIC_API_KEY` lines in `.env.local` glued by `grep|cut` into a 217-char header (curl error 43 · the SDK failed identically) · `read -p` is bash and macOS runs zsh · **`scripts/witness/step7-walk-db-qualification.js` shells out to `psql`, which reads `PG*` and NOT `DATABASE_URL`** (workaround: `export PGHOST/PGPORT/PGUSER/PGDATABASE`; it should read the connection string). ⭐ **FINDING THAT STANDS: the last boundary cannot be diagnosed.** `lib/ai/structured/router.ts` captures the provider's real error into `detail`; `lib/manuscript/ask/developmentalAskReader.ts:207` discards it (`if (!outcome.ok) return { ok: false, refusal: 'unreachable' }`) with a bare `catch` below and no logging — a genuine terminal-cognition failure surfaces as one word with no cause. **Not repaired.** ⛔ **AND THE WITNESS DAMAGED A LIVE DATABASE**: walk 12 renames `runtime_consent_state` and renames it back in a `finally` — written for a disposable cluster, run against the founder's working dev database on this session's instructions; an interrupted run left it renamed and every request 503-ing until repaired by hand (`ALTER TABLE runtime_consent_state__witness_offline RENAME TO runtime_consent_state`). Nothing lost, but **the fault injection is not crash-safe and must not point at a database anyone uses.** Repair not authorized. **Record: `docs/programme/S3_STEP7_WALK_RESULT_2026-09-10.md` (Addendum 3).**
- **PRIOR — 2026-09-10 — `S3 · P1` STEP 7 UPSTREAM WALKS (the run that preceded the terminal gate).** Branch `claude/s3-implementation`, tip `4eb2d281c`. **Walk result `51 passed · 0 failed · 1 NOT WITNESSED`.** Records: `docs/programme/S3_STEP7_WALK_RESULT_2026-09-10.md` (+ two addenda) · `…_WALK_DB_QUALIFICATION_2026-09-10.md`. ⭐ **The disclosure architecture held under every shape it was pressed on**: structure-only Ask raises no authorization surface · a body Ask pauses and **crosses nothing** · one press → one `actId` → one section-scoped receipt `crossed` on `writers_studio.ask->maia_developmental` · the same press twice → `ACT_ALREADY_PROCESSED` with truthful `completion` and **no second crossing**, a *different* act → `ALREADY_CONSUMED` · partial authorization → `BODY_SCOPE_INCOMPLETE` with **nothing crossed, not even the section that WAS authorized**, act unspent · multi-section → **N receipts sharing ONE `request_ref`**, authority still section-scoped · client-asserted `sections`/`heading`/`label`/`allowBody` changed nothing (server re-derives) · continuity refusals each stay themselves (404 `unknown` · 410 `expired` · malformed act **not** downgraded · foreign member 404 `not_found`) · the sixth state is real (consent substrate made genuinely unavailable → `DISCLOSURE_UNAVAILABLE`, `actSpent: true`, nothing read, claim spent in the record) · **R1 Work geometry byte-identical at open and close.** ⛔ **Do NOT reopen P1, S3, act identity, multi-section cardinality or D9 geometry** — the two walk failures were **recognition**, and are closed: **F1** two sections inside one structure unit were both offered as *"Before the water"* (a unit names a DIVISION; sections are finer), repaired by qualifying the label with canonical position — `heading` stays authored-or-null, the qualification is **unconditional** so the label is a pure function of `(heading, position)` (*a section whose name depends on its company does not have a name*), and distinctness is **structural** via `UNIQUE (draft_id, position)`; **F2** the displayed order was UUID-lexicographic because the result was re-keyed by the input array, discarding the query's `ORDER BY s.position` — the input now decides **membership only**; **F3** found by the F2 falsifier in the fallback branch, which asserted a canonical position it did not know → `Section N (unrecognized)`. Falsified: 7 of 23 `sectionRecognition.test.ts` obligations fail against the pre-repair implementation. ⚠️ **The 11 `askRouteBodyGate` tests repaired alongside were PRE-EXISTING `actId` staleness (confirmed at `HEAD` with the repair stashed) and are NOT discrimination evidence for the recognition repair** — do not let them be cited as such. **THE ONE OPEN OBLIGATION IS ENVIRONMENTAL, NOT ARCHITECTURAL:** `scripts/witness/s3-step7/walk.ts` **walk 13** — a real body-required Ask → real member act → real model call → `BODY_AUTHORIZED` → the answer returning into the **same** thread. It needs `ANTHROPIC_API_KEY` through the normal environment mechanism (`MAIA_INFERENCE_MODE` unset or `primary`; `local_only` refuses — there is no local structured provider). ⛔ **NO MOCK, NO PROVIDER BYPASS, and do NOT repurpose the agent/session credential** — the gate exists to witness the app's own provider path. `NOT WITNESSED` is a **first-class result**, never a skip and never a pass; `--require-cognition` makes it an exit failure. **Standing: S3 server CLOSED · migration acceptance CLOSED · writer-facing UI BUILT, upstream walks green · recognition CLOSED · R1 PASS · walk-13 instrument READY · real cognition NOT WITNESSED · STEP 7 OPEN · D9-PHENOMENOLOGY-WITNESS-01 UNSPENT · production migration NOT AUTHORIZED · production UNTOUCHED.** ⛔ **Do not spend the phenomenology witness or begin production preflight while Step 7 is open** — both depend on knowing the real Ask can complete. Read-only D9/S3 integration census material may be prepared; **integration is not accepted until walk 13 passes.** ⚠️ **The walk environment is EPHEMERAL** — cluster at `/var/lib/postgresql/walk` (:5433) and the `next dev` server on :3100 do not survive the container; rebuild via the qualification record, whose verdict is scoped to the Writer's Studio / S3 path and attests nothing about the 16 unrelated failed migration lanes.
- **PRIOR — 2026-09-10 — type-health gate defect (repaired).** `npm run typecheck` keyed diagnostic identity on tsc's raw message, but tsc's union print order is not stable across program compositions — adding ten unrelated files re-printed three already-baselined errors with their union members permuted, and the gate reported each as simultaneously *fixed* and *NEW*. Repair is order-only (`scripts/lib/typehealth-identity.js`): members **sorted, never deduplicated**, applied to both sides of the comparison before the key is built, `typecheck-baseline.json` **not rewritten**, `message` still exactly as tsc printed it. A failing gate no longer prints `Lock it in with: npm run typecheck:baseline` — *an instrument that can be quieted by the action it recommends is not a gate.* Falsified by `npm run typecheck:identity-witness` (34/34, red against three known-bad implementations) and by a real tsc probe. Record: `docs/ops/TYPEHEALTH_GATE_UNION_ORDER_DEFECT_2026-09-10.md`.
