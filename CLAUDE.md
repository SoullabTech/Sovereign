...
- **LANE OPENED — 2026-09-06 — MAIA-SHADOW-FIELD-01 (MAIA Shadow Field — Voluntary Encounter with the Unowned Self). DEFINED · NO CODE.** Split by founder ruling out of the whole-organism census lane (`JARVIS-HUMAN-EXPERIENCE-MASTER-RUN-v1` Phase 1); the census lane records one bounded Phase-2 opportunity and nothing else. Object: one governed **Field** (a voluntarily entered mode of relationship with its own purpose, epistemology, boundaries, practices, memory rules, exit conditions) with two entrances — Dedicated (member enters) and Invoked (member asks mid-conversation). **Entry by member choice, never classifier inference.** Seven candidate laws (Entry · Interpretive · Memory · Plurality · Relational · Exit · Dependency), six-register epistemic grammar, Elements as questions not scores, arc Encounter → Stay → Differentiate → Reclaim → Choose → Return, memory covenant session-bound-by-default with member-worded adoption only (first member-facing detect → ask → record; feeds Episodic Phase 2). Five constraints inherited from census findings X1/X2 · X8 · X10/S5 · X3/X4/X7 · ranked gap #2. Jarvis flow **SHADOW-01: DISCOVER → FALSIFY → CONSTITUTE → founder ratification → DESIGN → ▌HARD STOP▐ → PROTOTYPE → WITNESS → PROMOTE** (re-sequenced by founder at DISCOVER go: falsifiers before the constitution is ratified; prior art recorded as *potential conflict with candidate laws*, not violation) — no prototype until the stop is lifted by an explicit founder act. **DISCOVER CLOSED / ACCEPTED 2026-09-06** with fifteen founder dispositions (INHERIT the Journal Shadow Work surface + its open questioning; QUARANTINE system selection, house pathology, the live Guardian, tracker state, `shadow-insight` code, the shadow elemental voice, `agent_runs` shadow rows; RETIRE the Guardian's declarative, its injected projection doctrine, trigger vocabulary as classification, the phantom `shadow-integration-space` tool, the tracker's progress model, and `shadow-insight`'s hidden-model design principle) — census §12. **RUNTIME UNCHANGED; the live Guardian is preserved as falsification specimen S1, not repaired in this lane.** **FALSIFY CLOSED / ACCEPTED 2026-09-06 — Acceptance Instrument v1: F1–F14 ratified with amendments, F15 (dependency capture) + F16 (etiology / trauma manufacture) added, 0 struck** (FALSIFY §7; ratifying the instrument does not ratify L8′; F5 does not decide turn vs session). **CONSTITUTE: D1–D6 RULED 2026-09-06** (CONSTITUTE record §9 — D1 turn-scoped durability, spoken readings carried by the transcript but not memory; D2 A′ visible ordinary-turn offer, no offer-producer component; D3 practitioner = source not register; D4 member-chosen symbolic lens only, astrology out of prototype v1; D5 L8′ ratified as Field law L8, Invariant promotion post-WITNESS only; D6 Field-only with *assembly sovereignty* from the activation turn). **CONSTITUTE CLOSED / RATIFIED 2026-09-06 — Shadow Field Constitution v0.2 RATIFIED (lane-scoped: no Invariant promotion, no ordinary-path change, no runtime code). DESIGN CLOSED / ACCEPTED 2026-09-06 — claim state DESIGNED · NOT LIVE · HARD STOP ACTIVE · PROTOTYPE NOT AUTHORIZED. SHADOW-01 instantiated as a named Jarvis Flow (`docs/programme/JARVIS-SHADOW-01_FLOW.md`: cockpit · five Jarvis responsibilities · HARD STOP node card with JARVIS MAY / MAY NOT · PROTOTYPE subflow P0–P8 shape · runway). Runway finding: the canonical producer registry is closed (R30) and on the CMT-01 single-writer-frozen branch, so v1 should be a separate room on the Relational Navigation pattern (own route + prompts, direct model call, sanctuary flag, never `/list`), which satisfies assembly sovereignty structurally with zero `/list` blast radius and zero migrations (keep act = `spontaneous` atom + `provenance` JSONB); the `shadow-field` registry line is needed only when the Invoked entrance joins `/list`, already deferred. Prototype v1 = Dedicated room; Invoked deferred until activation-turn isolation is structurally shown** (`docs/programme/MAIA-SHADOW-FIELD-01_DESIGN_2026-09-06.md` + Field contract `docs/design/contracts/shadow-field.md`): Dedicated entrance inherits the Journal door with an *Enter the Shadow Field* activation gesture; Invoked entrance via three lawful offers and four forbidden counterparts, acceptance turn = activation turn; arc with prompt law and register table per movement (Encounter OBSERVED/FELT only → Stay → Differentiate in the encounter-and-projection order with Elements as five questions and possibilities offered one at a time → Reclaim as questions → Choose → Return questions then the keep act, MAIA-proposed wording only from member material); Leave on every screen, nothing on exit; PROTOTYPE handoff (separate room, assembly sovereignty import list, one registered producer `shadow-field`, keep gesture as sole writer, F1–F16 as refusals); falsifier trace F1–F16. **HARD STOP before PROTOTYPE stands.** Constitution history: v0.1 accepted subject to two corrections → v0.2 (`docs/programme/MAIA-SHADOW-FIELD-01_CONSTITUTION_v0.2_2026-09-06.md`; v0.1 retained as history): **C1 provenance immutability** (a MAIA POSSIBILITY never becomes member-authored; take-up creates a *distinct* MEMBER INTERPRETATION, the originating possibility keeps `system · inferred`; only the member's item is keep-eligible) and **C2 spoken ≠ durable** (speaking permits only visible transcript carriage through the current Field session; no storage, scoring, profiling, routing, personalization, or later-session retrieval; durable authority requires the keep act). **§9 naming RATIFIED**: member-facing Shadow Field · `shadowField` · `[MAIA/shadow-field]` · producer `shadow-field`. F12 rerun on v0.2: unchanged, 6 of 7 structural failures. v0.1 content: eight laws with structural forms; register/provenance mapping with the ceiling rule `authoredBy: system ⇒ {POSSIBILITY, ARCHETYPAL, UNKNOWN}`; activation act; offer contract; keep act + memory class (member-authored atom, never derived); exit/Sanctuary; Return without capture; etiology prohibition; harm grounding; naming (§9, founder decision). **F12 rerun: the reworded Guardian fails 6 of 7 points, all architectural** (entry provenance, offer-producer existence, encounterability, turn durability, ablation invariance, assembly sovereignty); passes only the register ceiling. Decision packets were in `…_CONSTITUTE_2026-09-06.md` (recommended: turn-scoped durability · spoken offer with no offer-producer component · practitioner material by provenance not a seventh register · astrology member-brought only, excluded from the first prototype · L8′ as Field law now, Invariant candidate after WITNESS · Field-only scope plus *a Field turn is never simultaneously an ordinary turn*); law text drafts only after rulings, then T1–T10 and the F12 run. FALSIFY record (`…_FALSIFY_2026-09-06.md`): ten cases, the adversarial rewording test (a Guardian reworded to satisfy L1–L7 as worded *passes* → the seven laws are utterance rules, not authority-direction rules → two structural laws needed: *entry is an act, not a match* and *a frame must be encounterable*), and candidate L8 attacked seven ways → survives as **L8′** (turn-scoped durability; governs system-authored representation, not model cognition; extends Inv 6/11/15 + P12). Fourteen falsifiers F1–F14 proposed as Acceptance Instrument v1; six decision points handed to CONSTITUTE (durability boundary, content-prompted offer, seventh register for practitioner material, astrology standing, L8′ text, scope toward the ordinary path). Earlier census pointer: Decisive finding: the live `/list` path already enters shadow framing uninvited on substring inference on every tier (Shadow Guardian, `maia-path-revelation.ts:258-266` → `WisdomRouter.ts:122-147` → `maiaService.ts:1464/1882/2110`, offering a `shadow-integration-space` tool that has no route) and runs a shadow elemental voice every turn into `agent_runs`; the one member-chosen shadow surface in production (Journal panel → `shadowWorkFlows.ts`) persists nothing and calls no model — *consented surface has no intelligence, intelligence has no consent*. `shadow-insight.ts` is dead (no importer chain, no input producer) but its designed intent — a hidden representation that shapes behavior, "never exposed to user" — is live in `processingProfiles` / `panconsciousFieldRouter` / `relationalObserver` (X1/X3); the question *may MAIA privately know something psychologically consequential the person cannot encounter, contest, contextualize, or revoke* is handed to FALSIFY/CONSTITUTE unanswered. Ten candidate falsifiers with in-tree specimens listed in the census §11. Prior art for DISCOVER already in tree and flagged (`lib/consciousness/shadowWorkFlows.ts` system-suggested flows; `lib/features/ShadowIntegrationTracker.ts` progress metrics; `lib/shadow-insight.ts` system-side inference) — inherit / retire / quarantine is a DISCOVER question, not a repair. Record: `docs/programme/MAIA-SHADOW-FIELD-01_LANE_DEFINITION_2026-09-06.md`. Next act: DISCOVER on founder go.
