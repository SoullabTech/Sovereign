...
- **LATEST — 2026-09-07 — `JARVIS-CIRCLES-01 · CIRCLE-04`: R1–R4 VERIFIED ON CANDIDATE (full pass 35/35); R5 census CLOSED; post-R5 cleanup P1–P5 + founder adjudication A–F IMPLEMENTED, NOT VERIFIED.** Branch `claude/jarvis-circles-programme-reouzc`; rulings `…_FOUNDER_RULINGS_2026-09-06.md` (**FR-01…FR-17**, open **CA-01…CA-16**); records `…_VERIFIER_COVERAGE_REGRESSION_2026-09-07.md`, `…_POST_R5_CLEANUP_2026-09-07.md`, `…_POST_R5_ADJUDICATION_2026-09-07.md`, `docs/architecture/CIRCLES_FIELD_PULSE_CONTAINMENT_PROVENANCE_2026-09-07.md`. **All runs founder-executed against DISPOSABLE SHADOW DATABASES from production schema; shadow+worktree deleted after; PRODUCTION NEVER MIGRATED, NEVER DEPLOYED — `circle_membership_removals` ABSENT, `withdrawn_at` ABSENT, still `4 circles · 4 memberships · 0 shares · 0 inquiries · 0 responses`.** Ladder: R1 `ca1c4736` 18/5 · R2 `8f31abc8` 28/2 · R3 `d1742472` 34/1 · **R4 full candidate pass `35/35 · 0 failed`** (the printed `36 passed` was a duplicated C14). ⭐ **CUSTODY: R1–R4 = VERIFIED ON CANDIDATE / PRODUCTION CLOSURE PENDING deploy.** **R5 census CLOSED:** B-01/B-02/B-03/B-06 RESOLVED INDIRECTLY ON CANDIDATE · B-04 DOCUMENTATION/PROVENANCE ONLY · B-05/B-08/B-09 STILL DEFECT (now repaired, see below) · B-07/B-10/CA-15 DEFER TO INVOKE · **V-01/V-02 instrument defects found**. 🔴 **The 2026-09-07 P1–P4 run `42 passed · 3 failed · 42/44 discharged` was an INSTRUMENT failure, not a product-boundary failure** — T7f provokes a UNIQUE violation, catches it in TS, and leaves the shared Group-T transaction **aborted**, so T5/T6 never execute; **FR-14 caught it** (under the old rule it would have read as a smaller green suite). **A — instrument correction:** `expectViolation(tx, savepoint, sqlstate, …)` on **both T7f and T6** — SAVEPOINT → attempt → success = FAIL → require the **exact SQLSTATE** (`23505` uniqueness, `23514` CHECK) → ROLLBACK TO → RELEASE. ⛔ *An arbitrary SQL error is not proof an invariant held.* ⭐ **FR-15 RATIFIED — WITHDRAWAL IS A TOMBSTONE:** *withdrawal preserves the historical fact that a response occurred, but not the withdrawn authored payload.* `LIVE = withdrawn_at NULL + payload NOT NULL` · `WITHDRAWN = withdrawn_at NOT NULL + payload NULL`, enforced as a schema invariant so the halves cannot drift. Unlike `shared_artifacts` (a pointer to something existing elsewhere), **an inquiry response IS the Circle-side authored representation** — no other object's integrity needs the words. The ROW survives because `UNIQUE(inquiry_id, member_id)` prevents *respond → see others → withdraw → answer again better informed*; **that constraint is load-bearing, the payload is not.** ⛔ `response_type` nulled too (authored semantics); ⛔ **no hash/digest/length or any recoverable substitute.** Migration **amended in place** (never durably applied → no window of retention). > ***Keep the fact that an act occurred when the system needs that fact for integrity; do not keep the person's surrendered meaning merely because storage makes it easy.*** ⭐ **FR-16 RATIFIED — MEMBERSHIP TERMINATION CASCADES:** *ending membership ends the eligibility of that member's live Circle-side representations to remain in the field.* `leaveCircle()` and `removeMemberWithClient()` tombstone live responses **inside their existing atomic transactions**, before membership standing changes. **The gap was unfixable by the member:** a response stayed visible after leaving and `withdrawResponse()` requires active membership — **nobody could remove it.** ⛔ **Not proxy withdrawal:** *AUTHOR WITHDRAWAL* = continuing consent exercised; *BOUNDARY CASCADE* = representation loses field eligibility because the relationship ended. Scoped to one Circle. ⭐ **FR-17 RATIFIED — STORED INQUIRY `integrating` RETIRED:** it had no distinct operational semantics — a stored duplicate of `closed AND field_synthesis IS NOT NULL` (**the FR-13 anti-pattern**), able to drift from it, with no exit. Now `InquiryStatus = 'open' | 'closed'`; `closeInquiry()` always closes; synthesis is an optional property displayed from `field_synthesis != null`. ⛔ **NOT an exit ceremony** — no durable meaning to exit from. ⛔ **HARD SCOPE BOUNDARY — `FieldPhase` is a DIFFERENT concept and is UNTOUCHED** (`FieldPhase` · `derivePhase()` · `FieldPresence` · Circles-page labels all byte-identical, asserted by **C18**); ⛔ do not equate *closed + synthesis* with Common-Ground Mediation or Circle maturation. **This REDUCES the CA-14 collision — one fewer `integrating` meaning something else.** **B-04 RESOLVED AS PROVENANCE:** the cited containment plan is unrecoverable across 4,801 commits and every ref; surviving authority = the 2026-07-17 code record + contained runtime + verifier **C4**; ⛔ no historical prose invented. ⭐ **CA-16 TRANSFERRED OUT OF THE LANE:** `PRESERVATION_AUDIT_2026-08-01.md` lists that file among 281 "stranded on the branch" documents — **it is not on that branch**; before any branch deletion, falsify the full 281-entry inventory against the actual tree. **A governance-integrity question, not Circle work** — ⛔ do not edit that audit from here; does not block this lane. **P4/B-08:** the pulse membership boundary moved into the CONTRACT — `getCirclePulse(circleId, memberId)` self-gates; `getCirclePulseLight` replaced by `getCirclePulseSummariesForMember(memberId)`, **one query where the authorization IS the join** (removes N+1 and bypass together); no RLS, route gates unchanged. **COVERAGE FLOOR NOW 54** — added C15 (pulse contract) · C16 (withdrawal visibility + **proxy withdrawal unrepresentable, no target-member parameter**) · C17 (retired status gone from type and writer) · C18 (FieldPhase untouched) · C19 (leave cascade wired) · S5 (foreign-Circle pulse refused) · T7a–T7h (withdrawal + tombstone + no-payload-regain) · T8a–T8c (leave/removal cascades, never crossing Circles) · T9a/T9b (DB rejects retired status; closed+synthesis representable). **Floor and emitted set cross-checked 54 = 54, no orphans either direction.** ⛔ **NOT VERIFIED — remote sessions have no `DATABASE_URL`/`node_modules`.** **Gate needs THREE migrations applied to the shadow in order: `20260906000003` → `20260907000001` (amended, tombstone) → `20260907000002`**; then every required obligation **PASS**, `0 failed`, **0 required WARN/SKIP/MISSING** (FR-14 as amended: only PASS discharges), fixture rollback, shadow+worktree deleted, production unchanged. **Standing: DEPLOY / INVOKE / COHORT NOT AUTHORIZED · no discovery · no facilitator assignment (CA-15) · founder UI gate untouched · doctrine byte-identical · no number ratified.** ⛔ **INVOKE readiness is a founder call, not Jarvis's to declare.**
