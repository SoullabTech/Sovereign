...
- **LATEST — 2026-09-06 — `JARVIS-CIRCLES-01 · CIRCLE-04 REPAIR`: R1 + R2 VERIFIED ON CANDIDATE; R3 IMPLEMENTED, NOT VERIFIED.** Branch `claude/jarvis-circles-programme-reouzc`; rulings `…_FOUNDER_RULINGS_2026-09-06.md` (**FR-01…FR-13**, open **CA-01…CA-15**); repair order `…_REPAIR_ORDER_2026-09-06.md`; records `…_REPAIR_R1_VERIFIED…`, `…_REPAIR_R2_2026-09-06.md`, `…_REPAIR_R2_VERIFIED_R3_2026-09-06.md`. **R1** (B-01 access containment, `ca1c4736`): `18 passed · 5 failed`, C13 PASS 16/16 routes. **R2** (FR-05 removal contract, `8f31abc8`): **`28 passed · 2 failed · 0 warned · 0 skipped`, exit 1** — C7, C8 and **T3a–T3i ALL PASS** (ordinary-member refusal · self-removal refusal · required grounds · authorized facilitator removal · actor+grounds recorded · access cut · share revocation · source preserved · other-Circle isolation). Run method: **disposable shadow database from production schema/data, R2 migration applied ONLY there, shadow deleted after**; production never migrated, never deployed, `circle_membership_removals` **ABSENT from production**; counts still `4 circles · 4 memberships · 0 shares · 0 inquiries · 0 responses`. ⭐ **CUSTODY: `R1 = R2 = VERIFIED ON CANDIDATE / PRODUCTION CLOSURE PENDING eventual deploy`.** *Verified ≠ deployed.* **Remaining failures: C6** `response_count` → R4 · **S4** FORMING/ACTIVE → R3. **CORRECTION absorbed:** the removal record is **append-only BY APPLICATION CONTRACT** — ordinary product actions, rejoining included, cannot overwrite removal history; ⛔ **not cryptographic immutability, not protection from a DB admin** — do not describe it as either. **FR-12 RATIFIED:** ⛔ do **not** widen FR-05 removal authority `facilitator`→`helper`; ⛔ do **not** auto-promote creators; `helper`·`facilitator`·`member` stay distinct — *helper is not facilitator; facilitation must be conferred by an explicit later governance act*; structured inquiries permit helper-or-facilitator but **removal is a materially stronger power**. Missing facilitator-assignment pathway = **INVOKE gap CA-15**, not a reason to corrupt R2. **Route KEPT** (founder-gated; a service with no reachable contract is the built≠wired ambiguity). **FR-13 RATIFIED — constitution state is DERIVED, NEVER STORED:** `CircleConstitutionState = 'forming' | 'active'`; `< 3 active members → FORMING`, `>= 3 → ACTIVE`; **FORMING = not presently constituted as a plural relational field**, covering **both initial formation AND re-formation** after an active Circle falls below plurality. ⛔ **No `circles.lifecycle_status` column** — it would duplicate membership truth and create an unadjudicable drift (*stored ACTIVE while count = 2*); ⛔ no `CHECK(member_count >= 3)`; ⛔ no cached member count as constitutional authority; ⛔ **no `COUNT(*) >= 3` reproduced around the codebase** — one canonical path `getCircleConstitutionState()`. Transitions are **facts, not discretionary system acts** (no timer, no administrator act, no inference). On falling below three: ⛔ do not delete · do not recruit · do not split · do not interpret the cause · **do not describe it as failure**. > ***FORMING and ACTIVE are not stages the software declares. They are constitutional facts derived from whether plurality is presently there.*** Richer lifecycle (maturation · rest · completion · birth · separation) stays separate = CA-04. **R3 landed:** `lib/circles/constitutionState.ts` (pure `deriveConstitutionState()` + `getCircleConstitutionState(circleId, client?)` + `CIRCLE_PLURALITY_THRESHOLD = 3`), **no migration, no schema change**; optional client so a caller inside a transaction sees its own uncommitted membership changes. ⚠️ **CA-14 SEMANTIC COLLISION — sharper than "same words":** `FieldPhase` = `'forming'|'active'|'integrating'|'quiet'` is an **activity heuristic** (`derivePhase()` returns `'active'` when an inquiry is open), so **two of its four values are the same strings meaning something else in the same module**, and **`CircleConstitutionState` is STRUCTURALLY ASSIGNABLE to `FieldPhase` — TypeScript cannot catch a mix-up.** Separation is a discipline, not a compiler guarantee. ⛔ FieldPhase **not** repaired or renamed (founder ruling); guard added as **C14** (neither module may depend on the other) — *Jarvis's addition, not instructed*. **Old S4 REMOVED, not adapted** (it tested for a stored lifecycle column, which under FR-13 asserts the wrong thing) → **S4a–S4e** in Group T walk ONE dedicated fixture Circle 1→2→3→2→3; ⛔ **the four historical production Circles are deliberately NOT read** — they could only manufacture a pass; the **derivation itself** is under test. **C6 untouched.** ⛔ **R3 NOT VERIFIED — remote sessions have no `DATABASE_URL`/`node_modules`.** Expected: S4 family PASS · C6 FAIL · every R1/R2 assertion still green; no target pass count manufactured. **Standing: R4 NOT STARTED · no deploy · R2/R3 migrations NOT applied to production · no facilitator assignment · founder UI gate untouched · COHORT NOT AUTHORIZED · doctrine byte-identical · no number ratified.**
