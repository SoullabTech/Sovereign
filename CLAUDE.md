...
5. **`npm run smoke`** — build + boot + reachability. Runs `scripts/smoke.sh`: `npm run build`, starts the server, waits for `/api/health`, then POSTs `/api/sovereign/app/maia` **unauthenticated and asserts 401/403** — i.e. the route is mounted *and* the auth gate is armed. An unauthenticated `200` fails the smoke. It proves the app **compiles, boots, and refuses correctly** — nothing about whether MAIA answers well; that is behavior, and belongs to the test suites.
   - ⚠️ **Not a per-commit check.** It does a full production build, so it costs minutes, not seconds. Run it before a PR or a deploy — not on every commit. For per-commit speed use `npm run typecheck` plus the jest suites covering what you touched.
   - `npm run smoke:deployed [url]` — `scripts/smoke-test.sh` against an already-running target (`/api/health`, `/api/version`, `/api/ready`). **Defaults to `https://soullab.life` — production.** Pass an explicit URL to check anything else.
   - ⚠️ Historical note: until 2026-08-04 this step read "Test with `npm run smoke` before committing", but **no `smoke` script had ever existed in `package.json`** (`git log -S '"smoke"' -- package.json` is empty). `scripts/smoke.sh` existed and was executable; the instruction was simply never wired to it. Any lane before that date citing "smoke passes" cited a command that exited `Missing script: "smoke"`. Wiring it also exposed a second staleness: the script demanded a 2xx from the sovereign endpoint, so it failed precisely when the auth gate worked. Both are fixed here.
