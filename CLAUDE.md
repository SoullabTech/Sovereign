...
- **Worktree disk accumulation**: `scripts/ain-worktree-claim.sh claim` creates a full worktree under `~/.claude/worktrees/ain-<id>`, each of which grows its own `node_modules` (1–2 GB for this repo) plus a `.next` cache. Before 2026-08-29 `release` dropped only the lock and left the tree on disk, so abandoned work units accumulated silently — 10+ trees took the Mac Studio to 96% (17 GiB free). `release` now reclaims the worktree, but **refuses** (keeps it, prints why) when it holds uncommitted changes or commits not on any remote. Sweep everything with `scripts/ain-worktree-claim.sh gc`; see what each tree costs with `list`; strip `node_modules`/`.next` from trees you must keep with `gc --caches`. Reclaim a worktree once its commit is pushed rather than leaving it around.
- **Unmanaged checkout lifecycle (the larger cause, measured 2026-09-04)**: the disk pressure on the Mac Studio was never primarily cache growth. A full census found **272 `node_modules` directories totalling ~35.7 GB**, plus ~11.5 GB of `.next` output, against 4.8 GiB free — while reclamation authority covered only `~/.claude/worktrees`. The ad-hoc checkout families are outside any lifecycle and accumulate ~2 GB per tree indefinitely: `~/MAIA-SOVEREIGN-worktrees/`, `~/maia-wt-*`, `~/wt-*`, `~/maia-witness/*`, `~/OpenMAIC`. Diagnose with `scripts/disk-deep-scan.sh` (read-only; ranks `node_modules`, build output, files >1 GB, engine storage, snapshots). **Never delete `~/.nvm/versions/*/lib/node_modules`** — globally installed tooling, not project deps; `npm i` will not restore it. Two APFS traps seen repeatedly: block reclamation lags deletion (21 GB of Docker cache and 14 GB of DeviceSupport each moved `df` by ~1 GiB at the time, with the space appearing later), so `sync; sleep 10; df` before concluding a cut failed; and `df` on `/System/Volumes/Data` reports container-wide free space, so used+avail need not equal size. Full policy (classification by custody / regenerability / ownership / access latency, plus the APFS traps): `docs/ops/STORAGE_CLASSIFICATION_POLICY.md`. Named follow-up: **Checkout Lifecycle GC** — extend the worktree reclamation model to discover approved checkout roots, classify active/dirty/protected trees, and reclaim only regenerable artifacts from inactive ones, never source or the checkout itself without separate authority.
