...
- **Writer's Studio programme**: start at `docs/programme/WRITERS_STUDIO_EXECUTION_DIRECTIVE.md` (current sequence, current SHA, the one integration lane), then `WRITERS_STUDIO_MASTER_BRIEF.md` (product constitution) and `WRITERS_STUDIO_PROGRAMME_BOARD.md` (observed state). **Read all three before touching any Writer's Studio / Author Studio / Writer Canvas / Press surface.** The acceptance walk is frozen at `docs/product/WRITERS_STUDIO_PHASE_1_WALK_SPECIFICATION.md` v1.1; P0-M passes and **both P0-D legs are witnessed** (file-backed and pasted) on candidate `83efa86df`. The founder released the hold and deployed `cd8a95271` on 2026-08-26 — the visible Studio (MAIA in the room, Materials, Structure, Versions, find/replace, Developmental Review, Reader) is **live**. **WS-01 formal acceptance is still outstanding** — Evidence 003 records PASS but does not accept; that is the founder's act. Deploy release is **per-candidate**: anything committed after `cd8a95271` is undeployed. Do not plan from this line alone: the directive is the current state.
...
**No invite unless the Co-Lab boundary verifier passes 31/31 in production.**

Run inside the container on minisforum:
```bash
# Preferred — the gate wrapper. Enforces the real invariant: passed >= 31 AND
# 0 failed AND 0 warned AND exit 0, so a verifier that silently DROPS checks
# blocks too. Bare `docker exec` only prints numbers; it enforces nothing.
ssh soullab@minisforum 'cd ~/MAIA-SOVEREIGN && scripts/pre-deploy-gate.sh colab'

# The verifier alone, if you want to read the matrix:
...
⚠️ The script was renamed to `verify-constitution-colab.ts`; the old name
`verify-colab-boundaries.ts` **does not exist** and fails with ERR_MODULE_NOT_FOUND.
`pre-deploy-gate.sh` always used the correct name, so the deploy-path gate has
been real throughout — only this documented manual command was stale (found
2026-08-27, WS2-01 candidate gate).
