...
- **LATEST — 2026-09-07 — `JARVIS-CIRCLES-01` · `CIRCLE-05 · INVOKE`: ⭐ **I0 CLOSED · I0.5 VERIFIED ON CANDIDATE · VERIFY FULL PASS 63/63 · I1 DESIGN OPEN.** Canonical witness of record: SHA **`1725a0857624d3d23040dfbb51d45c06d834b6f5`** — `63 passed · 0 failed · 0 warned · 0 skipped · 63/63 required obligations discharged by PASS · exit 0`; three pending migrations applied in order to a **disposable shadow**; verifier fixture residue `0|0|0|0`; shadow destroyed. **The committed SHA passed, not a disposable edit** — `1725a0857` is the docs correction atop the C21 instrument repair, so the head carries the real scanner fix. *(An earlier `59/4` attempt was a shadow-construction error — schema restored without the two disjoint service principals Group S requires — and **FR-14 correctly refused the resulting skips**: WARN/SKIP never discharge. The instrument caught a bad harness, which is what it is for.)* **⚠️ CUSTODY CORRECTED 2026-09-08 (founder): production is `4a5bf8ff9` (MAIL-03), NOT `cca4568bd`. Canonical tip is `e535e6246` — **3 commits AHEAD of production** (PDF-CLEAN, unrelated to Circles). ⭐ **`I0.5 RELEASE PREPARATION` AUTHORIZED against current canonical, no scope expansion**: (1) reconcile ✅ **DONE** — canonical `e535e6246` merged into the lane, **no conflicts**; commutation assessed by inspection: **ZERO migrations landed on canonical since merge-base `39daacae5`** (the three additive Circle migrations have nothing to commute with) · **ZERO canonical changes touch any circle file** · exactly **one overlapping file**, `config/accessMatrix.ts`, where canonical added an `/api/invites/` rule and the lane added the `/commons/join` DECLARED-vs-ENFORCED note — different regions, both pure additions. Merged rather than rebased: the branch is pushed and its history is the lane record. I0.5 artifacts verified intact post-merge (guarded upsert · 409 refusal · closed-join layout · **floor still 63/63 unique** · all three migrations present). ⛔ **(2) RERUN the 63-obligation witness on THIS candidate · (3) merge as its own PR · (4) deploy and witness separately — ALL FOUNDER ACTS; this session has no `DATABASE_URL`/`node_modules`.** ⛔ **No redesign; no FR-19…FR-31 implementation folded in.** *Evidence collected against an older base does not automatically prove the new artifact.* **Historical (superseded) witness: deployed SHA `cca4568bd…` · 4 circles · 4 active memberships · 0 shares/inquiries/responses · removal table ABSENT · `withdrawn_at` ABSENT · `integrating` still allowed. Production has moved for unrelated work since the `e4ac1bcac` witness, but the Circle work has NOT moved with it — `1725a0857` NOT in production ancestry, `26508109b` NOT in production ancestry.** ⛔ UNMIGRATED · UNDEPLOYED · DEPLOY NOT AUTHORIZED · COHORT NOT AUTHORIZED. **⭐ I1 DESIGN OPEN — design record `docs/programme/JARVIS-CIRCLES-01_I1_INTEREST_COMMONS_DESIGN_2026-09-07.md`; ⛔ I1 IMPLEMENT NOT AUTHORIZED (no migration · no schema · no API · no UI · no discovery).** *The roadmap changes character here: no longer repairing what a Circle must never do, but designing the first positive human act — **"this matters to me"** — and Soullab knows it because the person said it, not because MAIA decided it for them.* **I1 design shape:** four objects (`interest_commons` · `interest_declarations` · `interest_commons_circles` · `interest_term_proposals`), **nothing system-derived anywhere** — the structural inverse of `living_field_affinities` (no `affinity_score`, no `evidence_reason`, no `created_by='system'`, no embedding). ⭐ **Contextuality is a RETRIEVAL CONTRACT, not a column** — a row per `(member, commons)` read as `WHERE member_id = $1` *is* a global profile, so the scoping must live in the signature (`listDeclarationsForCommons` · `listMyDeclarations` self-only; no cross-Commons enumeration for anyone else) — the P4/B-08 *authorization-is-the-join* pattern. ⭐ **Matchability ≠ visibility** — default posture *matchable, not human-visible*; the real tension is named, not resolved: total invisibility satisfies privacy and **defeats the north-star verb** (nobody finds anybody), so **D-J2 asks what minimum disclosure lets two people meet without publishing a profile.** ⭐ **Canonical lookup must be LITERAL** — substring/prefix only; ⛔ no semantic search, no "did you mean", no nearest neighbour: *a semantic search over a taxonomy IS a classifier, and putting it in the lookup box does not change what it does.* ⭐ **Ranking is a status economy in disguise** — order by activity/count/recency and `contribution_points` is rebuilt in the sort key; discovery is **filters, never scorers** (`declared interest → eligible Commons → safe outer facts name/description/created_at`). ⭐ **A Commons must not become a side door around FR-18** — entry to an affiliated Circle still runs that Circle's full contract. **Withdrawal**: expressive text DELETED (FR-15 — the surrendered meaning); whether the *fact* is retained is **D-J6, returned unanswered** (an interest declaration is an act against nobody, so no independent review needs it — analysis, not a ruling). **Thirteen future obligations proposed (C23–C26, T11–T18), IDs RESERVED NOT REGISTERED** — registering a MISSING obligation would fail FR-14 by construction. **Seven docket items D-J1…D-J7** (canonical creation authority · declaration visibility · expressive-text visibility · Circle affiliation authority · synonym/rename/**merge** — *a merge re-points a declaration to a term the member did not choose* · withdrawal retention · CA-11 reuse) — **none answered to shorten the list.** **FR-06 remains the hard I1 boundary: declared interests only, contextual to the relevant Commons; MAIA conversation, memory, inferred themes, psychological interpretation, private memberships and behavioral surveillance are excluded.** ⚠️ **This bars ambient appropriation, not a person's sovereign act** — a member may declare an interest they first noticed while talking to MAIA; what is barred is MAIA declaring it for them.
- **LATEST — 2026-09-08 — `JARVIS-CIRCLES-01` · ⭐ **D-K3 / CA-15 OPEN FOR CONSTITUTIONAL ADJUDICATION** (⛔ inquiry only, NOT implementation) — record `docs/programme/JARVIS-CIRCLES-01_D-K3_PROCEDURAL_AUTHORITY_INQUIRY_2026-09-08.md`.** The question, framed precisely: ***how does a Circle confer, limit, transfer and withdraw procedural authority without converting facilitation into ownership or rank?*** — deliberately NOT *"who is the facilitator?"*, which presumes a standing person. **⛔ HARD BOUNDARY SET AT THE OUTSET: `created_by`, founding membership, seniority and invitation authority do NOT automatically confer facilitation authority** — otherwise FR-31 solves collective formation and then **quietly recreates a founder hierarchy through procedure**. Likely shape: a **constituted authority relation, not a permanent privileged member** (`Circle → collective act → authority conferred → person/people/rotation/bounded mandate → revocable and replaceable by the Circle`). Must serve six sites: inquiry · Tend · Repair · FR-31 activation · Circle-sponsored Salons · collective Keep. **Jarvis analysis offered as input, deciding nothing: (1) ⭐ is it ONE authority or SEVERAL?** — the six are different kinds of power, and **a single role holding all six is functionally a rank even if revocable** (concentration is what FR-08.7 objects to; revocability limits duration, not shape); decomposed per act it stays a **relation** (*"Maria is holding this inquiry"*), unified it becomes a **title**. **(2) the four verbs are not symmetric** — withdrawal ≠ resignation (the holder's own act needs no Circle decision); ⭐ **if the holder can TRANSFER the mandate, the holder has a property right in it — ownership under another name — so transfer must be RE-CONFERRAL by the Circle, never handoff.** **(3) "bounded mandate" hides three designs** — act-bounded (authority never idles; most sovereignty-preserving, highest friction) · scope-bounded · time-bounded (closest to an office, most likely to drift into rank). **(4) ⚠️ Repair may require an authority the standing holder cannot supply** — if the rupture involves them they cannot hold their own repair, so the repair-holder must be **selectable per instance**: a concrete argument for act-scoped conferral reached from the Repair site, not from principle. **(5) a Circle with NO conferred authority must read as RESTING, not deficient** — ⛔ the system must not prompt a Circle to appoint someone, which would be coercing governance (the same error FR-11 avoided by ruling FORMING legitimate). **(6) ⭐ D-K3 and FR-31 want the SAME PRIMITIVE** — conferral is a collective act, formation is a collective act, and **neither exists in the substrate** (D-M3); both need a **multi-party ratified act** (proposal · ratifiers · threshold · outcome). ***Build it once*** — bespoke mechanisms would diverge exactly as `FieldPhase` and `CircleConstitutionState` did (CA-14). **Docket K3-a…K3-h, none answered.** **RELEASE BOUNDARY RATIFIED: RELEASE A** = interest · Commons · Salon discovery · Salon attendance (Journey A) · **RELEASE B** = collective Circle formation · constituted facilitation · continuing relationship (Journey B) — *a person should be able to encounter the commons before Soullab has solved every constitutional question involved in forming durable collective relationships; Journey B creates a persistent social body, not merely access to an encounter.* **⛔ NOT opened: D-M4** (real, but blocks nothing until withdrawal/destruction paths are implemented) · **D-J5** (rename policy has far less leverage than D-K3). **Sequence:** `NOW D-K3 adjudication` ∥ `I0.5 reconcile → reverify → merge → deploy` → `then D-J1 · D-L1 · D-L3 · D-L6 → Journey A` → `later FR-31 + facilitation → Journey B`.
- **LATEST — 2026-09-07 (evening, later) — `JARVIS-CIRCLES-01` · ⭐ **TEN FOUNDER RULINGS FR-19…FR-28 · v0.2 OF BOTH DESIGN RECORDS · TWO MEMBER JOURNEYS.** ⛔ DESIGN ONLY.** Rulings in `…_FOUNDER_RULINGS_2026-09-06.md` **ADDENDUM II**; `CIRCLE_INTERIOR_FUNCTION_MODEL_v0.2.md` · `SALON_DESIGN_THREAD_v0.2.md` (v0.1 files RETAINED, marked superseded) · `SALON_TO_CIRCLE_JOURNEYS_v0.1.md`. **The ecology:** `INTEREST → INTEREST COMMONS → { SALON encounter · CIRCLE relationship · CO-LAB shared work }`, movement **only by explicit acts**. **⭐ FR-19 — *an interest declaration grants routing eligibility; it does not publish an identity claim about the member.*** Nobody sees *Kelly/Maria/David are interested in Jung*; the declaration shows **Kelly** the Commons, and **participation then creates human visibility**. Expressive text private, **no discovery authority**. **CLOSES D-J2 + D-J3.** **⭐ FR-20 — *a roster is a consequence of co-presence, not a directory; co-presence ends and the inferred relationship ends with it.*** Before: no attendee directory (host sees registrations to tend the gathering, **creating no reusable contact list**) · during: those present see each other, ephemeral *who is here right now* · after: ⛔ **no directory remains** — no "people you met", no co-attendance graph. **CLOSES D-L2.** ⚠️ *Honest limit recorded: the system builds no list and offers no export; it cannot stop a person writing names down.* **FR-21** Connect is its own act — *"I am open to continuing relationship beyond this gathering"*, not friendship; working direction `request → accept → a Dyadic threshold becomes available` (seam not ratified). **⭐⭐ FR-22 — *a native Circle offering is not a representation crossing in; it is authored in the Circle, so the Circle copy IS the source.*** Two species with **different revocation semantics**. Author withdrawal: payload leaves; if responses exist, preserve **only the minimal structural fact** (`[Offering withdrawn]` + surviving responses) — ⛔ no text, summary, embedding, reconstruction, **no hidden preserved payload**; author identity removed from the tombstone. ⭐ ***The author can surrender their meaning. They cannot retroactively surrender Maria's meaning for her.*** Leaving and removal do the same — **boundary cascade, not facilitator proxy-authorship**: *ending Circle membership ends eligibility of the departing member's live Circle-native authored material to remain in the field; dependent material authored by others survives independently.* **CLOSES D-K2.** ⚠️ *Honest limit: in a Circle of six an unattributed tombstone above attributed responses leaves the author re-identifiable — a stance, NOT anonymity; never describe it as anonymity.* **FR-23** a response **cannot be the parent of another response**; ⛔ no likes/reactions/"helpful"/counts/engagement sort — *a response is speech, not a metric-bearing object.* **⭐ FR-24 — Request Attention is a PROCESS REQUEST, not an accusation object**: `circle_id · requested_by · requested_at · state`, and ⛔ **`target_member_id` DOES NOT EXIST**; optional note to the tender is **ephemeral intake**, not Circle memory, not a member record; ⛔ the software never declares `Repaired ✅`. `REPAIR ≠ BOUNDARY/SAFETY ≠ REMOVAL`. **CLOSES D-K5.** **FR-25** Salon default afterlife **nothing** ⛔ (no transcript · recording · MAIA memory · summary · "key insights" · attendee/relationship graph · sentiment profile). **FR-26** ***a gathering is allowed to complete*** — *"Nothing — this gathering was enough"* is first-class; ⛔ absent: mailing list · follow host · add all attendees · subscribe. **FR-27** a Circle-hosted Salon needs **explicit Circle-side authorization** (the Circle is making an outward representation of itself); mechanism open, ⛔ **`created_by` must not become that authority by accident** (D-L4 constrained). **⭐ FR-28 — *discovery surfaces places of encounter, never people*** — ⛔ **no people-discovery surface in the first release**; *people discover places of encounter, then discover people by encountering them.* **FROZEN relational grammar v1:** *Interest helps you find a place · Salon helps you encounter people · Circle lets relationship deepen · Dyadic Field lets two people continue deliberately · Co-Lab lets people work together · Constellation lets Circles relate without becoming one field.* **THE TWO JOURNEY WALKS FOUND THREE OPEN QUESTIONS — their job, and they did it: D-M1** repair-intake **destruction point** (a name must be in the intake for the tender to act; *ephemeral* cannot mean "when the process closes" if closing is indefinite, or the intake is the dossier in another table) · **⭐ D-M2 does the attendance record survive the gathering?** FR-25's *nothing retained* and FR-20's *host may see registrations* **diverge after the fact** — destroyed = literally nothing, host loses their own record; persisted-unsurfaced = **a co-attendance graph exists, one query from surfaced** · **⭐⭐ D-M3 there is no collective formation act** — `circles` has one `created_by` and each member arrives by their own membership row, so *"the four of us are forming a Circle"* decomposes into **one creation and N joins**, making four people who decided together into one founder and three joiners **at the moment their equality mattered most** (and `created_by` is provenance not ownership, so the asymmetry is real in the interface while meaningless in authority); compounded by CA-15 — a Circle born from a Salon has **nobody who can hold procedural authority**. ⭐ **ALL THREE CLOSED SAME DAY — FR-29 · FR-30 · FR-31 (rulings ADDENDUM III).** **FR-29** an identity-bearing **repair intake is purpose-bound and ephemeral** — destroyed at **terminal disposition** (`resolved | declined | withdrawn | lapsed`), never at an indefinitely defined "process close"; ⛔ no repair narrative or participant identity may migrate into a **durable member dossier**; an unresolved intake needs a **hard maximum lifetime** (duration = operational parameter). Only **non-reconstructive** evidence survives (aggregate counts). ***The intake never becomes institutional memory.*** **⭐ FR-30 — identified co-attendance is DESTROYED, NOT HIDDEN. ⛔ "Persisted but unsurfaced" REJECTED**: retaining `A → Salon 17 · B → Salon 17 · C → Salon 17` **IS constructing the co-attendance graph**, whatever the interface shows — FR-20 would otherwise be cosmetic. Identified attendance exists **only for the operational life of the gathering**; after a defined reconciliation window the **person↔Salon relation is destroyed**. The host's sight of registrations is **temporary operational visibility, not a permanent participant ledger**. Retainable: *Salon 17 occurred · attendance = 23*; ⛔ never identity edges from which the 23 can be recovered; ⛔ a legally required accounting record **must not become a social-history exception by stealth**. This gives FR-25 a technical definition: **nothing relationally identifying remains from which co-attendance can be reconstructed.** ⚠️ **Consequence stated plainly so nobody restores it later: the member's OWN attendance history is destroyed too** — a *"your gatherings"* list is still a person↔Salon edge. ***A Salon is remembered by the people who were there.*** **⭐⭐ FR-31 — CREATION IS NOT FORMATION; a Circle is constituted collectively.** ⛔ Four people who decided together **may not begin as** *Alice creates · Bob/Carol/David join* — **the data model must not rewrite equality into founder and followers**. `PROPOSED CIRCLE initiated_by = Alice (provenance only)` → each ratifies → **FORMATION EVENT `constituted_by = {A,B,C,D}`** → CIRCLE FORMED. `created_by` grants **zero additional authority** (D-I6 preserved). ⛔ **CA-15 must NOT be repaired by silently making the initiator facilitator** — that recreates founder power through another field: **a newly formed Circle cannot become procedurally ACTIVE until its facilitation mode is itself constituted** (`WHO FORMED US?` and `HOW DO WE HOLD PROCESS?` are separate questions; modes — named · shared · rotating — are later design and **none may derive from `created_by`**). Until then **`FORMING` valid · `ACTIVE` not yet valid** — *better than pretending the Circle has procedural capacity it does not possess.* **⚠️ THREE FORWARD IMPLICATIONS RECORDED (implications, not rulings): (1) FR-11's derivation gains a SECOND TERM** — `deriveConstitutionState(count)` returns ACTIVE at 3+, but under FR-31 ACTIVE **also** requires constituted facilitation, so **S4c becomes *three AND facilitation constituted***; ⛔ **not changed now** (the current assertions are correct for the current substrate) — recorded so **the verified 63/63 is not later mistaken for coverage of a rule that had not yet been made**. **(2) Three retention clocks now exist** — repair-intake TTL · Salon reconciliation window · lapse of an unratified proposed Circle — same class, **purpose-bound retention**; unset durations are architecturally fine but ⛔ **must not become "forever" by absence**. **(3) FR-30 and FR-31 are NOT in tension** — a Salon destroys its group edge, a Circle **preserves `constituted_by`**, because the four chose an ongoing relationship and that set *is* the Circle's constitution: *co-presence ends and its inference ends with it; constitution is authored and stays.* ⚠️ No retroactive problem — production holds 4 Circles / 4 memberships (one each, all FORMING), so nothing existing was constituted under the superseded model. > ***The journey walks moved the Circle from a collection of equal members to an equality-preserving act of constitution. That distinction is structural, not poetic.*** Obligations **C33–C36 · T30–T33 RESERVED NOT REGISTERED** — these rulings say what the system must **mean**; ⛔ they authorize **no schema and no implementation**. Still open: D-J1 · D-J5 · D-J6 · D-J7 · D-K1 · D-K3 · D-K4 · D-K6 · D-K7 · D-L1 · D-L3 · D-L5 · D-L6 · D-L7. **CLOSED: D-M1 (FR-29) · D-M2 (FR-30) · D-M3 (FR-31).** **CARRIED FORWARD, ⛔ NOT ADJUDICATED — D-M4** *"destroyed" must eventually include **derived copies*** (index/search tables · caches · analytics + event payloads · exports · materialized views · operational logs · over-retained backups) — backups need **no** surgical deletion yet, but the doctrine owes a precise line between **live reconstructability** (an application-readable social graph → must not survive) and an **expiring disaster-recovery copy** (different question), *otherwise "destroyed" becomes true in one table and false in the system*; ⭐ **scope observation: the constitution now says destroyed/deleted in FOUR places — FR-15 · FR-22 · FR-29 · FR-30 — and they should share ONE authored definition rather than four implementations each deciding how far deletion reaches** (not opened as a lane) · **D-M5 `constituted_by` is HISTORICAL CONSTITUTION, not current membership** — ⛔ a founding member leaving must not silently rewrite the Circle as constituted by three, and ⛔ joining later must not make someone **retroactively a constitutor**; points toward a future dissolution/reconstitution question, **not a reason to rule now**. **The ACTIVE formula, written once:** `ACTIVE = active_members >= 3 AND facilitation_constituted` — ⛔ **not a new test pasted onto an old claim**; the **63/63 remains valid evidence for the substrate it tested and must never be retroactively described as proving FR-31.** > ***The architecture now distinguishes cleanly among co-presence · membership · constitution — a Salon can vanish relationally after it occurs, while a Circle can legitimately retain the authored fact of who brought that continuing relationship into being.***
- **LATEST — 2026-09-07 (evening) — `JARVIS-CIRCLES-01` · TWO DESIGN RECORDS, ⛔ DESIGN ONLY, NO IMPLEMENTATION AUTHORIZED.**
  **(1) `docs/programme/CIRCLE_INTERIOR_FUNCTION_MODEL_v0.1.md` — the Circle as a lived room.** Seven human acts: **Arrive · Witness · Offer · Respond · Inquire · Remember · Tend.** Grounded in a verified read of the current surfaces: `Mode = 'alive'|'offerings'|'journey'` ("What's alive" / "Offerings" / "The journey") · `derivePhase(hasActiveInquiry, hasSignals, hasActivity)` → `active` if an inquiry is open, `integrating` on any signals, `forming` on **any activity at all**, else `quiet` — ⭐ **it has no notion of people** · `SharedFeed` reads **imported artifacts only — there is NO Circle-native authoring path anywhere** · `CircleSettings` = creator-only invite + leave · `FieldMemory` = closed inquiries + syntheses. **Three changes proposed:** `What's alive` → **`Here`** (it currently means *an inquiry is open*; ordinary relational life belongs there) · Offerings gains a **Circle-native** path (`Offer to the Circle`, never "Post") · the pulse becomes **secondary to human-authored orientation** and "last movement" leaves the surface — ⭐ *recency does not get to tell humans what their field is; a field can be profoundly alive while nobody has typed for a week.* ⛔ `FieldPhase` untouched (CA-14 stands, C18 asserts it). **⭐⭐ D-K2 — the question this model creates and the existing rules cannot answer: a Circle-native offering has NO source elsewhere.** Every crossing rule assumes an original survives (revocation removes the representation, T1/T2; leaving keeps originals private, as the live leave copy promises). **For native speech there is no original** — so withdrawal, leaving and removal are undefined for it. ⛔ **Native offering must not be built before this is ruled.** **⭐ D-K5 — a repair request must not itself become a record against a person**, or it is an accusation log with a gentle name; `rupture/repair ≠ boundary/safety ≠ removal` becomes product function. Response depth **capped at one level** (`Offering ↳ responses`) — *nesting is where a room becomes a platform*; ⛔ no reactions/likes/counts/"top response" (each is `contribution_points` wearing an emoji); ⛔ ordinary responses are **never** gated by contribute-before-see (inquiry-only, D-06 as corrected). Witnessing writes **nothing observable**. Later: **Connect** (Circle enables an encounter it does not own — a Dyadic Field, FR-03) · **Gather** (⛔ no automatic transcript, no MAIA extraction). Obligations **C27–C30 · T19–T24 RESERVED NOT REGISTERED**; docket **D-K1…D-K7**.
  **(2) `docs/programme/SALON_DESIGN_THREAD_v0.1.md` — opened by founder act BEFORE Circle discovery is finished.** A Salon is **not another field type** — a **temporary gathering form**, member-initiated, **no membership**, **may simply end**, leads to a Circle only by an explicit new act. `interest → Interest Commons → Salon → human encounter → "we want to continue this" → explicit Circle formation`. *A Circle says we are in relationship · a Salon says come be in conversation for a while · a Commons says this is something many of us care about · a Co-Lab says let's make something together.* ⭐ **Salons are the missing middle** between *"I am interested in this"* and *"I want an ongoing relationship with these people"* — far more natural than an algorithm saying *"here are six people you should connect with."* ⭐⭐ **Effect on D-J2, stated precisely: THE GATHERING CREATES VISIBILITY THROUGH PARTICIPATION** — a declaration stays private and still does real work; no `Kelly · Maria · David are interested in X` roster is ever rendered. **But D-J2 is DISPLACED, NOT ANSWERED**: disclosure moves from a **standing property of a person** to a **member act at a moment of their choosing** — and **attending is itself a disclosure**. The question narrows to **D-L2: what does attendance disclose, to whom, and for how long?** ⛔ **The failure mode named in advance: a browsable attendee list is a people-search database with extra steps.** The rule — ⭐ **a roster is a consequence of co-presence, not a directory.** **Membrane: every crossing independent** (attend · connect · enter a dyad · form a Circle · join a Circle · offer material onward) — ⭐ ***connection may increase reach, but may never lower consent***; this is **FR-08.5 one layer out**. **A Circle may host a Salon** — the Salon is *a membrane crossing authored by the Circle*, not access to its interior; **guests do not become members**. **Default afterlife of a Salon: NOTHING** — ⛔ no automatic transcript, no MAIA extraction; *a gathering is where the no-stealth-memory vow bites hardest.* ⭐ **The dangerous convenience refused in advance: a "people you've gathered with" surface** — trivially derivable from attendance and exactly the profile this avoids; **Connect stays explicit, never derived from co-attendance.** ⛔ Structurally absent: engagement optimization · followers · host rankings/ratings · "top Salon" · attendance counts · trending — *the host is the person tending this particular gathering, not a creator, channel or brand.* Obligations **C31–C32 · T25–T29 RESERVED NOT REGISTERED**; docket **D-L1…D-L7**. ⛔ **I1 may not need to resolve human-visible declarations at all if Salons carry the meeting — D-J2 stays open, materially narrowed.**
- **PRIOR (2026-09-07) — `I0.5 · ENTRY SAFETY CLOSURE` — the repair detail, superseded as to standing by the canonical pass above.** Branch `claude/jarvis-circles-programme-reouzc`; record `docs/programme/JARVIS-CIRCLES-01_I0.5_ENTRY_SAFETY_2026-09-07.md`; rulings addendum in `…_FOUNDER_RULINGS_2026-09-06.md`. Opened by founder act on the two I0 findings that are **defects against already-ratified law**, not design questions. **⭐ FR-18 RATIFIED — *a generic Circle invitation may not reinstate a removed member.* Canonical shorthand (founder precision): _"a recorded removal standing outranks a generic invitation"_ — deliberately NOT the broader "standing outranks invitation", which could be read as deciding CA-08.** ⚠️ **This ruling corrected Jarvis**: the I0 docket recommended revoking the invite token on removal; refused, because the token is **Circle-wide** — revoking it withdraws the invitation from *everyone* to answer *one* person's standing. The defect was never that the token survived, it was that **a generic bearer credential had enough authority to overwrite a recorded relational act**. Repair: `joinWithInviteWithClient()` seam; ⭐ **the authority is the MUTATION, not a precheck** — the guard lives inside the upsert (`ON CONFLICT … DO UPDATE … WHERE circle_memberships.status <> 'removed' RETURNING`; zero rows → `REINSTATEMENT_REQUIRED`) → **409** carrying **no Circle name, no grounds, no token verdict** (*a refusal is not an occasion to disclose*). ⚠️ **Founder source review REJECTED the first candidate `093379e8d` before any verifier run**: it read the standing, refused if `removed`, then upserted `active` unconditionally — `transaction()` is an ordinary BEGIN with no row lock, so a removal committing between the read and the write let the invitation overwrite it. **FR-18 was prechecked but not authoritative at the mutation boundary, and T10a–T10e could not tell the two apart** (all five evaluate a standing already `removed` before the join began). Preliminary SELECT deleted — one authority, no second thing to mistake for it. *No migration; no schema change; `left` behaviour identical; CA-08 still open.* **I-02 — STATE CLOSURE ON PAGE**: new `app/commons/join/layout.tsx` renders a closed state, so an unauthorized visitor is never asked for a token, never asked for a **consent mode** (*asking for a sharing decision about a relationship they cannot enter collects a sovereign act the system has no standing to receive*), and never submits a doomed join; `config/accessMatrix.ts` records DECLARED-vs-ENFORCED. ⛔ **I-03 preserved deliberately** — the closed surface reads no invite, so a valid and an invalid token stay **indistinguishable** before authorization. ⛔ **I8 obligation (not an I0.5 defect):** the page checks `requireFounder()` and the API `requireCircleAccess()` — they agree **today** only because Circle API authority is itself founder-only; when `CIRCLE_ACCESS_MEMBER_IDS` is constituted at I8, `/commons/join` **must migrate to the same Circle-access authority**. ⛔ Do not change it now. ⛔ **The API gate is unchanged** — a layout does not run for a route handler; `requireCircleAccess()` is still the authorization (C22 guards that confusion, which is the shape of B-01). **Verifier floor 54 → 63** under FR-14 (*the number is descriptive; the named set is the law*): **C20** surface refuses before rendering · **C21** solicits nothing AND validates nothing · **C22** join door gated + can refuse reinstatement · **T10a** valid live invite does not reinstate · **T10b** standing stays `removed` (**no write occurred**, not merely a thrown error) · **T10c** FR-05 removal record intact · **⭐ T10d** the **same live invite still admits an independently eligible member** — the obligation that makes the founder's correction falsifiable: a future "fix" by token revocation passes T10a–T10c and **FAILS T10d** · **T10e** no other Circle affected · **⭐ T10f THE INTERLEAVING WITNESS** — a **real `removeMemberWithClient()` act landing between invitation evaluation and the membership mutation** still loses: refusal, standing stays `removed`, removal record intact. Built deterministically via an `InviteClient` wrapper on the same rolled-back transaction (a second connection would need committed fixtures and would break the verifier's rollback-only consequence contract). **A precheck candidate passes T10a–T10e and FAILS T10f.** **Not authorized, founder: *do not add yet*** — per-invitee tokens · invite expiry · reinstatement workflow · facilitator review UI. ⛔ **CA-08 not decided** (`left` ≠ `removed`). **D-I dispositions ratified: D-I1 HYBRID** (only **explicit selection** drives matching; free text is **expressive only, never inferred into taxonomy** — the operative form of FR-06) · **D-I2 BUILD FRESH**, implementation term **"Interest Commons"** · **D-I3** cohort mechanism at I8, identities only at cohort authorization, ⛔ **do not touch `FOUNDER_MEMBER_IDS`** · **D-I5** member count **and** derived FORMING/ACTIVE are **INTERIOR** · **D-I6** `created_by` is **provenance, not permanent ownership**. Outer membrane narrows to `name`/`description`/`created_at`; authority axes carried to I4/I6 (CA-15 open). ⭐ **FOUNDER-RUN VERIFICATION OF `ae0fadd54` (disposable shadow, three pending migrations applied to the shadow only): `62 passed · 1 failed · 0 warned · 0 skipped · coverage 62/63 · exit 1` — EVERY BEHAVIORAL OBLIGATION PASSED, `T10a–T10f` included.** ⭐ **T10f PASSING IS THE SUBSTANTIVE RESULT: a removal landing after invitation evaluation and before membership mutation still defeats the generic invitation — the exact race `093379e8d` could not survive.** The single failure was **C21, a verifier false positive**: it scanned the RAW layout source, which contains the sentence *"nothing here reads `circle_invites`"* — **prose documenting the absence, matched as evidence of the presence**; the page inspects no token. Founder proved it on a disposable copy changing **only C21** to strip comments → `63 passed · 0 failed · exit 0`, residue zero. **Repair `3ed98ceab` is INSTRUMENT ONLY** — C21 now strips block and line comments before scanning, the same discipline **C6** has used since R4 and for the same reason; token set NOT weakened; ⛔ layout, `joinWithInviteWithClient()`, FR-18, T10a–T10f, the floor and all Circle behavior UNCHANGED. *A prose ban must never read as the banned behavior returning — an instrument that scans prose can fail on a file precisely because that file documents its own compliance.* **Typecheck at `ae0fadd54`: `230 errors · baseline 239 · 9 fixed · 0 regressions · exit 0`.** **Production witness: SHA `e4ac1bcac` · 4 circles · 4 active memberships · 0 shares/inquiries/responses · removal table ABSENT · `withdrawn_at` ABSENT · `integrating` still allowed — UNCHANGED, UNMIGRATED, UNDEPLOYED.** ⛔ **CANONICAL VERIFY NOT YET ACCEPTED** — the 63/63 evidence is from a **disposable probe, not a run of a committed SHA; a probe is not the record.** Acceptance requires the full verifier from the committed SHA against a disposable shadow (`20260906000003` → `20260907000001` → `20260907000002`, shadow only): `63/63 · 0 failed · 0 warned · 0 skipped · exit 0`, residue zero. ⛔ **I1 DESIGN NOT OPENED** — it opens on that canonical run; *Jarvis does not open a stage on a run it has not seen.* ⛔ No deploy · no migration authored · schema untouched · cohort not authorized · no invitation sent · no facilitator assigned · no discovery built · founder UI gate untouched.
