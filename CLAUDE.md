...
- **LATEST — 2026-09-06 — `JARVIS-CIRCLES-01`: CONSTITUTIONAL MINIMUM RATIFIED (founder act) → `CIRCLE-03 · VERIFY` OPEN; verifier IMPLEMENTED but NOT RUN.** Branch `claude/jarvis-circles-programme-reouzc`; charter `docs/programme/JARVIS-CIRCLES-01_CHARTER_2026-09-06.md`; rulings `…_FOUNDER_RULINGS_2026-09-06.md`. **Identifier correction (Jarvis errors, recorded not erased):** the v0.1 draft reused `A-01`/`A-11` for two different questions each, and docket v2 marked **A-12 a VERIFY blocker while the verifier spec excluded it** — A-12 was never a blocker. Canonical scheme now **`FR-nn` ratified · `CA-nn` open**; historical labels valid only inside their own documents. **RATIFIED: FR-01** consent law (*connection may increase reach, but may never lower the consent level of what is being connected*; crossing is explicit·representational·destination-specific·revocable·opaque-by-default, source untouched) · **FR-02** six-field ontology (Personal · Dyadic · Circle · Constellation *(unit = Circles)* · Commons · Co-Lab), defined by unit/purpose/authority/membrane, **no numerical maximum** · **FR-03** Dyadic Field is first-class and **distinct**; **Circle begins at three — plurality, not merely connection** · **FR-04** `Feel → Contribute → Browse` governs **structured inquiry only**; *a member may witness without being compelled to contribute; quiet presence is legitimate*; doctrine wording returns for explicit amendment, never silent rewrite · **FR-05** facilitator authority **procedural, not interpretive** (may not decide meaning, declare interpretation, define unheld consensus, or remove for disagreement/difference/discomfort); **removal = boundary/safety act only**, member-cannot-remove-member, grounds recorded, cuts access, **cascades revocation exactly as leaving**, never alters source, removed member gets review by someone other than the remover · **FR-06** discovery = **declared interests only**, contextual to a Commons, never global-by-default; never conversation/atoms/semantic memory/anchors/inferred themes/psychological interpretation/private membership/behavioral surveillance · **FR-07** **no facilitator, majority, system or larger field may re-release a member's authored material**; joint material crosses only via a new representation with every represented contributor retaining consent; **until a collective-release mechanism exists, collective Circle material does not cross** · **FR-08 never-crosses WITH CORRECTION** · **FR-10** *Circles scale by multiplication, not enlargement*, no threshold attached. **FR-09 (Field Witness default / Common-Ground Mediator invoked; any member may request; no silent switch; Circle-level assent act eventually required) = DIRECTION RECORDED, NOT a VERIFY blocker, NOT built.** ⭐ **FR-08 correction — my draft was overbroad:** *"private MAIA conversation content … by any path, including summary"* would have destroyed the Personal→Circle membrane. **Privacy forbids ambient appropriation, not a person's sovereign act of sharing their own material** — a member outside Sanctuary may explicitly author a representation of their own MAIA exchange and Offer it. Sanctuary remains absolute; inference/summarization/recommendation/memory-mining/ambient context remain barred; **no crossing is a live pointer.** **VERIFY: `scripts/verify-constitution-circles.ts` — 22 assertions, 3 groups (C source · S service-vs-real-principals · T rolled-back fixtures).** Three-group shape is forced: **no RLS — the boundary lives in TypeScript**, so SQL-only proves nothing and service-only cannot see rolled-back fixtures. Group T always `ROLLBACK`s in `finally`; pass condition **`0 failed`, never the total**. ⛔ **NOT RUN — no `DATABASE_URL`, no `node_modules` in the remote session; predicted `17 passed · 5 failed` is STATIC ANALYSIS, NOT EVIDENCE.** Run: `docker exec maia-sovereign sh -c 'DATABASE_URL="$DATABASE_URL" npx tsx scripts/verify-constitution-circles.ts'`. **Predicted failures — 3 of 5 are one defect:** C7+C8+T3 = **FR-05 removal, ratified today and implemented nowhere** (no writer for `status='removed'`, no grounds/actor column, no cascade); C6 = `response_count` leaks a participation signal; **S4 is a QUESTION not a defect** — FR-03's "three or more" fails against four one-member production Circles, but a Circle must be able to form, so a literal reading makes every Circle unconstitutional at creation: **the first place the unratified lifecycle (CA-04) blocks a ratified rule.** Jarvis did **not** soften the assertion to make it pass. **Class-B repair order (PROVISIONAL until a real run):** R1 FR-05 removal · R2 plurality **(needs the wording ruling first — ⛔ do not add `CHECK(members>=3)`)** · R3 `response_count` · R4 B-01 founder gate before any cohort · R5 remainder. **Standing: cohorts NOT authorized · founder gate untouched · zero repairs made · ⛔ B-01 does not close because the verifier exists · doctrine byte-identical · no number ratified.** Prior record: DISCOVER census/falsification/morphology/membrane-map/gap-register; production witness = 4 circles · 4 memberships · 3 members · 0 shared artifacts · 0 inquiries ⇒ **nothing in the Circle substrate is Cat 6**; strategic finding — *"helping people find relevant people" is no longer differentiating; Soullab's distinct object is the deliberately bounded sovereign relational field.*
