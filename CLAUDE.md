...
- **LATEST — 2026-07-19**: **Episodic wire complete in production; the phase is now founder work, not builder work.** PR #609 (DEEP-primary consultation lane, §II.C) merged by Kelly + deployed + prod-verified 4/4 (`459dbcc4c`, ~16:22Z) — every prompt seam authorized to generate with memory now carries conversational + episodic + atoms addenda; remaining asymmetries (`between/chat` observe-only, consultation lane env-off) are named governance choices, not gaps. Status stays **wired, not surfacing**: zero marked moments exist; the next milestone is the return-continuity witness, which cannot be scheduled, only arrive (natural first mark → time → spontaneous return). Same day: **Relational Doorways candidate SEALED** (`docs/architecture/RELATIONAL_DOORWAYS_CANDIDATE_2026-07-19.md` — Cat 1, do-not-reopen until episodic evidence + Kelly; "AIN does not preserve the field; it keeps the doorways"); **open-PR triage delivered** (`docs/ops/PR_TRIAGE_2026-07-19.md` — 40 open → ~10 truly live; 8 recommended closes await Kelly's clicks). **Kelly's actual desk (in order)**: Sanctuary incident notification (fact-check + send) → Larry/Jondi comfort checks re: landing cards → landing live-page walk (gates essay → Landing v2) → **write the essay** ("the next movement is writing, not deployment") → S5 paper red-lines → PR close sweep + Yvonne #524 call. October = live COEE naturally, no protocol. Nothing meaningful remains blocked by engineering.
...
- **Historical (2026-05-24 → 2026-07-19, condensed — full archaeology in `docs/architecture/STATE_AND_ROADMAP_2026-05-24.md` + session memory)**: the conversational Phase 2 era (fork resolution, wire-site correction, addenda-channel divergence) is **COMPLETE**. Conversational recall merged, deployed, and production-**VERIFIED 2026-07-13** (memory `project_conversational_recall_verified` — never re-verify from log grep). The addenda divergence is fully closed: §II.B (DEEP-repair) closed 2026-05-26 via `appendAllContextAddenda`; §II.C (DEEP-primary consultation lane) closed 2026-07-19 via PR #609. Episodic Phase 2 + the "Keep this moment" mark gesture are deployed (see LATEST). Breakthrough substrate deployed. **Directives still in force from that era**: Kelly's posture shift (*"full memory in all arenas in a safe but functional way"*) — the freeze doctrine holds as **discipline, not function-blocker** (member-marked vs system-inferred · no synthesis · provenance-grounded · no static-UI claims without verified state). **Still held under freeze**: Coherence/Field wire-up (`COHERENCE_FIELD_WIRE_UP_SPEC` §0.C), Morphic/Somatic/Achievements (Later + named gates), Pattern Attunement (downstream of episodic + tact), cross-layer synthesis, any member-facing "field state"/"coherence"/"RFI"/"UFI" surface.
...
- **State of the system (six-category typology, per `docs/architecture/STATE_AND_ROADMAP_2026-05-24.md` §8 + memory `project_six_category_artifact_typology`)** — replaces earlier "honest inventory" framing. Each category is legitimate work; **collapsing 1–5 into 6 is the inflation drift to refuse**:
  ...
  - **Cat 6 — recently landed** (deployed + verified, superseding the old "branch-only" entry): conversational recall (prod-VERIFIED 2026-07-13); episodic recall Phase 2 + mark gesture (deployed 2026-07-19, `459dbcc4c` — **wired, not surfacing**; awaiting first natural member mark).
