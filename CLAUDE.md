...
- **LATEST — 2026-09-13 — `JARVIS-ORCHESTRATION-CORE-01` BW-03/BW-04 FROZEN · `MI-01` CLOSED.** Branch `claude/wonderful-edison-29hwdg`. **BW-03 `f276c730` — CLASS-C FAILURE SEMANTICS, one crossing (focus).** Outcome algebra `loaded · empty · materialization_failed · invariant_failed`; `UNAVAILABLE` stays the binder's word one layer up. ⭐ **`BW-LAW-2` enforced by the TYPE**: `FocusMaterialization` cannot express the pre-authority refusal, so a content loader has no word for "not authorized". ⛔ `empty` is deliberately NOT a failure — folding it into failure is loud and wrong; folding failure into it is the W7 defect. Disclosure changes at the authority line: **pre-authority foreign ≡ absent (byte-identical); post-authority the member is told plainly that their Work exists and could not be loaded** — which discloses nothing, because authority was already proved. ⭐ **The falsification ran in the order that makes it evidence**: against the STILL-BROKEN query the witness returned `materialization_failed` carrying the exact schema fault, distinguishable from the pre-authority refusal; **only then** was `manuscripts.user_id` → `member_manuscripts.member_id` repaired and the identical witness rerun → `loaded`. **First time a Work has ever been materialized at that crossing.** *The repair is not what makes this safe; the classification is.* **BW-04 `d3bc8670` — CLASS-C OBSERVABILITY CENSUS (read-only): `C1 15 · C2 1 · C3 4 · C4 0`.** 19 of 20 contain no try/catch; no route converts an exception to absence (`collapse_to_absent = 0` across 23 routes). Only C2 is `resolveSituatedWork`, **unwired**. ⭐ **W7 was the outlier, not the pattern** — and it was **statically visible the whole time**; *an audit that could have been run is not an audit that was run.* **`MI-01` CLOSED — COMPLETE WITHIN DECLARED HIGH-CONSEQUENCE SCOPE · RESIDUAL SURFACE UNMEASURED** (Pass 1 `8a260313` · MI-A1+Pass 2 `1390af6e` · Pass 2B `56f97e1e`). ⛔ NOT "all JSONB safe", NOT "Tier 4 audited". Censused **by consequence, never by count** (369 jsonb cols · 303 row casts · 338 `JSON.parse` — *risk is not proportional to cast count*). Findings: the organism **validates where it MUTATES and casts where it REASONS**; the **canonical turn hydrates no persisted JSON**; **live memory (`member_memory_atoms`, `member_daily_anchors`) carries NO jsonb at all** and the jsonb-heavy episodic/somatic/morphic services are **dormant and unreachable**; **no hydrated jsonb feeds any authorization decision anywhere**. Two live specimens closed by **different warrants**: `service_connectors.capabilities` (cast-only · one stable shape · **producers closed at the enumerated application boundary**, all 7 upsert sites pass compile-time literals · consequence = agent action) → **WARRANTED**, a counterexample to *"every JSONB read requires runtime validation"*; `developmental_readings.observations` (cast-only · **known v1/v2 drift** · consequence = cognition) → **WARRANTED FOR CONSUMED PROPOSITIONS** — ⭐ the drift runs in the SAFE direction (v2 *relaxed* `phenomenon` to optional, so every v1 row is a valid v2 row; **the risk is carried by NEW rows, not old ones**) and the one drifted field is read conditionally at both consumption sites. **Laws preserved: `MI-LAW-1` warrant-not-validation** (*the question is never "was this runtime validated?" but "why is this particular proposition safe to rely upon here?"*) · **`MI-LAW-2` proof is proposition-local** (outcome-verification warrants the recovered prose, never its sibling fields) · **`MI-METHOD-1` instruments nominate, they do not adjudicate** — earned by FOUR falsifications kept in the record (brace matcher misbinned 7 of 33; a helper-list omission reported 6 routes as unauthorized; a grep read a COMMENT as a dependency; ⚠️ column-name grep found 5 writers where the call graph found 7 — **false reassurance, already published when corrected**). **Reopen triggers recorded instead of a validator nobody needs: `MI-REOPEN-1`** (a NEW PRODUCER — not a reader change — invalidates the capabilities warrant) · **`MI-REOPEN-2`** (a contract change reopens compatibility only for consumed invariants). ⛔ **Tier 4 UNMEASURED BY RULING — reopen by consequence or evidence, never by completeness pressure.** ⛔ If a conservation guard is ever built it must be AST/type/symbol-level — **never another grep-count guard.** ⛔ **Standing: no validator · no migration · no version tag · no repair · no guard built · baseline still 33 · BP-1/BP-2/BP-3/BP-4 OPEN · OPEN-1 UNTOUCHED · AUTH-05 and both MI hypotheses DEMONSTRATED, NOT RATIFIED · no deploy · no production witness.** *BW: a reference does not prove authority. MI: a representation does not prove meaning.* ⭐⭐ **Trust belongs to claims, not containers.**

- **LATEST — 2026-09-13 — `JARVIS-ORCHESTRATION-CORE-01`: BW-01 FROZEN · BW-01R WITNESSED (8/8, real DB) · BW-02 CENSUS COMPLETE.** Branch `claude/wonderful-edison-29hwdg`. Lane opened by founder act on the Writer's Studio systems layer. **Spec v0.1** (ten contracts, CANDIDATE) → **Amendment A1 + R1** → **BW-01 implementation** → **freeze ruling** → **BW-01R runtime witness** → **BW-02 census**. ⭐ **`BoundWorkScope`** (`lib/jarvis/boundWorkScope.ts`) — Work authority as a minted capability rather than `memberId: string`. **Two locks**: nominal class with a private member (compile) + module-private `WeakSet` with `isBoundWorkScope()` (runtime). ⭐ **Inheritance is ADDITIVE** — object-specific binding + typed refusal + *proof destruction at serialization* from `BoundEvidence`; compile-time unforgeability + runtime provenance from `MemberIdentity`. **`toJSON()` THROWS** (BW-AUTH-2: a `BoundWorkScope` is an exercised judgment, not data; the capability is ephemeral, the claim may travel). Derivation consumes the **minted identity OBJECT**, never an extracted `VerifiedMemberId` — the brand proves how a value was *typed*, the WeakSet proves how the authority object was *created*. ⛔ **ONE call site**: the founder-gated focus crossing (`WRITERS_STUDIO_FOCUS_ENABLED`, off by default) — **production behaviour unchanged by construction**. **Gates**: `lib/jarvis` 20/20 · affected surface 92 suites / 1617 passed / 0 failed · typecheck 229 vs baseline 239, 0 regressions · `check:no-supabase` clean. ⭐ **Guards ATTACKED, not merely run**: a new raw-`memberId` export fails BW-F5 by name; `const relocatedBinder = bindWorkScope` fails BW-F7 on two independent tests (a call-site count would have stayed green while the minting authority moved). ⭐⭐ **`BW-LAW-1` / `AUTH-01` RATIFIED**: *authorization must precede materialization; content retrieval may CONSUME authority, never MANUFACTURE it* — operative test: *an authority primitive may depend on an ownership PREDICATE, never on a content LOADER.* Generalized as **AUTH-01…AUTH-04** (spec §13.1), with the five states that had collapsed — IDENTITY · REFERENCE · OWNERSHIP · CONSENT · AUTHORITY, and CONTENT always last. ⛔ **No `BoundEverythingScope`; canonize the law before generalizing the primitive.** 🔴 **BW-01R FOUND A LIVE DEFECT NO MOCKED TEST COULD**: `assembleFocus` joins `manuscripts m … m.user_id` — **that table and column do not exist** (`manuscript_sections.manuscript_id → member_manuscripts(id)`, owner column `member_id`). It throws, the throw is caught, `null` is returned — **the focus crossing has never been able to read a Work**, and an authorized focus has always been reported as unavailable. ⛔ **NOT REPAIRED** (repair unauthorized); pinned as witness case **W7**. ⭐ It is also an unplanned demonstration of the law: before BW-01 authorization and materialization were the same act, so this was *unisolatable*; after it, the witness reports authorization succeeding and materialization failing for an unrelated reason. **BW-02 distribution (33 baseline exports): `A 1 · B 12 · C 20 · D 0 · E 0`** — ⭐ **Class D EMPTY: no raw identifier in the corpus confers authority by itself.** But the benign distribution does **not** license the benign inference: ⭐⭐ **Class C is not safe-by-construction — it is safe-only-if-the-predicate-is-right, and its failure mode is SILENCE** (a broken Class C returns nothing, indistinguishable from *there is nothing*; a broken Class B refuses loudly). **Exploitability: not demonstrated anywhere. Observability: demonstrated, and it cost a whole feature.** The open question is therefore *how does a Class C function fail loudly?* — ⛔ not answered, not authorized. ⚠️ **Two instrument faults recorded, not deleted**: the C8 symmetry test first failed on shared consent state between two crossings (→ **`BW-METHOD-1`: security symmetry tests require symmetric preconditions**, or a stateful fixture manufactures apparent leakage or conceals real leakage); and the census's first automated pass returned `B 9 · C 17 · E 7` because its brace matcher truncated on braces inside template literals — **it made the organism look worse than it is**. ⛔ **Standing: BW-01 FROZEN · NO MIGRATION OF THE 33 · baseline STILL 33 (ratchet down only, never up without adjudication) · `memberOwnsWork` duplication NOT repaired (informative, and four spellings of one predicate is not yet enough consumers to justify extracting it) · W7 NOT repaired · BP-1/BP-2/BP-3/BP-4 OPEN · OPEN-1 UNTOUCHED — OPEN-1 is the work; `resolve.ts` constrains its theory (`Moved`, `unmeasured`, no fuzzy matching); J9 is the acceptance bar for closure, not a prerequisite lane · NO SCHEMA · NO MIGRATION · NO DEPLOY · NO PRODUCTION WITNESS · BRANCH GATE still prerequisite to any schema from this lane.** *Data that names a thing must not silently become proof that one may act upon the thing.*
