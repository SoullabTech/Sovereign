...
- **LATEST — 2026-08-31**: **`VOICE-CANONICAL-CONVERGENCE-02` — CLOSED IN PRODUCTION.** Spoken and typed turns now cross the same canonical cognition spine, and every link between the merge and the member is measured rather than inferred:
  ```text
  repository                 ✅ 55021771a
  running container          ✅ 55021771a
  public health provenance   ✅ 55021771a
  authenticated /maia bytes  ✅ 40/40 byte-identical
  negative control           ✅ went RED
  PWA microphone             ✅ Scarlett 2i2
  PWA spoken turn            ✅ captured
  canonical MAIA response    ✅ witnessed
  listening rearm            ✅ witnessed
  ```
  Repair: PR #1157 (`55021771a`) removed the `sendStreamingMessage(...) → return` leg from `handleVoiceTranscript` **structurally**, not by defaulting a flag off — a flag would have made the defect dormant rather than impossible. The streaming implementation (`hooks/useStreamingVoice.ts`, `app/api/voice/stream-conversation/route.ts`) is preserved and undeleted; only voice's *reachability* into it is gone. Enforcement in `__tests__/voice-non-degradation.test.ts` pins three compiler-derived properties over the whole handler — the exit set, the call sets (handler-wide + per-guard), and the **ordered** cognition tail — under 7 probes. ⛔ Four earlier enforcement versions each went green while wrong, all as denylists that fail open on the unknown; the doctrine in `docs/canon/MAIA_CONVERSATIONAL_INTELLIGENCE_NON_DEGRADATION.md` records all four.
  Referent proof: `scripts/verify-served-bundle.sh` (byte-identity witness, refusals 19–28, with a negative control that must go RED before any green is trusted). **Byte identity rather than a string grep** — this repair *removed* a leg, so the surviving `THE BETWEEN` strings appear in a stale divergent bundle too and a grep for them cannot fail. **Byte identity rather than a spoken-turn witness** — that needs working capture to prove the referent that gates testing capture.
- **NEXT LANE — Desktop capture parity.** One canonical MAIA, two surfaces:
  ```text
  ONE CANONICAL MAIA
          ├── PWA voice      ✅ known-good control
          └── Desktop voice  ⬜ next lane: capture parity
  ```
  The narrowed question: **what does Electron do differently from the proven Chrome/PWA path?** Primary surfaces — BrowserView session media-permission handling; device enumeration/selection inside the remote view; Desktop UA classification (`info.isDesktop`). ⛔ The remote BrowserView stays **off** the preload bridge (Ruling B, 2026-08-27).
  ⚠️ **First fact for that lane, measured 2026-08-31**: `maia-desktop` has **no BrowserView and no `loadURL`** — `src/main.js:481` is `loadFile` only, and `soullab.life` appears solely as an API base (`src/session.js:26`). Media permission handlers exist on `session.defaultSession` (`main.js:489-493`), scoped to the local file URL. So "parity with the PWA" presupposes a remote view that does not exist yet; the lane's first question is whether Desktop wraps the platform surface or stays a native client — **not** a permissions diff. Ruling B already forbids the local renderer becoming a second MAIA conversation UI.
  ⏸️ **Parked as a PREDICTION ONLY**: `DEFAULT_MAX_RECORDING_MS = 8000` (`lib/voice/androidVoiceFallback.ts`), an Android-recovery bound. **No defect, no repair, no tuning** unless a live PWA turn is actually witnessed cutting off at ~8s. Kelly confirmed 2026-08-31 that it is not cutting him off.
  ⛔ **Out of scope for this lane**: no PWA changes, no Whisper work, no sample-rate work, no reopening convergence.
