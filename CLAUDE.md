...
- **LATEST — 2026-09-06 — `JARVIS-CIRCLES-01 · SHARED-INTEREST FIELDS` OPENED AT DISCOVER (founder act).** Branch `claude/jarvis-circles-programme-reouzc`; charter `docs/programme/JARVIS-CIRCLES-01_CHARTER_2026-09-06.md`. **Two founder amendments landed mid-DISCOVER, before any output was written.** **A:** the opening `2–200 members in one Circle` target is **WITHDRAWN**; design law candidate *"Circles scale by multiplication, not enlargement."* **B (ontological, supersedes A's numbers):** *"Circle is not the name of the container. Circle is the name of a particular quality of relational field."* Five fields, each defined by **unit + membrane, never headcount**: Personal (1) · Circle (~3–24) · **Constellation (unit = Circles, not individuals)** · Commons (dozens→thousands) · Co-Lab (sized by the work). `"Commons Circle"` = likely category error. Open on purpose: *are two people a Circle, or a Dyadic Field?* **Census (READ ONLY, commit `39daacae`, no DB access — zero runtime rows observed):** ~2,800 LOC + 6 tables exist; auth is sound (verified-session only, documented anti-impersonation hardening); membership scoping, consent + revocation cascade, and **server-side contribute-before-you-see** are correctly built. **Absent: discovery (0%, the north-star verb has no substrate) · MAIA-in-Circle (0 call sites) · Circle↔Co-Lab bridge (0 imports) · Circle boundary verifier (Circles absent from `verify-constitution-colab.ts`).** 🔴 **G-01: the founder gate is a UI gate, not an authorization gate** — `accessMatrix.ts:504` grants all of `/api/circles` at `minTier: 'free'`, `/commons/join` is public, no API route imports `requireFounder`; membership scoping still holds, so it is not an inter-Circle leak, but *"Circles is not open for v1"* describes the UI only. **Retrieval pass RUN under a falsification posture and it contradicted the thresholds:** a 2025 systematic review (N=21,425) found group size significant in only **7 of 17** studies with *"little or no evidence"* below/above five; Dunbar re-analysis gives 95% CIs of **4–520** and **2–336**; observed discontinuities sit at **9 and 17**, not 15/24. **Size ranks FIFTH** behind facilitation · purpose · interaction structure · duration. What *is* validated: **contribute-before-you-see** (independent contribution before exposure — social influence otherwise *raises collective error* without accuracy gain) and **nesting over enlargement** (Ostrom's 8th principle; 100+ person minipublics achieve deliberative quality as *facilitated small groups*, never one enlarged room). Multiplication-not-enlargement therefore survives **with an amended rationale: size is not the cause of failure; size is what makes the real causes unaffordable.** Outputs: `CIRCLE_SCALE_FALSIFICATION.md` (8 mandated reports) · `…_EXISTING_SUBSTRATE_CENSUS…` · `…_RESEARCH_SYNTHESIS…` · `…_CONTRADICTION_GAP_REGISTER…` (G-01…G-22) · `…_RELATIONAL_SCALE_MORPHOLOGY…` · `…_MEMBRANE_MAP…` · `…_FOUNDER_DECISION_DOCKET…` (D-01…D-19). **HALTED at founder adjudication.** Founder gate untouched · doctrine byte-identical · zero code changed · no number ratified. **Comparative product analysis NOT PERFORMED (largest DISCOVER gap, D-17); rupture-and-repair across scale unresearched (D-18); one read-only production query recommended before CIRCLE-02 (D-19) — a constitution written for a substrate of unknown usage risks legislating for an empty room.** Fission must be a member act: the system may **notice and ask**, only the Circle may decide (detect → ask → record, never on a timer).
