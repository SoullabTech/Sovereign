...
- **LATEST — 2026-09-08 — WRITER'S STUDIO TRUTH / CUSTODY: two deploys, one member witness PASSED.** ⭐ **A Studio-born Work with zero Source sections and 2,072 words is visible to the Studio again.** **PRODUCTION:** `d326fc479` (Remove Work) then `3afa51b9f` (truth-observability + writing presence) — health PASS internal and external, DB PASS, `safeMode false`, provenance verified on both channels (`printenv` == `Config.Env` == asserted). No migration in either deploy. **`WS-DEVELOP-REFUSAL-TRUTH-OBS-01` — DEPLOYED, INSTALLED, PARTIALLY WITNESSED.** `sweepExpired()` had no caller; it now has exactly one and it is the clock (founder ruling: **host cron, never an in-process timer** — a timer would reintroduce the same dependency one layer down, *process lifetime*). Manual production sweep PASS: `outcome=ok · removed_count=0 · cutoff_day=2026-09-01 · duration_ms=1` — the cutoff is exactly seven days back, which only a real clock could establish. Host cron installed at `17 * * * *`; the exact command and redirect were exercised by hand, `EXIT=0`. ⛔ **INSTALLATION ≠ AUTONOMOUS FIRING** — cron has not yet been observed to fire on its own, and `removed_count=0` proves the CLI runs, **not** that an expired file was deleted (nothing in production is yet seven days old). Both remain open operational witnesses. Also corrected in-place: the manuscripts API comment and the `writingProvenance` test header both asserted an enumeration of `updated_at` writers that **missed the checkpoint route**, which advances the timestamp and the revision trail with **no `content` in its SET list** — a member can checkpoint a verbatim-seeded import and author nothing. **`STUDIO-WRITING-PRESENCE-01` — MERGED · DEPLOYED · HUMAN WITNESS PASS (PR #1269, class-a).** `manuscript_sections` is written by exactly one route — import — so for anything begun in the Studio `charCount` is 0 permanently, and **four** surfaces were reading it as a presence fact: S1 said *"No writing yet"* forever; ⭐ **S2 meant a Studio-born Work could never be continuable — a false STATE, worse than S1's false sentence**; S3 could never foreground it; S4 suppressed *"written &lt;when&gt;"* for exactly the writing written here. Repair separates three truths that may not stand in for one another — `charCount` **remains Source extent**, `hasWriting` is an OR over both layers, and `hasCurrentMemberContribution` compares the current draft to the durable revision-1 baseline, **failing closed** when that baseline is absent. `"written <when>"` is **removed, not renamed**: no lawful writing-time fact exists. The substantive-writing predicate is `~ '[^[:space:]]'`, not `btrim`, **witnessed on real PostgreSQL 16** — `btrim` strips ordinary spaces only, so tab/newline/CRLF-only drafts returned TRUE and could have been foregrounded and page-counted while holding no character. **Witness subject:** fresh Studio-born *"Writing Presence Test"* — **2,072 words · `Sections 0` · one continuous draft**, i.e. Source extent 0 — and Home recognized it as live writing, offered it under RETURN, said no *"No writing yet"*, made no false *"written &lt;when&gt;"*, and Return reopened the same Work with the writing intact. ⚠️ **Check 3 is INVALID INSTRUMENT, not a product failure**: it asked a bound-Work RETURN surface for a page count that surface has never rendered (`pagesLabel` renders only on manuscript cards and the feature). The subject itself was valid for it; only the surface was wrong. Canvas separately showed **~8 pages** from Draft extent. **OPEN / RECORDED:** Remove Work **B** — naming continuity ruled (RW-B1..B4: the Work name survives as *lineage*, never as manuscript title), repair not built; **C** — RW-C1..C6 ratified, append-only Work-act lineage at schema design R3, **not** authorized; **D** — `living_work_materials.relationship_sentence` is destroyed by Remove Work, member-authored, **classification still OPEN, breach NOT established, ledger copy FORBIDDEN**; **Studio Home asymmetry** — a bound Work with no declared `form` now renders no meta line while unclaimed writing renders extent; RECORDED ONLY, no repair authorized. **BLOCKED / PENDING:** `WORK-ACT-LINEAGE-01` R3 blocked on three production row counts (`living_works` + `living_work_expressions` + `living_work_materials`) — the single-transaction reconstruction assumes a scale nobody has read; Whole Manuscript `af013cb4a` §4b human witness pending; `DEVELOP-DWF-01` constitutionally closed (Q-A and Q-B ratified), **build not authorized**.
