...
- **LATEST — 2026-09-07 — `JARVIS-CIRCLES-01 · CIRCLE-04 REPAIR`: R1+R2+R3 VERIFIED ON CANDIDATE; **R4 VERIFICATION NOT ACCEPTED — INSTRUMENT REGRESSION**; **FR-14 verifier coverage law RATIFIED**; instrument repaired at `7d6670d7`, re-run required.** Branch `claude/jarvis-circles-programme-reouzc`; rulings `…_FOUNDER_RULINGS_2026-09-06.md` (**FR-01…FR-14** + cross-lane principle; open **CA-01…CA-15**); regression record `…_VERIFIER_COVERAGE_REGRESSION_2026-09-07.md`. **Ladder (all founder-run against DISPOSABLE SHADOW DATABASES from production schema; R2 migration applied to shadow ONLY; shadow+worktree deleted after; production NEVER migrated, NEVER deployed, `circle_membership_removals` ABSENT, still `4 circles · 4 memberships · 0 shares · 0 inquiries · 0 responses`):** R1 `ca1c4736` 18/5 · R2 `8f31abc8` 28/2 · R3 `d1742472` **34/1** (S4a–S4e + C14 all PASS). 🔴 **R4 `a148a13e` returned `31 passed · 0 failed · exit 0` — NOT A PASS.** **Jarvis's error:** the C6 replacement spliced from the C6 marker to the C14 marker and **silently deleted C9 (FR-06 discovery boundary) · C10 (FR-07 collective-release absence) · C11 (FR-01/FR-08 representational crossing) · C13 (B-01 API access gate)**. The improvement `34/1 → 31/0` came **partly from four assertions disappearing**. Founder's direct check confirmed the four product boundaries still hold on the candidate (discovery paths 0 · collective-release paths 0 · no live-pointer JOIN · 17 routes, 0 ungated) — **but inspecting a boundary once by hand is not the instrument holding it.** The 31/0 run is **PRESERVED as evidence, not erased**. **R4 PRODUCT REPAIR = provisionally sound; R4 VERIFICATION = NOT ACCEPTED.** ⭐ **FR-14 RATIFIED — VERIFIER COVERAGE LAW: *a verifier passes only when every executed assertion passes AND every required constitutional assertion is still present.*** `PASS = 0 failed AND no required assertion missing`. **`0 failed` is necessary but NOT sufficient** — this run proved *delete four assertions → none of the remainder fail → verifier reports `0 failed`*. > ***An instrument can satisfy all of its remaining questions by forgetting to ask the difficult ones.*** The invariant is not *the suite is green* but ***the obligations are still represented, and every represented obligation passes.*** ⛔ **NOT a frozen numeric total** (brittle — legitimate new assertions raise it) → **NAMED COVERAGE FLOOR**: `required missing → FAIL` · `required failed → FAIL` · `new assertion added → allowed` · `all required + new pass → PASS`. ⛔ **The verifier must not be able to become greener by forgetting what it used to verify.** ⛔ The total is never authoritative. ⛔ **An ID leaves the floor only by a founder act retiring the obligation itself — never to make a run green.** **Instrument repair `7d6670d7` (separate commit, within R4, NOT R5, no product change):** C9/C10/C11/C13 **restored verbatim from `d1742472`**; kept new destination-aware C6 · C14 · every R1/R2/R3 assertion · R4 product changes; **`REQUIRED_ASSERTIONS` names all 35 obligations**; every `pass`/`fail`/`warn`/`skip` records its leading ID; a missing required ID reports in a **COVERAGE** section and **counts as a FAILURE** (*a missing obligation is not a smaller test suite, it is an unverified boundary*); checked **last** so an aborted group still reports what went unasked; result line prints `coverage: N/35` beside the counts so a total cannot be mistaken for a gate. Mechanism unit-tested in isolation: removing `C13` → `missing: C13` → FAIL. ⛔ **NOT VERIFIED from remote sessions (no `DATABASE_URL`/`node_modules`).** **Re-run must confirm all 35 required IDs execute AND `0 failed`** + fixture rollback + shadow/worktree deleted + production unmigrated; ⛔ **do not call it PASS because the numeric total matches an expectation.** **R4 product content (unchanged):** `response_count` REMOVED not hidden from all three sites (correlated `COUNT(*)` in `listInquiries()`, its return type, the stale `FieldMemory.tsx` declaration) — nothing consumed it; ⛔ not substituted with `has_responses`/percentages/progress/ratios/badges/any proxy; **SCOPE: FR-08.7 concerns Circle SOCIAL SURFACES and member-facing status mechanics — counting stays legitimate for constitutional derivation, authorization, integrity, verification, operations**; C6 is destination-aware and ⛔ deliberately does **not** sweep for `COUNT(*)`. **Standing: R5 NOT STARTED · DEPLOY / INVOKE / COHORT NOT AUTHORIZED · R2 migration not applied to production · no facilitator assignment (CA-15) · CA-14 FieldPhase collision OPEN · founder UI gate untouched · doctrine byte-identical · no number ratified.** Only on a valid full pass does `R4 = VERIFIED ON CANDIDATE` and `R5 · CLASS-B RE-CENSUS` open (read-only, six classifications, **no repair during the census**).
