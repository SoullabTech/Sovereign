...
- **LATEST — 2026-09-11**: **`VOICE-RECOGNITION-ENGINE-01` SUSPENDED at §13 — device witness halted by a live-path lifecycle failure; successor lane `IOS-CONVERSATION-RUNTIME-01` opened as a document (`docs/programme/IOS-CONVERSATION-RUNTIME-01_LANE.md`), NO BUILD AUTHORIZED YET.** Standing: `LIVE PATH SMOKE FAIL · S1 PASS · S2 PASS · S3 FAIL intermittent · S4 FAIL when stranded · PROBE NOT RUN · A/B NOT RUN · REPAIR NOT AUTHORIZED IN THIS LANE`; failure = intermittent speak→listen lifecycle stall; recovery = the 90 s/120 s `[voice:watchdog]` in `OracleConversation.tsx` forcing listening, not spontaneous; attribution unresolved (the lane changed `AudioSessionManager.swift`, `VoiceController.swift`, the native provider and the recognition boundary — the seam is among changed surfaces, which one causes it is not established). Founder reading: conversation-lifecycle failure, not transcription quality — *"a better ear inside a body that still occasionally forgets how to breathe."* Native candidate NOT ADJUDICATED; `legacy_until_witnessed` and production routing unchanged; no dev mode entered. **Next act (no source change): calibrated Console capture** — positive control while healthy (prove `AudioSessionManager` lines are observable), reproduce the stall, three orb taps, read A (native error) / B (native clean, playback never completes) / C (taps never reach native) / D (non-diagnostic: infer nothing). Absence of lines is evidence only after the positive control. Then the founder authorizes exactly one locus-specific repair, in the successor lane. Refused for now: watchdog shortening / web re-arm mitigation (would mask the first reproducible failure) and native revert (unattributed); Control A (§12.12) held. Successor lane target: `HUMAN SPEAKS → LISTENING → TURN CLOSES → MAIA THINKS → MAIA SPEAKS → SPEECH ENDS → LISTENING, EVERY TIME`; seven invariants (one audio-session owner; explicit states; every transition has success/failure/timeout; idempotent prepare*; orb sovereign; TTS completion not sole authority for mic re-entry; per-turn transition telemetry); geometry `ConversationAudioCoordinator` over TTS + `RecognitionEngine` with `AVAudioSession` as ONE AUTHORITY; brutal acceptance (20 turns / 10 pauses / 10 failed-TTS / 10 rapid re-entries / bg-fg / lock / ringer / Bluetooth — ZERO stranded, ZERO self-fix, ZERO Kelly rescue) before anything is replaced; SpeechAnalyzer comparison resumes only after that.
- **LATEST — 2026-09-03**: **`VOICE-RECOGNITION-ENGINE-01` — IMPLEMENTATION COMPLETE, NATIVE ACCEPTANCE AND DEVICE WITNESS PENDING** (the development act here is closed; the lane is not — terminal state `CLOSED — LEGACY RETAINED | MODERN EXPERIMENTAL | PROMOTION RECOMMENDED` is assigned only after the Mac witness; founder ruling: *compile, witness, adjudicate — no more building*; record in lane doc §8) (branch `claude/layer-2-recognition-upgrade-99d1re`). iOS Layer 2 recognition moved onto an engine-neutral boundary (`ios/App/App/Recognition/`): raw `AVAudioPCMBuffer` tap → `RecognitionEngine` → `LegacySFSpeechEngine` (baseline, **still the default** by policy `legacy_until_witnessed`) | `SpeechAnalyzerEngine` (iOS 26 SpeechTranscriber, DictationTranscriber fallback, `@available`-gated, floor stays 16.0). Vocabulary: capture `flowing|unavailable` · recognition `producing|stalled` · transcript `volatile|finalized` · **human turn `open|complete` owned only by `lib/voice/recognition/humanTurnAuthority.ts`** — recognizer finality never closes the turn. Findings: `VoiceController.swift` was never in the Xcode Sources phase (now registered); repo build number is 2511, the "2515" baseline is not in git. Gates: `__tests__/voice-recognition-engine-01-*.test.ts`. Lane doc: `docs/programme/VOICE-RECOGNITION-ENGINE-01_LANE.md`. Roadmap §5 updated from "not recovered" to this ruling. **STOP line: compile, then same-device baseline-vs-modern walk on `/voice-controller-test`; default flips only after the modern engine wins.** **Remote pre-witness pass 2026-09-03 (lane doc §9, branch `claude/voice-recognition-acceptance-witness-ffeadt`, fast-forward of the lane branch):** provenance resolved — `VoiceController.swift` never in any committed pbxproj since its 2026-05-14 introduction, and the live conversation path binds `@capacitor-community/speech-recognition`, not `VoiceController`, so prior device evidence carries no attribution to the controller; builds 2512–2515 are not in git (fastlane `bump_build` writes a literal plist build number outside git — inference); recognition gates 37/37; `voice-response-toggle-mobile` failure pre-existing (reads only `OracleConversation.tsx`, unchanged by the lane). Native compile + device witness remain BLOCKED off-Mac; decision NOT REACHED; Mac handoff commands in §9.6. **Founder ruling after §9 (lane doc §10):** witness question narrowed to *is the new native subsystem a viable candidate* — not *flip production*; terminal vocabulary now `NATIVE CANDIDATE REJECTED | NATIVE CANDIDATE EXPERIMENTAL | PRODUCTION INTEGRATION RECOMMENDED`; even the last is not authorization to touch production recognition — the next seam is LIVE CONVERSATION → ENGINE-NEUTRAL BOUNDARY (`VOICE-RECOGNITION-PRODUCTION-INTEGRATION-01`, not opened). Bounded exposure noted: the lane's `AudioSessionManager.swift` teardown edit is on the live path (intended-equivalent, compile-coupled to `Recognition/`). Mac continues from `5a5ad687`. **§11 (2026-09-03):** first native compile on the Mac (relayed) FAILED with exactly one Class A diagnostic — `DictationTranscriber` has no member `isAvailable`; repair applied remotely in `RecognitionEngineSelector.swift` only (dictation eligibility = API presence + locale support, `dictationTranscriberAvailable` stays nil, reasons no longer claim a device test); Mac worktree showed only Podfile/Podfile.lock drift — present-worktree hypothesis B closed, historical unsupported not disproven; `/maia` shared-seam smoke S1–S4 ACCEPTED as a gate and runs before `/voice-controller-test`; status now FIRST NATIVE COMPILE COMPLETE — REPAIR APPLIED — SECOND COMPILE PENDING. **§12 (2026-09-11):** second native compile on the Mac (relayed) **BUILD SUCCEEDED, zero errors** at `73d0df30d` on `claude/voice-recognition-acceptance-witness-ffeadt`; the only preceding messages were CocoaPods run-script dependency-analysis warnings (Pods-generated, pre-existing, non-blocking); **O1 native viability: provisionally PASS** — necessary for witness, says nothing about runtime; no further compile repair indicated or authorized; production routing and the `legacy_until_witnessed` default unchanged. Status now SECOND NATIVE COMPILE COMPLETE — O1 PROVISIONALLY PASS — INSTALL + SMOKE PENDING. **§12.6–§12.7 (2026-09-11):** `/voice-controller-test` is excluded from the static beta bundle by design (P12 ruling; `capacitor-patch-routes.sh`), so the Probe/A-B surface is reachable only in Capacitor dev mode. **Founder ruling: dev-mode evidence ADMISSIBLE for Probe + native A/B, BOUNDED — it establishes native-candidate behaviour only, never production integration, production routing, static-beta availability, or that `/maia` uses `VoiceController`.** Record two layers separately: `NATIVE SUBJECT: 73d0df30d` and `WEB BUNDLE: beta static export at <actual SHA>`; do not claim binary identity across modes (native source + build configuration unchanged; dev mode changes WebView content source + Capacitor config). **§12.8:** first Step 1 attempt FAILED — `npm run ios:bundle` skips `capacitor-patch-routes.sh` and cannot static-export this repo (runbook error, not subject; the five named API routes are old and untouched); corrected command is `CAPACITOR_MODE=beta scripts/build-ios-static.sh build && CAPACITOR_MODE=beta npx cap sync ios`. **§12.9:** retry SUCCEEDED — static export 498 pages, `/maia` in, `/voice-controller-test` excluded (confirmed in patch log), 13 plugins synced, tree clean; actual `WEB BUNDLE: beta static export at 5846a0824` (the reset to `c756eb412` failed because zsh passed a trailing `# comment` as pathspecs; delta is docs-only). Runbook rule: no trailing `#` comments on pasted lines. **§12.10:** installed on the iPhone 16 Pro Max via `xcodebuild` (third compile, BUILD SUCCEEDED, Xcode 26.3/17C529) + `devicectl device install app` (`INSTALL METHOD: xcodebuild + devicectl (Debug)`); first launch refused only because the phone was locked (not a STOP). Header observed from the installed artefact: APP VERSION 1.2.0 · BUILD 2511 (equals repo; the out-of-git 2515 is not under witness) · iOS 26.6.1 (23G83). Launched on the unlocked phone at 21:14. **§12.11: `/maia` LIVE PATH SMOKE = FAIL, intermittent** — S1/S2 pass, S3 reply rendered without spoken audio and the mic did not return to listening; recurs, sometimes recovers. Classification: the lane's `AudioSessionManager.swift` teardown edit IS on the live path (`VoiceController.prepareForSpeaking/Listening` → native `performFullTeardown`), but the only live-path behavioural delta is the removed unconditional `engine.inputNode` access; not proven causal, not excluded. `enableVoiceInChat` defaults true, so not a wiped-localStorage effect. **Attribution turns on one question: did the same intermittent behaviour occur on the pre-lane build (2511/2515) on this phone?** **§12.12 founder ruling:** PRE-EXISTING SYMPTOM FAMILY yes · SAME FAILURE ON SAME PHONE BEFORE TODAY unknown · SMOKE fail · PROBE/A-B stopped · ATTRIBUTION unresolved; founder's characterization *"it tends to resolve itself with time"* recorded. Ordered: pre-lane **Control A** on the same phone — native `a4305f4d6` (lane merge base: no `Recognition/`, old `AudioSessionManager.swift`) + the identical web bundle `5846a0824` copied from the witness worktree's `out/` (hybrid, admissible as control only) — same S1–S4 ×5, ringer loud, Console.app `AudioSessionManager` capture on BOTH runs; pre-declared readings in §12.12. **NO REVERT AUTHORIZED.** STOP — no dev mode, no Probe, no A/B, no repair; §13 not written. **Frozen next sequence (Mac, each a STOP on failure): build beta static bundle via `build-ios-static.sh` → install on iPhone → record both layers → `/maia` S1–S4 on the beta bundle → (PASS only) re-sync Capacitor dev mode on the same native source → `MODE: CAPACITOR DEV` on every evidence line → Probe baseline/modern/dictation (`not implemented`, `web` without bridge, or wrong engine = STOP) → A/B baseline→modern→baseline→modern → adjudicate in §10.2 vocabulary (record as §13) → restore/discard dev-mode state, nothing from dev mode committed as product configuration.** Sibling thread: CMT-01 M2 live witness ACCEPTED at runtime `5f65038d2` (49/49 zero-diff) is recorded on `claude/cmt-focus-runtime-records-2026-09-10` (`e6dad6961`), not on this branch.
