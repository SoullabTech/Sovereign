...
7. **Never ask a member for a credential the system has never established with them** (founder rule, 2026-09-11) — SETTLED, and binds every candidate door. A member who authenticates by email code being shown a password form is the interface contradicting the authentication model. The rule is general: it applies wherever a surface presumes a credential rather than establishing one.
   ...
   **Standing:** spike NOT RUN · geometry NOT RULED · no door changed · production untouched. **Lane now runs as `MEMBER-ACCESS-01`** (`docs/programme/MEMBER-ACCESS-01_*`): Stages 0–6 complete, Stage 7A (P-1 evidence capture) implemented + falsified 17/17 and **NOT deployed**. Password disposition RULED 2026-09-11 — *legacy authenticator, retired per member on evidence; new members create none*. ⚠️ **Operational evidence for the door:** the admin reset tool prints a username the intended email door never uses, and every member routed to an email code needs the extra instruction *"click Email me a sign-in code instead"* — **the legacy model leaks through operational surfaces, not only the front door** (`docs/incidents/INCIDENT_2026-09-11_EXPOSED_MEMBER_PASSWORD.md`). Evidence so far: 5 of 7 stalled accounts sit at the step straight after account creation; an operator holding a valid 6-digit code was refused by the password form (2026-09-11). ⛔ Do not repair the door as a drive-by — that would adjudicate the lane by edit.
