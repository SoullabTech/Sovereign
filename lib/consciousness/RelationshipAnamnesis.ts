...
// Use API routes instead of direct browser client
export async function saveRelationshipEssence(essence: RelationshipEssence): Promise<void> {
  try {
    ...
    const response = await fetch('/api/relationship-essence', {
      method: 'POST',
      headers: {
    ...
    console.log('💾 [ANAMNESIS] Essence saved via API', essence.soulSignature);
  } catch (error) {
    console.warn('⚠️ [ANAMNESIS] Failed to save essence, continuing without persistence:', error);
  }
}

export async function loadRelationshipEssence(soulSignature: string): Promise<RelationshipEssence | null> {
  try {
    const response = await fetch(`/api/relationship-essence?soulSignature=${encodeURIComponent(soulSignature)}`);
    ...
    console.log(`💫 [ANAMNESIS] Essence loaded via API (${essence.encounterCount} encounters)`);
    return essence;
  } catch (error) {
    console.warn('⚠️ [ANAMNESIS] Failed to load essence, continuing as first encounter:', error);
    return null;
  }
}
