...
// Session-based initialization tracking to prevent repeat initialization in same session
const initializedSessions = new Map<string, boolean>();

// Check if user needs initialization
export async function needsConsciousnessInitialization(userId: string): Promise<boolean> {
  // Create a session key that persists within the same conversation
  const sessionKey = `session_${userId}_persistent`;

  // Check if this session has already been initialized
  if (initializedSessions.has(sessionKey)) {
    console.log(`🧠 [Initialization Check] Session ${sessionKey} already initialized - skipping`);
    return false;
  }

  // Mark this session as initialized to prevent repeat initialization
  initializedSessions.set(sessionKey, true);
  console.log(`🧠 [Initialization Check] Session ${sessionKey} needs initialization - proceeding`);
