 ...
 *
 * The endpoint did not actually leak, but only by accident: it reads
 * `conversation_messages`, a relation that has never existed. Verified against
 * production 2026-08-14 — HTTP 500, `relation "conversation_messages" does not
 * exist`. The missing table was the sole barrier. The real store,
 * `conversation_turns`, holds ~40k turns across ~212 members, so renaming the
 * table without first fixing the perimeter would have converted a broken
 * endpoint into an unauthenticated full-history exfiltration endpoint.
 *
