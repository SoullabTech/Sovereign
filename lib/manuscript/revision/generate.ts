...
export function revisionSystemPrompt(sections: readonly AuthorizedSection[]): string {
  return [
    ...
    'The constraints below are not a list to satisfy one at a time. A revision that',
    'obeys one by breaking another has not improved — it has traded one loss for',
    'another. Before you answer, read your proposal against the source and confirm',
    'that ALL of these survive together:',
    '  - every entity, and what each one is in relation TO',
    '  - every distinct claim',
    '  - every causal or consequential relation between them',
