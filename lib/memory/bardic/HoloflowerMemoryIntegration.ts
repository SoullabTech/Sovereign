/**
 * HOLOFLOWER MEMORY INTEGRATION (Sovereign)
 *
...
export class HoloflowerMemoryIntegration {
  ...
  async saveHoloflowerReading(reading: HoloflowerReadingData): Promise<IntegrationResult> {
    ...
    try {
      ...
      const journalEntry = await journalService.saveJournalEntry(journalInput, reading.userId);
  ...
  async detectSoulPatterns(userId: string): Promise<void> {
    ...
    try {
      ...
      const entries = await journalService.getJournalEntries(userId, 50);
  ...
  private async detectDominantElementPattern(userId: string, entries: HoloflowerJournalEntry[]): Promise<void> {
    ...
    entries.forEach(entry => {
      // Handle both JSON and object spiral_stage
      const stage = typeof entry.spiral_stage === 'string'
        ? JSON.parse(entry.spiral_stage)
        : entry.spiral_stage;
  ...
  private async detectGrowthTrajectory(userId: string, entries: HoloflowerJournalEntry[]): Promise<void> {
    ...
    const early = sorted.slice(0, 3).map(e => {
      const stage = typeof e.spiral_stage === 'string' ? JSON.parse(e.spiral_stage) : e.spiral_stage;
    ...
    if (earlyMode !== recentMode && earlyMode && recentMode) {
      ...
      await soulPatternService.saveSoulPattern({
        ...
        observations_count: entries.length,
