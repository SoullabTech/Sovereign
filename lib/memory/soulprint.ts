...
class SoulprintEngine {
  ...
  async getSoulprintForUser(userId: string): Promise<SoulprintSnapshot> {
    const cached = this.soulprints.get(userId);
    if (cached) {
      return cached;
    }
    ...
    if (this.persistenceEnabled) {
      try {
        const persisted = await this.loadFromStorage(userId);
        if (persisted) {
          this.soulprints.set(userId, persisted);
          return persisted;
        }
      } catch (error) {
        console.warn('Failed to load soulprint from storage:', error);
      }
    }
    ...
    if (this.persistenceEnabled) {
      await this.saveToStorage(newSoulprint).catch(console.error);
    }

    return newSoulprint;
  }

  private async loadFromStorage(userId: string): Promise<SoulprintSnapshot | null> {
    if (typeof window !== 'undefined') {
      const stored = localStorage.getItem(`soulprint:${userId}`);
      return stored ? JSON.parse(stored) : null;
    }
    return null;
  }

  private async saveToStorage(soulprint: SoulprintSnapshot): Promise<void> {
    if (typeof window !== 'undefined') {
      localStorage.setItem(`soulprint:${soulprint.userId}`, JSON.stringify(soulprint));
    }
  }
  ...
  async updateSoulprint(update: SoulprintUpdate): Promise<SoulprintSnapshot> {
    ...
    if (this.persistenceEnabled) {
      await this.saveToStorage(soulprint).catch(console.error);
    }
