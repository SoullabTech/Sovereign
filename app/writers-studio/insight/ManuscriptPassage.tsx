...
export default function ManuscriptPassage({ body, range, proposal, children, annotation, highlight = true }: {
  ...
  const valid = Number.isInteger(start) && Number.isInteger(end) && start >= 0 && end > start && end <= points.length;
  ...
  const earlier = paragraphStart < 0 ? '' : before.slice(0, paragraphStart + 2);
  const leadIn = paragraphStart < 0 ? before : before.slice(paragraphStart + 2);
  ...
  return <div className="ws-manuscript-passage">
    {earlier && <div className="ws-manuscript-context">{earlier}</div>}
    <div className="ws-manuscript-context ws-annotated-paragraph">
      ...
      {leadIn}<span className={(valid && highlight) || shown ? "ws-marked-passage" : "ws-section-note-anchor"} data-preview={Boolean(shown)} data-editorial-locus>
      {shown ? diff ? <>{diff.before}<del>{diff.removed}</del><ins>{diff.added}</ins>{diff.after}</> : shown.wording || <em>Proposed removal</em> : <SourceText text={original} />}
...
/** Import filenames remain in source and offsets; present them as source metadata,
 * never as an editorial suggestion or silently deleted manuscript content. */
function SourceText({ text }: { text: string }) {
  return <>{text.split(/^([ \t]*ch\d+\.xhtml[ \t]*)$/gm).map((part, i) => /^[ \t]*ch\d+\.xhtml[ \t]*$/.test(part)
    ? <span key={i} className="ws-import-marker" title="Filename from the imported manuscript; source text is unchanged">{part}</span>
    : part)}</>;
}
