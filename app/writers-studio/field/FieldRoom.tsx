...
export default function FieldRoom({
  ...
  return (
    <div
      ...
      style={{
        /* ⭐ THE ROOM IS AN INTERIOR, NOT A PAGE. It fills the seat the Studio
           shell gives WRITE and becomes the containing block its own orbits are
           positioned against. Before this it declared `minHeight: 100vh` and
           positioned against the viewport, which made it a second shell — and a
           second shell is the destination this room exists to stop being. */
    ...
    >
      ...
      <nav
        ...
        data-field-rail
        style={{
          position: OUT_OF_FLOW, left: 0, top: 0, bottom: 0, width: `${RAIL_REM}rem`,
      ...
      >
        {/* ⛔ NO SECOND MASTHEAD. This room used to render the Work's name in
            its own <h1>. Inside the Studio that is a duplicate: the shell
            already carries the wordmark, the Work's name and the mode bar, and
            they persist across WRITE and DEVELOP precisely so the writer never
            has to re-identify what is on the table. Two headings naming one
            Work is how two generations of a product read as two products.

            `title` and `note` remain in the props because the room must still
            be able to name a Work where no shell surrounds it — a study
            surface, a future embed — but nothing in the Studio path reads
            them. */}
      ...
      {workbench !== undefined && (
        <div
          ...
          style={{
            position: OUT_OF_FLOW, left: `${RAIL_REM}rem`, right: 0,
