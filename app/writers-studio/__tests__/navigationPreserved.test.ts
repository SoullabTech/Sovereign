...
describe('the focus presentation seam', () => {
  ...
  /**
   * ⛔ AN UNUSED SEAM CHANGES NOTHING — AND NOW THAT IS LITERALLY TRUE.
   *
   * The first form rendered its wrapper <div> UNCONDITIONALLY and only varied
   * the style, so "byte-for-byte what it was" was nearly true: the tree gained
   * an element wherever no room drew. Founder ruling 2026-09-10 made the
   * wrapper itself conditional, so an unused seam adds no element, no
   * positioning context and no metric override.
   *
   * ⭐ THE ASSERTION FOLLOWS THE STRONGER CLAIM. `OverlayOrigin` returns a bare
   * fragment when inactive — that is the property, and it is what the comment
   * on the seam is now entitled to say.
   */
  it('is inert when no room draws through it — no wrapper, no metrics', () => {
    const src = bare(surface);
    /* The wrapper exists only when a room draws. */
    expect(src).toMatch(/if \(!active\) return <>\{children\}<\/>;/);
    expect(src).toContain('<OverlayOrigin active={Boolean(renderSectionOverlay)}>');
    /* And the editor's own metrics are overridden only then. */
    expect(src).toContain('...(renderSectionOverlay ? { padding: 0, margin: 0 } : null)');
    /* ⛔ The old unconditional form must not come back. */
    expect(src).not.toContain("<div style={renderSectionOverlay ? { position: 'relative' } : undefined}>");
  });
