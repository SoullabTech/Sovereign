...
test('Guided opens with MAIA reading, coverage and responses without requiring an angle', async () => {
  ...
  /* ⭐ C6 convergence: an exact bound response enters the same governed
...
test('Work direction enters only after explicit member gesture and remains editable/removable', async () => {
  ...
  const tryButton = Array.from(container.querySelectorAll('button'))
    .find(b => b.textContent === 'Try a revision');
  ...
  expect(onRevise).toHaveBeenCalledTimes(1);
  expect(onRevise.mock.calls[0][0].sectionId).toBe('s20');
  expect(onRevise.mock.calls[0][1]).toContain('My edited direction');
  expect(onRevise.mock.calls[0][1]).toContain('Do not treat partial agreement as a stored declaration');
  expect(container.querySelector('[data-dialogue-question]')).toBeNull();
