...
import { WholeManuscriptSurface } from '@/app/writers-studio/canvas/WholeManuscriptSurface';
import { ProposalEvidenceInWork } from '@/app/writers-studio/canvas/ProposalWorkSurface';
...
const evidence = (sectionId: string) => {
...
const renderWhole = async (jumpTo: string | null) => {
  await act(async () => {
    root.render(createElement(WholeManuscriptSurface, {
      writing,
      jumpTo,
      onJumpHandled: jest.fn(),
...
describe('C10 · Whole voluntary return addresses the proposal locus', () => {
  it('keeps the return owed across mounting until the exact locus is addressed', async () => {
    ...
    /* Same Whole navigation command produced by the voluntary asked return. */
    await renderWhole(TARGET_ID);
    ...
    /* Current bad state, stated so a selector/mount failure cannot impersonate it. */
    if (actualTarget === shell && actualTarget !== locus) {
      throw new Error(
        'C10_CAPABILITY_RED: fixture valid; exact locus rendered/resolvable; ' +
