...
function CanvasRoom({ editorialEnabled }: { editorialEnabled: boolean }) {
  ...
  /* ── WS-EDITORIAL-UI-01A · WHICH EDITORIAL CONVERSATION THIS ROOM MEANS ──
     Latched from the route by the SAME rule as the manuscript above: adopt
     when empty, never overwrite. The address is what survives close, reopen,
     reload and back — ⛔ not this state, which is only the room's copy of it.

     ⛔ There is no fallback. `resolveManuscript` may degrade to the most
     recent manuscript; this may not degrade to anything, because the schema
     admits MANY threads per chain and "the latest one" is a guess wearing the
     costume of a lookup. Either the address names a thread or none is open. */
  const routeEditorialThread = searchParams === null ? null : editorialThreadIdFrom(searchParams);
  const [editorialThreadId, setEditorialThreadId] = useState<string | null>(routeEditorialThread);
  useEffect(() => {
    if (routeEditorialThread === null) return;
    setEditorialThreadId((prev) => adoptRouteIdentity(prev, routeEditorialThread));
  }, [routeEditorialThread]);
  const [editorialOpening, setEditorialOpening] = useState(false);
  const [editorialRefusal, setEditorialRefusal] = useState<string | null>(null);
  ...
  /* ⭐⭐ THE OPENING ACT IS THE MEMBER'S GESTURE, AND IT HAPPENS ONCE.
     ...
     the panel's mount effect: a component that opens a relationship when it
     renders opens one every time it remounts.
 ...
 */
export default function CanvasClient({ editorialEnabled }: { editorialEnabled: boolean }) {
  return (
    <Suspense fallback={<div style={{ minHeight: '100vh', background: GROUND.base }} />}>
      <CanvasRoom editorialEnabled={editorialEnabled} />
    </Suspense>
  );
}
