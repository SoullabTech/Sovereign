...
const STATUS_COLORS: Record<string, string> = {
  ...
  referred_out: 'bg-purple-100 text-purple-700',
};
      ...
      <div className="min-h-screen bg-gradient-to-b from-stone-50 to-stone-100">
        <div className="max-w-5xl mx-auto px-6 py-8">
          <div className="animate-pulse">
            <div className="h-6 bg-stone-200 rounded w-24 mb-4" />
            <div className="h-8 bg-stone-200 rounded w-64 mb-8" />
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              <div className="lg:col-span-2 h-64 bg-stone-100 rounded" />
      ...
      <div className="min-h-screen bg-gradient-to-b from-stone-50 to-stone-100">
        <div className="max-w-5xl mx-auto px-6 py-8">
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            <p className="text-red-700">{error || 'Container not found'}</p>
            <button
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="flex items-start justify-between gap-4 mb-6">
          <div>
            ...
            <h1 className="text-2xl font-light text-stone-800">{containerTitle}</h1>
            <div className="flex items-center gap-3 mt-2">
              <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${STATUS_COLORS[container.status] || 'bg-stone-100'}`}>
                {container.status}
              </span>
              <span className="text-sm text-stone-500 capitalize">{container.type}</span>
              {container.startAt && (
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-4 py-2 rounded-md bg-white border border-stone-200 text-stone-600 hover:bg-stone-50 transition-colors"
            >
              Transition
            </button>
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-lg border border-stone-200/60 p-5">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xs font-medium tracking-wider text-stone-400 uppercase">
                  Upcoming Sessions
                </h2>
                <button
                  onClick={() => router.push(`/practitioner/containers/${containerId}/session/new`)}
              ...
              {upcomingSessions.length === 0 ? (
                <p className="text-sm text-stone-400 italic">No sessions scheduled</p>
              ) : (
            ...
            <div className="bg-white rounded-lg border border-stone-200/60 p-5">
              ...
              {pastSessions.length === 0 ? (
                <p className="text-sm text-stone-400 italic">No sessions yet</p>
              ) : (
            ...
            <div className="bg-white rounded-lg border border-stone-200/60 p-5">
              ...
              {openTasks.length === 0 ? (
                <p className="text-sm text-stone-400 italic">No open tasks</p>
              ) : (
                <div className="space-y-2">
                  {openTasks.map(task => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className={`text-xs ${isOverdue ? 'text-red-500' : 'text-stone-400'}`}>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg border border-stone-200/60 p-5">
              ...
              {participants.length === 0 ? (
                <p className="text-sm text-stone-400 italic">No participants yet</p>
              ) : (
                <div className="space-y-3">
                  {participants.map(p => (
            ...
            <div className="bg-white rounded-lg border border-stone-200/60 p-5">
              ...
              {agreements.length === 0 ? (
                <p className="text-sm text-stone-400 italic">No agreements</p>
              ) : (
                <div className="space-y-2">
                  {agreements.map(a => (
                    <div
            ...
            <div className="bg-white rounded-lg border border-stone-200/60 p-5">
              ...
              <dl className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <dd className="text-stone-700">{formatShortDate(container.createdAt)}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-stone-500">Type</dt>
                  <dd className="text-stone-700 capitalize">{container.type}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-stone-500">Visibility</dt>
                  <dd className="text-stone-700 capitalize">{container.visibility}</dd>
                </div>
                {container.riskFlags && container.riskFlags.length > 0 && (
                  <div className="flex justify-between">
                    <dt className="text-stone-500">Risk flags</dt>
                    <dd className="text-amber-600">{container.riskFlags.join(', ')}</dd>
                  </div>
                )}
              </dl>
            </div>
