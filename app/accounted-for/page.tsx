      ...
      <div className="mx-auto min-h-screen max-w-[1040px] border-x border-soullab-border-subtle bg-soullab-canvas pb-16">
        <div className="mx-auto max-w-[68ch] px-6 text-[17px] leading-[1.6]">
          ...
          <section className="pt-11">
            ...
            <P>AIN OS is therefore sovereign across model turnover. Nothing that constitutes MAIA lives inside the model: memory, provenance, developmental history, elemental and archetypal orientation, relational continuity, permissions, and member-owned data are all held in the architecture around it. Models may improve, be deprecated, or be withdrawn by the companies that run them; the member&apos;s history, relationships and developmental continuity stay where they are. That is the difference between using an AI model and building a sovereign intelligence architecture.</P>
            <P>Stated plainly: the models are in service to the member and the architecture, not the member to the models.</P>
            <P>What persists is worth naming rather than implying. It is the architecture, the memory, the provenance, the developmental history, the relational continuity, and the member&apos;s authority over all of it. That is a claim about where this system&apos;s identity is held. It is not a claim that some identical subject survives a change of model &mdash; there is no evidence for that, and the sovereignty claim does not need it.</P>
            <P>Sovereignty across model turnover is an architectural property, not a track record: it says nothing constitutive is stored in the model, not that MAIA has been run across several. What the provider layer actually looks like today, with its own labels, is set out under reservation four.</P>
          </section>
