        ...
        <div className="mx-auto max-w-[68ch] px-6 text-[17px] leading-[1.6]">
          ...
          <Section eyebrow="What we do not know" heading="Questions that remain genuinely open">
            ...
            <ul className="mb-5 list-disc pl-5 text-soullab-text-secondary [&_li]:mb-2.5">
              ...
              <li>How memory should be decomposed at all. The fifth inquiry proposes withdrawing the episodic, semantic, pattern and present-state typology in favor of what the architecture already does: memory by who authored it and when it was true, with what was actually said kept beneath anything derived from it. And whether the present should overturn an accumulated model of the person now has a precise proposed answer: what the person says now overrides; what MAIA infers about now does not, it asks. Whether accurate memory, held that way, still narrows a person&apos;s future remains open; nobody has measured it.</li>
          ...
          <Section eyebrow="What we are testing" heading="The Human&ndash;AI Relationship research programme, by rung">
            ...
            <ul className="mb-5 list-disc pl-5 text-soullab-text-secondary [&_li]:mb-2.5">
              ...
              <li><strong className="text-soullab-text-primary">A critical-challenge pass, five inquiries, whose job is to break the synthesis</strong> &mdash; companionship and dependence, attributed personhood, alliance and repair, persuasion critiques, memory and identity. Three have been run against roughly a hundred and sixty sources, each sweep instructed to look for evidence against Soullab&apos;s preferred conclusion, and all three are adjudicated. The first found that no trial anywhere has tested a relationship design that points outward against one that maximizes engagement, and that no therapeutic chatbot trial has measured effects on human relationships. The second narrowed the concern from a person attributing inner life, which is a stable human disposition, to a system claiming need or reciprocity it cannot warrant. The third found the transfer of human rupture-and-repair concepts to an AI to be the weakest inference in the synthesis: MAIA may correct and re-attune; she does not claim relational repair, and the party that answers for the relationship is Soullab. The fourth, now open, turns the challenge on Soullab itself: whether we can tell sovereignty-supporting influence from manipulation, or have only built a flattering vocabulary for the influence we prefer. Every source in those records is verified to exist; none has yet been read line by line against its text, so no figure from them appears on this page.</li>
