 ...
 * ---------------------------------------------------------------------------
 * `EXPORT_SOURCE_TABLE` below names a relation that is absent from the
 * witnessed production state (2026-08-14: `relation "conversation_messages"
 * does not exist`), and no migration creating it was found in this repository.
 * That is the strength of the claim — it is not an assertion about every
 * historical environment. An authenticated member exporting their own history
 * still fails at the query. That is the intended intermediate state: a secure
 * broken endpoint.
 *
 * DO NOT "fix" this by renaming the table to `conversation_turns`. In the same
 * witnessed state that store held ~40k turns across 212 distinct `user_id`
 * values (distinct ids, not verified member identities) and carries NO
 * Sanctuary column, so a rename would silently ship an export path that cannot
 * honour the Sanctuary boundary. Binding a real source is a separate, governed
 * unit that must first
