...
export async function GET(request: NextRequest) {
  try {
    ...
    /*
     * Practitioner door — server-decided, per the one-page/two-clearances
     * direction (2026-07-13): the holder's field is reached from the bottom of
     * the member Home, and the door exists in the payload ONLY for the member
     * who holds a practice field (getAuthoredField derives the field from the
     * session member; there is no field parameter to probe). Every other
     * member receives `practitioner: null` — no door, no admin data. The
     * admin surface itself is gated again at /api/now-what/admin.
     */
    let practitioner: { fieldSlug: string } | null = null;
    try {
      const { getAuthoredField } = await import('@/lib/practiceField/programAuthoringService');
      const authored = await getAuthoredField(memberId);
      if (authored?.fieldSlug) practitioner = { fieldSlug: authored.fieldSlug };
    } catch (err) {
      console.warn('[NowWhat/home] practitioner read failed (non-fatal):', err);
    }
