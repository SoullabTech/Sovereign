...
function getDeployAgeHours(): number | null {
  try {
    const raw = execSync(
      'docker inspect maia-sovereign --format "{{.Created}}" 2>/dev/null',
      { timeout: 3000 },
    )
      .toString()
      .trim();
...
function validateCookie(cookieValue: string | undefined): boolean {
  const password = process.env.STATUS_PAGE_PASSWORD;
...
export async function GET(request: NextRequest) {
  const cookieValue = request.cookies.get('status_session')?.value;
  if (!validateCookie(cookieValue)) {
    return NextResponse.json({ error: 'unauthorized' }, { status: 401 });
  }

  const [services, incidents, system] = await Promise.all([
    getServicesWithStatus(24),
    getIncidents(20),
    getSystemMetrics(),
  ]);

  return NextResponse.json({ services, incidents, system, checkedAt: new Date().toISOString() });
}
