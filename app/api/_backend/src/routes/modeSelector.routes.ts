...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (
        !sacredMirrorMode ||
        !["jung", "buddha", "hybrid", "adaptive"].includes(sacredMirrorMode)
      ) {
        return res.status(400).json({
          success: false,
          message:
            "Valid sacred mirror mode is required (jung, buddha, hybrid, or adaptive)",
        });
      }
