...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // ── SESSION IS AUTHENTICATION (invariant, 2026-08-24) ──────────────────
    ...
    // localStorage session with no server session behind it. That produces the
    // "MAIA thinks I'm someone else / I'm signed in here but not there" class
    // of bug — the client believes it is authenticated while the server does
    // not. No valid server session means authentication has NOT completed, so
    // the session is created BEFORE any success is reported, and a failure is
