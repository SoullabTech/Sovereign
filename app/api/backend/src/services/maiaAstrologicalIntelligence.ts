...
// import { comprehensiveAstrologicalService } from './archive/comprehensiveAstrologicalService'; // Disabled for deployment
...
class MAIAAstrologicalIntelligenceService {
  ...
  async getOrCreateContext(userId: string): Promise<MAIAAstrologicalContext> {
    ...
    try {
      // const transits = await comprehensiveAstrologicalService.trackCurrentTransits(userId); // Disabled for deployment
  ...
  private async generateTimingAdvice(userId: string, themes: string[], context: MAIAAstrologicalContext): Promise<string | undefined> {
    try {
      // const timing = await comprehensiveAstrologicalService.generateSacredTiming(userId); // Disabled for deployment
    ...
    return undefined; // Return undefined when service is disabled
  }
