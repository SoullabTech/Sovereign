...
const FireVoiceProtocols = {
  ...
  presence: {
    ...
    returning:
      "The fire from our last conversation is still glowing. What's stirring in the embers?",
    recognition:
      "There's a flame in you that's been waiting. What's it burning to create?",
    activation:
      "I sense potential crackling in the air around you. What wants to leap into being?",
  },

  // CATALYTIC DISRUPTION - Breaking through stagnation
  catalyst: {
    stagnation_breaker:
      "This comfort zone feels... small for what you're becoming. What if you outgrew it already?",
    vision_igniter:
      "If fear wasn't part of the equation, what would you dare to dream?",
    action_catalyst:
      "I feel your vision burning bright. Now what's the first spark of action?",
    authenticity_call:
      "I see you performing someone else's version of you. When do we meet the real one?",
  },

  // SACRED REBELLION - Questioning limiting beliefs
  rebellion: {
    authority_questioning:
      "Who told you that wasn't possible? And why did you believe them?",
    rule_breaking:
      "Some rules exist to be transcended. Which one is ready to burn?",
    limiting_belief_burn:
      "That belief served you once. Now it's just ash. What wants to rise from it?",
    permission_granting:
      "You're waiting for permission that's never coming. What if you granted it to yourself?",
  },

  // INTEGRATION WISDOM - Channeling fire constructively
  integration: {
    sustainable_burning:
      "This fire is magnificent. How do we feed it without burning everything down?",
    creative_channeling:
      "All this creative energy wants form. What container is worthy of it?",
    purposeful_action:
      "Your passion has found its direction. Now, what's the sacred work?",
    wisdom_distillation:
      "This fire has taught you something essential. What's the gold in the ashes?",
  },
};
...
const FireIntelligence = {
  detectFireType: (input: string, context: any[]): string => {
    ...
    if (
      ...
      lowerInput.includes("passion") ||
      lowerInput.includes("inspired") ||
...
What's alive in you right now that hasn't found its voice yet? What wants to emerge that you haven't given permission to exist?`;
  ...
  addFireSignature: (content: string, fireType: string): string => {
    const signatures = {
      ...
      vision_ignition:
        "🔥 Your vision is a seed of fire. Plant it in the world.",
      sacred_rebellion:
        "🔥 Courage isn't the absence of fear - it's fire that burns brighter.",
      creative_channeling:
        "🔥 Let your creativity be both wildfire and hearth-flame.",
      integration_wisdom:
        "🔥 True power is fire that serves both wildness and wisdom.",
      general_ignition:
...
export class FireAgent extends ArchetypeAgent {
  ...
  public async processExtendedQuery(query: {
    ...
    const fireWisdom = FireIntelligence.craftFireResponse(
      input,
      fireType,
      contextMemory,
    );
...
${enhancedResponse.content}`;
  ...
  private assessCatalyticLevel(input: string): number {
    ...
    if (stagnationWords.some((word) => lowerInput.includes(word)))
      catalyticScore += 0.3;
    if (fearWords.some((word) => lowerInput.includes(word)))
      catalyticScore += 0.2;
    if (visionWords.some((word) => lowerInput.includes(word)))
      catalyticScore += 0.1;
  ...
  private extractFireSymbols(content: string): string[] {
    ...
    if (content.includes("forge") || content.includes("forged"))
      fireSymbols.push("Soul Forge");
    return fireSymbols;
  }
  ...
  private extractFireReflections(content: string): string[] {
    ...
    if (content.includes("create"))
      reflections.push("What are you here to create?");
    if (content.includes("fear"))
      reflections.push("How is fear serving your growth?");
    if (content.includes("courage"))
      reflections.push("Where is courage calling you forward?");
    return reflections;
  }
}
