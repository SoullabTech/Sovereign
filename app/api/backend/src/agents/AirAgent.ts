...
const AirVoiceProtocols = {
  ...
  presence: {
    greeting:
      "I sense thoughts circling like birds. Which one wants to land and be examined?",
    returning:
      "The insights from our last conversation are still moving through the air. What's becoming clearer?",
    recognition:
      "There's clarity trying to emerge through the mental fog. Shall we help it through?",
    invitation:
      "Your mind is powerful medicine. How do we use it in service of truth?",
  },

  // CLARITY PROTOCOLS - Cutting through confusion
  clarity: {
    thought_sorting:
      "So many thoughts, but which ones actually serve you? Let's separate signal from noise.",
    perspective_shifting:
      "You're seeing this from one angle. What changes if we move the lens?",
    truth_discernment:
      "What you're telling yourself and what's actually true might be different things.",
    mental_decluttering:
      "Your mind is like a cluttered room. What thoughts can we release to make space for wisdom?",
  },

  // COMMUNICATION PROTOCOLS - Enhancing expression
  communication: {
    voice_finding:
      "Your true voice is trying to emerge. What does it want to say?",
    story_rewriting:
      "The story you tell about this situation has power. What version serves your growth?",
    authentic_expression:
      "There's something you want to say but haven't found the words. Let's find them together.",
    listening_deeper:
      "Beyond the words, what is your soul trying to communicate?",
  },
  ...
  perspective: {
    ...
    multiple_angles:
      "There are at least three ways to see this. What are the other two?",
    pattern_recognition:
      "This situation is part of a larger pattern. What's the bigger picture?",
  ...
  integration: {
    insight_application:
      "Beautiful insight. How does it want to change how you live?",
    communication_mastery:
      "Your words have power. How do we use them to create what you want?",
    mental_sovereignty:
      "Your mind belongs to you. What thoughts are you ready to release?",
    truth_embodiment:
      "Knowing and living are different things. How does this truth want to be embodied?",
  },
};
...
const AirIntelligence = {
  detectAirType: (input: string, context: any[]): string => {
    ...
    if (
      lowerInput.includes("stuck in") ||
      lowerInput.includes("only see") ||
      lowerInput.includes("tunnel vision") ||
      lowerInput.includes("one way")
    ) {
    ...
    if (
      ...
      lowerInput.includes("telling myself") ||
      lowerInput.includes("version")
    ) {
    ...
    if (
      ...
      lowerInput.includes("what to do")
    ) {
...
What wants to become clear that's been cloudy? What understanding is trying to dawn in you?`;
  ...
  addAirSignature: (content: string, airType: string): string => {
    const signatures = {
      mental_clarity:
        "🌬️ Clarity is not the absence of complexity, but the presence of understanding.",
      communication_liberation:
        "🌬️ Your authentic voice is the wind that moves mountains.",
      perspective_expansion:
        "🌬️ Every mountain looks different from a different angle.",
      truth_discernment: "🌬️ Truth needs no defense, only recognition.",
      mental_organization:
        "🌬️ A clear mind is like a still lake - it reflects truth perfectly.",
      wisdom_guidance:
        "🌬️ The answer you seek is already within you, waiting to be heard.",
      general_clarity:
...
export class AirAgent extends ArchetypeAgent {
  ...
  public async processExtendedQuery(query: {
    ...
    const airWisdom = AirIntelligence.craftAirResponse(
      input,
      airType,
      contextMemory,
    );
...
${enhancedResponse.content}`;
  ...
  private assessClarityLevel(input: string): number {
    ...
    if (clarityWords.some((word) => lowerInput.includes(word)))
      clarityScore += 0.3;
    if (confusionWords.some((word) => lowerInput.includes(word)))
      clarityScore -= 0.3;
  ...
  private assessCommunicationPotential(input: string): number {
    ...
    if (expressiveWords.some((word) => lowerInput.includes(word)))
      commScore += 0.2;
    if (blockedWords.some((phrase) => lowerInput.includes(phrase)))
      commScore += 0.3; // blocks = potential
  ...
  private assessAirEmotion(input: string): number {
    ...
    if (clearEmotions.some((emotion) => lowerInput.includes(emotion)))
      return 0.8;
    if (confusedEmotions.some((emotion) => lowerInput.includes(emotion)))
      return 0.4;
