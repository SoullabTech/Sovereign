...
// Live source only: reads GET /api/admin/maia/feedback and mutates via PATCH /api/admin/maia/feedback/[id].
...
export default function FeedbackInboxPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/admin/maia/feedback');
  ...
  const patch = useCallback(
    async (id: number, body: Record<string, unknown>) => {
      try {
        await fetch(`/api/admin/maia/feedback/${id}`, {
          method: 'PATCH',
      ...
      <div className="sticky top-0 z-20 border-b border-white/10 bg-zinc-950/95 backdrop-blur px-5 py-4">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            <button
              onClick={() => router.push('/admin/maia')}
