  ...
  return (
    <div className="relative w-full max-w-4xl mx-auto">
      ...
      {/* Memory Indicator — UI chrome in its own dedicated band, in normal
          flow above the input container. mt-2 gives extra space above the
          band; pl-28 indents the chrome past the Ask MAIA pill's right edge
          so the pill cannot intrude into the band's left side. Heavily
          opacity-reduced so the eye registers it as ambient state. */}
      {hasMemory && (
        <motion.div
          initial={{ opacity: 0, y: -2 }}
          animate={{ opacity: 1, y: 0 }}
          className="mt-2 mb-3 pl-28 pr-1 flex items-center gap-1.5 text-[10px] uppercase tracking-[0.15em] text-gold-divine/30 select-none pointer-events-none"
