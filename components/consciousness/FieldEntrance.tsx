    ...
    if (currentPhaseIndex < sequence.phases.length - 1) {
      ...
      const timer = setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          setShowPrompt(true)
        }, 1500) // Slower, more contemplative transition
      }, duration)
...
function calculateDuration(phase: any): number {
  const baseDuration = {
    'very_slow': 12000,  // 12 seconds for deep contemplation
    'slow': 8000,       // 8 seconds for grounding
    'medium': 6000,     // 6 seconds for presence
    'flowing': 4000,    // 4 seconds minimum
  }

  return baseDuration[phase.rhythm.pace as keyof typeof baseDuration] || 6000
}
