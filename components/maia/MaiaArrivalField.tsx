 ...
 *   Greeting → Invitation → Jewel → Primary action → Suggested prompts →
 *   Composer → The House (quiet, at the base).
 *
 ...
 * send (composer or a suggested prompt) the parent activates and this field
 * gives way to the conversation. No logic, routing, auth, or flags are changed
 * here — this is presentation of the arrival state only.
 */
...
export interface MaiaArrivalFieldProps {
  ...
  /**
   * Cross the threshold without speaking. Activation is not expression: the
   * member is answering MAIA's "I'm here when you're ready", not opening the
   * conversation. This is a state transition only — no transcript turn, no
   * generated reply. Sending "I'm ready" as a literal message made MAIA greet
   * the member a second time; the ceremony is one greeting, one answer.
   */
  onActivate: () => void;
...
export function MaiaArrivalField({ greeting, subtext, userInitial = 'K', onSend, onActivate, onOpenHouse, onKeep, onTapJewel }: MaiaArrivalFieldProps) {
  ...
  return createPortal(
    ...
    >
      {/* Compact header — utilities only. The House is NOT here: one renderer owns
          the doorway, and it is the quiet one at the base of the composition. A
          header pill duplicated it and competed with the beginning. */}
      ...
      <div className="flex w-full max-w-[560px] flex-col items-center">
        ...
        >
          ...
          />
          <span className="mt-4 text-[10.5px] font-semibold uppercase tracking-[0.28em] text-[rgba(220,210,235,0.42)]">
            Tap to speak
          </span>
        </button>
        ...
        <button
          type="button"
          onClick={onActivate}
          className="mt-7 rounded-full border border-[rgba(230,169,74,0.55)] px-10 py-2.5 text-[clamp(15px,2vw,17px)] text-[#e6c48c] transition-colors duration-200 hover:border-[rgba(230,169,74,0.95)] hover:text-[#f2d9aa] focus-visible:border-[rgba(230,169,74,0.95)] focus:outline-none"
        ...
        >
          ...
          <input
            ...
            style={{ fontFamily: SERIF }}
          />
          <span className="text-[rgba(230,169,74,0.7)]" aria-hidden>◍</span>
        </form>

        {/* The House — quiet doorway at the base of the composition, clear of the composer */}
        <button
          type="button"
          onClick={onOpenHouse}
