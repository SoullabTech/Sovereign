...
export function MaiaHouseSheet({
  ...
  const ctx = (): HouseDispatchContext => ({
    isNative,
    // MERGE RECONCILIATION (2026-07-28): main's registry dispatch is the
    // successor design; the native lane's finding rides inside it. Native
    // shell: SPA-first with document-load fallback — a document load remounts
    // the app and resets the live conversation (measured, build 2504). See
    // lib/navigation/capacitorNavigate.ts. Web: nativeNavigate is a no-op
    // and the normal router runs.
    push: (p) => {
      if (nativeNavigate((r) => router.push(r), p)) return;
      router.push(p);
    },
    openSheet,
    onClose,
