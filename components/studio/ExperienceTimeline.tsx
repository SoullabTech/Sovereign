...
/**
 * ExperienceTimeline — Field Events Between Council Consultations
 *
 * Practitioners log what happened in the field: breakthroughs, setbacks,
 * reflections, field events. This is the connective tissue between iterations.
 */
...
import type { DecisionExperience, ExperienceType } from '@/lib/studio/leadership/types';
...
const TYPE_CONFIG: Record<ExperienceType, { icon: typeof Compass; color: string; label: string }> = {
  ...
  setback: { icon: Cloud, color: 'text-slate-400', label: 'Setback' },
};
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-1.5">
        {(Object.entries(TYPE_CONFIG) as [ExperienceType, typeof TYPE_CONFIG[ExperienceType]][]).map(([key, cfg]) => {
          const Icon = cfg.icon;
          return (
            <button
              key={key}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      {ElementIcon && (
                        <ElementIcon className={`w-3 h-3 ${elementCfg!.color}`} />
                      )}
