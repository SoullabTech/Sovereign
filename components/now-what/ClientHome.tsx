...
interface HomePayload {
  ...
  /** Server-decided: present ONLY when this member holds a practice field. */
  practitioner?: { fieldSlug: string } | null;
}
  ...
  return (
    <div className="nwh-root">
      <div className="nwh-frame">
        ...
        {/* ── The practitioner's door — server-decided, absent for members ──
            The flag exists in the payload only when this member holds a
            practice field (one page, two clearances: member environment
            above, holder's field below). Everyone else receives nothing —
            no door, no admin data. */}
        {data?.practitioner && (
          <div className="nwh-admin">
            <a className="nwh-admin-door" href="/now-what/admin">
              <span className="nwh-admin-label">Practitioner</span>
              <span className="nwh-admin-line">Your field — author, explore, and monitor the environment your clients enter.</span>
              <span className="nwh-admin-verb">Enter your field &rarr;</span>
            </a>
          </div>
        )}
      </div>
      ...
      <style>{`
        ...
        /* The practitioner's door — quiet, at the very bottom, below the
           member environment. Rendered only when the payload grants it. */
        .nwh-admin { margin-top: 36px; display: flex; justify-content: center; }
        .nwh-admin-door {
          display: flex; flex-direction: column; align-items: center; gap: 6px;
          text-decoration: none; text-align: center;
          border-top: 1px solid ${RULE}; padding: 22px 28px 4px; max-width: 34rem;
        }
        .nwh-admin-label {
          font-size: 11px; letter-spacing: 0.4em; text-transform: uppercase;
          color: ${INK_FAINT};
        }
        .nwh-admin-line { font-size: 13px; font-weight: 300; color: ${INK_SOFT}; line-height: 1.6; }
        .nwh-admin-verb { font-size: 13.5px; color: ${BRONZE}; margin-top: 4px; }
        .nwh-admin-door:hover .nwh-admin-verb { text-decoration: underline; }
