...
import { Mic, MicOff, Loader2, CheckCircle, AlertCircle, XCircle, Zap } from 'lucide-react'
...
export function CompactVoiceEngineStatus() {
  const [status, setStatus] = useState<VoiceEngineStatus>({
    engine: 'unknown',
    mode: 'disabled',
    healthy: false,
  ...
  const getStatusColor = () => {
    if (!status.healthy) return 'text-red-400'
    if (status.engine === 'sesame' && status.mode === 'local') return 'text-green-400'
    if (status.engine === 'sesame' && status.mode === 'api') return 'text-blue-400'
    if (status.engine === 'elevenlabs') return 'text-orange-400'
    return 'text-gray-400'
  }

  const getStatusIcon = () => {
    if (!status.healthy) return <XCircle className="h-3 w-3" />
    if (status.engine === 'sesame') return <Zap className="h-3 w-3" />
    if (status.engine === 'elevenlabs') return <AlertCircle className="h-3 w-3" />
    return <MicOff className="h-3 w-3" />
  }

  const getStatusText = () => {
    if (!status.healthy) return 'Voice Offline'
    if (status.engine === 'sesame' && status.mode === 'local') return 'Sesame Local'
    if (status.engine === 'sesame' && status.mode === 'api') return 'Sesame API'
    if (status.engine === 'elevenlabs') return 'ElevenLabs'
    return 'Unknown'
  ...
  return (
    <div className={`flex items-center gap-1 text-xs ${getStatusColor()}`} title={`Voice Status: ${getStatusText()}`}>
      {getStatusIcon()}
