...
# write-tree FIRST, with the hook's environment intact: when git invokes a hook
# it may export GIT_INDEX_FILE, and that is the index whose content is actually
# being committed. Reading the index must honour it.
PROPOSED_TREE="$(git -C "$REPO_ROOT" write-tree 2>/dev/null)" \
  || blocked "could not write the staged tree (git write-tree failed)"

# ⚠️ NOW drop the inherited git environment. `git commit` exports GIT_DIR /
# GIT_WORK_TREE into hooks, and `git worktree add` inherits them and fails
# ("could not create a worktree at HEAD"). This does not reproduce when the
# policy is run directly from a shell, which is exactly why it survived a
# direct-invocation E2E test and only appeared under a real commit.
# JARVIS CORE §B: an instrument is not proven until reconciled with the real
# execution path.
