...
esbuild.buildSync({ entryPoints:[path.join(__dirname,'inline-preview.tsx')], bundle:true, platform:'browser', format:'iife', jsx:'automatic', minify:true, outfile:path.join(out,'preview.js'), define:{'process.env.NODE_ENV':'"production"'}, tsconfig:path.join(root,'tsconfig.json') });
