...
function Preview() {
  const [open,setOpen]=useState(true), [noteOpen,setNoteOpen]=useState(true), [instruction,setInstruction]=useState('');
  ...
  <ManuscriptPassage body={body} annotation={open ? {label:'Image and movement',open:noteOpen,onToggle:()=>setNoteOpen(!noteOpen)} : undefined} range={{start:Array.from(body.split(original)[0]).length,end:Array.from(body.split(original)[0]).length+Array.from(original).length}} proposal={open?preview:null}>
    <div className="ws-insight wsi-inline" hidden={!open || !noteOpen}><header className="wsi-inline-header"><strong>01 · Image and movement</strong><button onClick={()=>setNoteOpen(false)}>Close note</button></header>
    <RevisionDesk inline active={open && noteOpen} onPreview={setPreview} showInspiration={false} manuscriptId="synthetic-preview" title="A quiet room" currentText={original} sectionBody={body} thread={thread as any} version={version} instruction={instruction} onInstruction={setInstruction} onSend={()=>setMessage('Preview only: your question and draft are preserved. No model request was sent.')} onSelectVersion={()=>{}} onApply={()=>setMessage('Preview only: no manuscript was changed.')} onSaveMember={async()=>{setMessage('Preview only: no version was persisted.');return false;}} busy={false} message={message} response={null} onKeep={()=>{setPreview(null);setMessage('Current wording retained.');}} />
    </div>
