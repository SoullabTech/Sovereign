...
#
#   usage: scripts/witness/k00-container-purge.sh <archive-dir> --expect-count N [--dry-run]
#
...
EXPECT=""; DRY=0
while [ $# -gt 0 ]; do case "$1" in --expect-count) EXPECT="$2"; shift 2;; --dry-run) DRY=1; shift;; *) echo "unknown arg $1" >&2; exit 2;; esac; done
...
OUT="$LEDGER/purge-$STAMP$([ $DRY = 1 ] && echo .DRY-RUN).txt"
say(){ echo "$*" | tee -a "$OUT"; }
refuse(){ say "## REFUSED — $*"; say "nothing was written; the device is untouched"; exit 4; }
say "# purge $STAMP $([ $DRY = 1 ] && echo '(DRY RUN — no write)')"
...
LISTING="$(list_raw)" || refuse "preimage 2: the container listing failed three times"
...
JOURNALS="$(grep -oE 'kernel00-[A-Za-z0-9-]+-[0-9]+\.jsonl' <<<"$LISTING" | sort -u)"
ROWS="$( [ -n "$JOURNALS" ] && wc -l <<<"$JOURNALS" | tr -d ' ' || echo 0)"
say "preimage: reported total=$TOTAL · journal rows=$ROWS"
[ "$TOTAL" -eq "$ROWS" ] || refuse "preimage 3/4: reported total $TOTAL ≠ journal rows $ROWS — tmp/ holds something that is not an archived journal"
...
AJ="$(grep -cE 'kernel00-[A-Za-z0-9-]+-[0-9]+\.jsonl' <<<"$AFTER")"
