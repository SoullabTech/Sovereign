...
say "## command (verbatim): python3 scripts/witness/k00-log-window-read.py $W ${J:--} $CAL --expect-sha $EXP"
python3 scripts/witness/k00-log-window-read.py "$W" "${J:--}" "$CAL" --expect-sha "$EXP" | tee -a "$REC"; RC=${PIPESTATUS[0]}
