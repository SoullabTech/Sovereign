...
console.log(`  frame unchanged until writer acts:  ${(await scale())==='SECTIONS 0–2'?'PASS':'FAIL'} ("${await scale()}")`);
if(hasBtn){
  console.log(`  button label: "${await p.locator('#say .invite').innerText()}"`);
  await p.locator('#say .invite').click(); await p.waitForTimeout(200);
  console.log(`  WRITER extends -> frame moves:      ${/SECTIONS 0/.test(await scale())&&(await scale())!=='SECTIONS 0–2'?'PASS':'FAIL'} ("${await scale()}")`);
}
await b.close();
