...
# ⭐ SUCCESSOR-SCHEMA ADAPTATION (W4-S1, carrier §13) — NOT evidence laundering.
# A thread has EXACTLY ONE SUBJECT, so the chain-bound rows below pass
# anchor = NULL; the unbound row S1 keeps its anchor. ⛔ Before W4-S1 anchor
# was NOT NULL, so this fixture HAD to supply one — which is why it wrote a
# two-subject thread, and why W4 found it. ⭐ git preserves the original at
# 303212ae7, and S8 below is strictly stronger than what it replaces.
...
refuses "S5c · ⛔ chain → NULL (the quiet unbinding)" \
  ...
  "immutable"
# ⭐⭐ THE INVERSE OBLIGATION §13 REQUIRES IN THIS SAME ACT. The fixture above
# stopped writing two-subject threads; this proves the DATABASE now refuses
# them. ⛔ Without it the adaptation would only be a fixture that stopped
# doing something — which is not the same as a schema that forbids it.
refuses "S8 · ⛔⛔ a thread carrying BOTH an anchor AND a chain" \
  "$TH ('11110000-0000-4000-8000-00000000000f','aaaaaaaa-0000-4000-8000-00000000000a','$M','{\"on\":\"work\"}','c1','author','$CX');" \
...
# ⭐ NARROWED by W4-S2, law UNCHANGED. The old predicate matched ANY unique
# index mentioning proposal_chain_id, so the FK target UNIQUE (id,
# proposal_chain_id) failed it — although that index constrains NOTHING, id
# being the PK. What the obligation forbids is a unique key that makes a chain
# determine at most one thread: one naming proposal_chain_id and NOT including
# id. ⛔ A unique index leading with proposal_chain_id still fails this.
