...
Usage: k00-ledger.py [--stratum LABEL] [--index N] [--mode I|L] [--w4] [--subject p5b0|phase-a|vpio-01] FILE...
...
STEPS = {'p5b0': P5B0_STEPS, 'phase-a': PHASE_A_STEPS, 'vpio-01': VPIO01_STEPS}
...
SUBJECTS = {k: len(v) for k, v in STEPS.items()}   # gen-1 trace step count per known subject: p5b0 13 · phase-a 14 · vpio-01 11
...
def subject_matches(steps1, refused1, subject):
    ...
    proper prefix of them accompanied by the gen-1 graph_start_refused record (VPIO-01B §4)."""
...
def selftest():
    ...
    for name, rows, subject, want, want_ev in cases:
        ...
        if subject == 'vpio-01' and cls not in ('SUBJECT-MISMATCH',) and any('engineRunning' in json.dumps(r) for r in rows):
            bad = True
        fails += bad
