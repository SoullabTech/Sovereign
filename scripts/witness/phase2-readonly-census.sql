...
\echo '════════════════════════════════════════════════════════════════'
\echo 'CHECKLIST — record reached / not reached, and what the control says, for each path.'
\echo 'Do not change any setting during the walk beyond toggling and restoring; do not send turns'
\echo 'that would form memory unless Sanctuary is confirmed ON.'
\echo
...
\echo '           Record: is the default a per-device value, and does a fresh device inherit it?'
\echo '  [ ] 3.2  Does the account default reach a live session? Start a session with the default ON;'
\echo '           confirm the request carries meta.sanctuary === true (isSanctuary derivation:'
\echo '           app/api/sovereign/app/maia/list/route.ts:398) — observable as the'
\echo '           [Route/MemoryDebug] ... sanctuary=true log line (route.ts:524), NOT from the UI.'
