...
FILES=(
  ...
  "lib/writersStudio/editorialWorkspace.ts"
)
BK="$(mktemp -d)"
for f in "${FILES[@]}"; do
...
for m in "$ROOT"/scripts/witness/w3-mutations/*.py; do
  name="$(basename "$m" .py)"; restore
  if ! python3 "$m"; then echo "  ⛔ STALE  $name"; crashed=$((crashed+1)); continue; fi
  out="$(timeout 600 npx jest app/writers-studio/__tests__/editorialWorkspace.test.ts 2>&1)"; rc=$?
