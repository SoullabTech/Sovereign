...
cmd_status() {
    ...
    echo "candidate              $(wm_get CANDIDATE_SHORT_SHA)  $(wm_get CANDIDATE_SUBJECT)"
    echo "tree at prepare        $(wm_get CANDIDATE_TREE_STATE)"
    echo "compose project        $(wm_get COMPOSE_PROJECT)"
    echo "artifact assertion     '$(wm_get ARTIFACT_PATTERN)' in $(wm_get ARTIFACT_SOURCE_PATH)"
    echo "  discriminating       $(wm_get ARTIFACT_ASSERTION_DISCRIMINATING)"
    echo "migrations             $(wm_get MIGRATIONS)"
    echo "app health             $(wm_get APP_HEALTH)"
    echo "RUNTIME_PROVENANCE     $(wm_get RUNTIME_PROVENANCE)"
    echo "PRODUCTION_ISOLATION   $(wm_get PRODUCTION_ISOLATION)"
    echo "WITNESS_READY          $(wm_get WITNESS_READY)"
    echo "SERVER_EVIDENCE        $(wm_get SERVER_EVIDENCE)"
    echo "CLIENT_CONSOLE_CAPTURE $(wm_get CLIENT_CONSOLE_CAPTURE)"
    echo "EVIDENCE_COMPLETE      $(wm_get EVIDENCE_COMPLETE)"
