...
# ⛔⛔ THE STUB DEPENDENCY IS A DISCLOSED, UNREPAIRED CUSTODY GAP.
#
# Disclosed at `b67eb15e5`: `/tmp/step2_runtime_stubs.sql` is MACHINE-LOCAL and
# is in no commit (`git log --all -- '*step2_runtime_stubs*'` → 0 matches), so
# this witness CANNOT BE REBUILT FROM A FRESH CLONE. ⛔ That repair is held under
# its own separate authorization and is deliberately NOT taken here: absorbing a
# disclosed finding into an unrelated schema act is how findings disappear.
#
# ⭐ The refusal below is therefore the correct behaviour, and "REFUSED · stubs
# not found" is a FIRST-CLASS RESULT — never a skip, and never a pass.
...
for m in 20260901000001_ask_threads \
         ...
         20260915000001_ask_threads_subject_preparation \
         20260915000002_editorial_turn_bindings; do
