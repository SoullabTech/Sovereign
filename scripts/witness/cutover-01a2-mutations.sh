...
FILES=(
  "lib/writersStudio/writeStateClient.ts"
...
for m in "$ROOT"/scripts/witness/cutover-01a2-mutations/*.py; do
  name="$(basename "$m" .py)"
  restore
  if ! python3 "$m"; then echo "  CRASHED  $name (transform did not apply)"; crashed=$((crashed+1)); continue; fi
  out="$(timeout 300 npx tsx scripts/witness/cutover-01a2-witness.ts 2>&1)"; rc=$?
  if [ "$rc" -eq 0 ]; then
