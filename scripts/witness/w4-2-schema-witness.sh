...
# DATABASE's behaviour, over the applied pair W4-S1 + W4-S2.
#
# ⭐ INHERITED EVIDENCE. These obligations were authored and falsified against
# the superseded one-file prototype and transfer unchanged, because every one is
# a statement about SEMANTICS and none depends on the packaging. ⛔ What did NOT
# transfer unexamined — rollback ordering, and the placement of the W5-3 inverse
# obligation — was re-derived rather than copied.
#
...
# ⭐⭐ THE ASSERTION THAT PROVES W4-S2's PART A RAN. A constraint left NOT VALID
# is enforced for new rows and silently unenforced over the existing ones, so
# X3/X4 would pass while the invariant was never established over the table.
# ⭐ In the two-file shape this is sharper than it was in the prototype: the
# W4-S1 phase witness proves these same CHECKs are PRESENT and NOT validated
# after S1 alone, so this obligation isolates exactly what the SECOND ledger act
# contributes — ⛔ it can no longer be satisfied by the file that installed them.
