#!/usr/bin/env node
/**
 * R1-1C strict typecheck — strict + noUncheckedIndexedAccess + react-jsx over the Review navigation
 * succession seam, the flagship shell and Review presentation files it lives in, the frozen witness fixtures it renders,
 * and the R1-1C suite. Inherited allowances are BLOB-PINNED to the R1-1C base: an allowance that
 * moves is a regression, not an allowance.
 */
import { execFileSync, spawnSync } from 'node:child_process';

const BASE = 'e5a89f2a4bec408a4d0c04eef2df59a4ccf19507';
const CONFIG = 'tsconfig.ws-flagship-r1-1c.json';
const ALLOWED = [
  ...
  /* The live host's own proven neighbours (the C1B allowance set), blob-pinned to the R1-1C base. */
...
console.log(`R1-1C TYPECHECK — ${CONFIG} (strict · noUncheckedIndexedAccess · react-jsx)\n`);
