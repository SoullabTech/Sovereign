...
export function buildNativePrompt(packet, repo) {
  ...
  return [
    ...
    "   Before emitting, verify the TARGET FILE will be syntactically self-contained after every edit.",
    "   Every identifier introduced into the target must already be in that target file's scope after the edits; an import visible only in a precedent file does NOT put it in scope.",
    "   If an introduced identifier requires an import shown by a precedent, include the target-file import as a separate exact edit in the same EDIT_SCRIPT.",
