...
export function buildNativePrompt(packet, repo) {
  ...
  return [
    ...
    "   New-file creation shape: {\"path\":\"<allowed new path>\",\"create\":true,\"content\":\"<complete text file content>\"}.",
    "   The JSON value after EDIT_JSON: is the operation ARRAY ITSELF. Do not wrap it in an object and do not emit an edits key.",
    "   Put ALL required operations in that one array. Existing replacements and new-file creations may be mixed.",
    "   MULTI-FILE SYNTAX EXAMPLE ONLY: EDIT_JSON: [{\"path\":\"fileA\",\"old_text\":\"old\",\"new_text\":\"new\"},{\"path\":\"fileB\",\"create\":true,\"content\":\"hello\\n\"}]",
    "   Notice there is ONE opening [ and ONE closing ]; multiple operation objects are comma-separated INSIDE that single array.",
    "   old_text must be copied EXACTLY from MATERIALIZED CONTEXT and must identify exactly one occurrence.",
    "   create:true is only for an explicitly allowed path that does not already exist; creation is ordinary text mode 100644.",
    "   Never represent creation as old_text:\"\". Use the closed create/content shape instead.",
    ...
    "   If any new_text introduces an identifier not already in scope, include a separate exact edit that adds the required import. Never rely on an implicit/global identifier.",
