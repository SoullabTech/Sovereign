...
export function buildNativePrompt(packet, repo) {
  ...
  return [
    ...
    "A) EDIT_JSON: emit exactly one physical line whose FIRST BYTES are exactly `EDIT_JSON: ` followed by one JSON object.",
    "   Do NOT echo `A)`, `A) EDIT_JSON:`, the option label, analysis, or any text before `EDIT_JSON: `.",
    "   Existing-file replacement shape: {\"path\":\"<allowed existing path>\",\"old_text\":\"<exact existing text>\",\"new_text\":\"<replacement text>\"}.",
    "   New-file creation shape: {\"path\":\"<explicitly allowed new path>\",\"new_text\":\"<complete non-empty file contents>\"}. Omit old_text ONLY for creation.",
    "   The JSON value after EDIT_JSON: is the edit ARRAY ITSELF. Do not wrap it in an object and do not emit an edits key.",
    "   Put ALL required objects in that one array. One candidate may replace text in multiple existing files and may also create new text files.",
    "   MULTI-FILE SYNTAX EXAMPLE ONLY: EDIT_JSON: [{\"path\":\"fileA\",\"old_text\":\"oldA\",\"new_text\":\"newA\"},{\"path\":\"fileB\",\"old_text\":\"oldB\",\"new_text\":\"newB\"}]",
    "   MIXED SYNTAX EXAMPLE ONLY: EDIT_JSON: [{\"path\":\"fileA\",\"old_text\":\"oldA\",\"new_text\":\"newA\"},{\"path\":\"new.txt\",\"new_text\":\"complete contents\"}]",
    "   Notice there is ONE opening [ and ONE closing ]; objects are comma-separated INSIDE that single array.",
    "   For replacement, old_text must be copied EXACTLY from MATERIALIZED CONTEXT and must identify exactly one occurrence.",
    "   For creation, the path must appear LITERALLY in ALLOWED FILES; wildcard authorization is insufficient.",
    "   Creation is text-only, fixed mode 100644, requires a non-empty new_text, an existing non-symlink parent directory, and a path that does not already exist or track a file.",
    "   A created path may appear only once in the array. Put its FINAL complete contents in that one creation object.",
    ...
    "   Before emitting, silently verify every identifier you add to an existing file is already in scope or is imported from the precedent shown in MATERIALIZED CONTEXT.",
    "   If replacement new_text introduces an identifier not already in scope, include a separate exact replacement that adds the required import. Never rely on an implicit/global identifier.",
    "   New-file contents must be grounded in the OBJECTIVE, ESTABLISHED FACTS, and materialized precedent. If the required content is not grounded, emit GOVERNANCE_GATE instead of inventing it.",
