...
export function buildNativePrompt(packet, repo) {
  ...
  return [
    ...
    "   BYTE-LEVEL RULE: when choosing PATCH, the first bytes of the response MUST be exactly: diff --git followed by one space.",
    ...
    "   HUNK INTEGRITY: each @@ header old/new line count must exactly match the hunk body so git apply --check accepts the patch.",
