...
import { validateWorkUnitV1 } from './work-unit-v1.mjs';

export const LIFECYCLE_VERSION = 'W2.v1';
...
export function authorizedCoreViewV1(workUnit) {
  return deepFreeze(canonicalAuthorizedCore(workUnit));
}

export function authorizedCoreSnapshotV1(workUnit) {
...
function envelopeBlockers(envelope) {
  ...
  blocks.push(...validateWorkUnitV1(workUnit));
  ...
  if (['DRAFT', 'BOUNDED'].includes(state)) {
    ...
    } else if (snapshot !== authorizedCoreSnapshotV1(workUnit)) {
      blocks.push(blocker(
...
export function createLifecycleEnvelopeV1(workUnit) {
  const schemaBlocks = validateWorkUnitV1(workUnit);
...
export function transitionLifecycleV1(envelope, request) {
  ...
  if (request.to === 'AUTHORIZED') {
    ...
    nextGuard.authorized_core_snapshot = authorizedCoreSnapshotV1(nextWorkUnit);
  }
