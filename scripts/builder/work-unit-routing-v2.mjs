/**
 * JARVIS Work Unit Routing Binding V2 — J5 family-first pure binding.
 *
 * W3 boundary:
...
function canonicalPosture(workUnit) {
  const posture = workUnit?.routing_request?.requested_posture ?? 'default';
...
export function deriveRoutingInputV2(workUnit) {
  if (!isObject(workUnit)) {
    return deepFreeze({
      ok: false,
      input: null,
      blockers: [blocker('WORK_UNIT_REQUIRED', 'A canonical W0.v2 Work Unit is required.')],
    });
  }
  ...
  if (workUnit.work_unit_version !== 'W0.v2') {
    return deepFreeze({
      ...
      blockers: [blocker(
        ...
        'W3.v2 binds only W0.v2 Work Units.',
        'work_unit_version',
      )],
    });
  }
  ...
  if (!posture) {
    return deepFreeze({
      ...
      blockers: [blocker(
        ...
        'Work Unit routing_request.requested_posture is not recognized by W3.v2.',
        'routing_request.requested_posture',
      )],
    });
  }
  ...
  const capability = workUnit.identity?.capability ?? null;

  const input = {
    deterministic: {
      capability,
      registered: typeof capability === 'string'
        && capability.length > 0
        && Object.prototype.hasOwnProperty.call(CAPABILITIES, capability),
    },
    evidence_class: workUnit.custody?.evidence_class,
    task_shape: workUnit.identity?.task_shape,
    review_pressure: workUnit.routing_request?.review_pressure,
    ...
    authority: {
      ...
      // W3.v2 preserves the canonical read-only routing membrane even if the
    ...
    work_unit: {
      risk_class: workUnit.identity?.work_class === 'VERIFICATION' ? 'high' : 'mechanical',
      explicit_independent_review: true,
    },
  };

  const blocks = [
    ...requiredDisclosureBlockers(workUnit, posture),
  ];
...
export function routeAuthoritySubsetV2(workUnit, route) {
  const checks = [];
  const missing = [];
...
function preBindBlockers(envelope) {
  ...
  if (!nonBlank(envelope.guard.authorized_core_snapshot)) {
    ...
    !== authorizedCoreSnapshotV2(workUnit)) {
    blocks.push(blocker(
...
function routerBlockers(route) {
  ...
  if (route.deterministic?.selected !== true
    && route.primary == null
    && (!Array.isArray(route.challengers) || route.challengers.length === 0)) {
    blocks.push(blocker(
      'ROUTE_PARTICIPANT_REQUIRED',
      'A non-deterministic J5 route must contain at least one route participant.',
      'primary',
    ));
  }
...
export function bindAuthorizedRouteV2(envelope) {
  ...
  const workUnit = envelope.work_unit;
  const derived = deriveRoutingInputV2(workUnit);
  ...
  const authorityProof = routeAuthoritySubsetV2(workUnit, route);
  ...
  bound.work_unit.routing.route_source = ROUTE_SOURCE_V2;
  ...
  const transitioned = transitionLifecycleV2(bound, {
    to: 'ROUTED',
