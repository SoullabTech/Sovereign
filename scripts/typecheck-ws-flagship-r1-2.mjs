#!/usr/bin/env node
/**
 * R1-2 strict typecheck — strict + noUncheckedIndexedAccess + react-jsx over the Review return-navigation
 * succession seam, the flagship shell and Review presentation files it lives in, the frozen witness fixtures it renders,
 * and the R1-2 suite. Inherited allowances are BLOB-PINNED to the R1-2 base: an allowance that
 * moves is a regression, not an allowance.
 */
import { execFileSync, spawnSync } from 'node:child_process';

const BASE = 'e9f7ffcb51c64702ce0e4c2719b46ff4663e5594';
const CONFIG = 'tsconfig.ws-flagship-r1-2.json';
const ALLOWED = [
  ...
  /* The live host's own proven neighbours (the C1B allowance set), blob-pinned to the R1-2 base. */
...
console.log(`R1-2 TYPECHECK — ${CONFIG} (strict · noUncheckedIndexedAccess · react-jsx)\n`);
