...
const systems:Record<Condition,string>={
 A_NARRATIVE:`Offline ecological-validity research. Summarize the conversation-so-far in no more than 220 words as one compact narrative process representation. Preserve member-authored meanings, established symbols, corrections, unresolved tensions, open loops, and repeated interaction patterns when evidenced. Do not invent hidden meaning or future outcomes.`,
 B_PAIRWISE:`Offline ecological-validity research. Return JSON only with {nodes:[{id,label,standing}],relations:[{source,predicate,target,evidenceTurns}]}. Use pairwise relations only. Preserve exact member meanings and corrections. Allowed predicates: REFERS_TO,CORRECTS,CONFIRMS,RETURNS_TO,ADOPTED_AS,EVOKES,INHIBITS,ALIGNMENT,TENSION,GROUNDS,SUPPORTS,REOPENS. Do not encode higher-order motifs or temporal trajectories. Keep under 220 words.`,
 C_RELATION_GEOMETRY:`Offline ecological-validity research. Return JSON only with {objects:[...],relations:[...]}. Each relation may independently contain dynamic:{predicate,direction,source,target,evidenceTurns}|null, configuration:{predicate,endpoints,evidenceTurns}|null, standing, provenance, processScope. Preserve corrections and established meanings. Do not encode higher-order motifs or temporal trajectories. Keep under 220 words.`,
 D_GEOMETRY_TEMPORAL:`Offline ecological-validity research. Return JSON only with {objects:[...],relations:[...],motifs:[...],temporal:[...],openLoops:[...]}. Preserve multidimensional relation geometry, standing/provenance, repeated/reopened patterns, higher-order configuration only when evidenced, and temporal operators EMERGES,DISSOLVES,TRANSFORMS,RETURNS_TO,PERSISTS,REOPENS. Do not predict future outcomes. Keep under 220 words.`,
};
...
async function main(){
 ...
 for(const e of events){
  ...
  const reps=await Promise.all(CONDITIONS.map(async condition=>({condition,rep:await callClaude(systems[condition],`CONVERSATION SO FAR:\n${beforeText}`)})));
  const decoded=await Promise.all(reps.map(async x=>({...x,dec:await callClaude(decodeSystem,`REPRESENTATION:\n${x.rep}\n\nFEATURES:\n${JSON.stringify(target)}`)})));
  for(const {condition,rep,dec} of decoded){
   ...
   rows.push({event:e.id,condition,sessionHash,cutoff:e.cutoff,outcomeTurns:e.outcomeTurns,sourceHash,outcomeHash,representationHash:sha(rep),decoderHash:sha(dec),representationWords:words(rep),budgetViolation:words(rep)>220,parseError,features:featureRows,featureCorrect:featureRows.filter(x=>x.supported).length,featureTotal:featureRows.length});
 ...
 const byCondition=Object.fromEntries(CONDITIONS.map(c=>{
  ...
  return [c,{events:rs.length,featureRecall:correct/total,featureCorrect:correct,featureTotal:total,eventPerfect:rs.filter(r=>r.featureCorrect===r.featureTotal).length,budgetViolations:rs.filter(r=>r.budgetViolation).length,meanWords:rs.reduce((n,r)=>n+r.representationWords,0)/rs.length}];
 }));
 ...
 const evidence={schema:'RELATIONAL_GEOMETRY_H7A_ECOLOGICAL_PROCESS_VALIDITY_V1',authority:'offline research only; raw production conversation bytes remain /private/tmp and are not committed',inference:{provider:'anthropic',model:MODEL,path:'local Claude CLI restricted/no-tools',effort:'low'},design:'Later member behavior selects which t1-grounded precursor structures mattered; representations are built from t1 only. Blind decoder sees representation + precursor descriptions, never t2 raw outcome.',events:events.map(e=>({id:e.id,cutoff:e.cutoff,outcomeTurns:e.outcomeTurns,featureIds:e.features.map(f=>f.id)})),byCondition,byEvent,rows};
 const out=path.join(process.cwd(),'docs/programme/evidence/relational-geometry-integration/H7A_ECOLOGICAL_PROCESS_VALIDITY_2026-09-16.json');
 fs.writeFileSync(out,JSON.stringify(evidence,null,2)+'\n');
 fs.writeFileSync('/private/tmp/h7a-ecological-process-validity-raw.json',JSON.stringify(raw,null,2)+'\n');
