...
{
  "contextEvidenceIds": ["..."],
  "synthesis": [{"text":"...", "supportEvidenceIds":["..."]}],
  "question":"..."
}
...
- contextEvidenceIds contains only optional historical ids E-OLD / M-OLD.
- synthesis is MAIA's new perception, written directly to the person using you/your or neutral nouns. Never use I/me/my/mine/myself and never say member/the member.
...
- At most 2 context refs, 1 synthesis, 1 question.
`;

const PLAN_SCHEMA = {
  type: 'object', additionalProperties: false, required: ['contextEvidenceIds', 'synthesis', 'question'],
  properties: {
    contextEvidenceIds: { type: 'array', minItems: 0, maxItems: 2, items: { type: 'string', enum: ['E-OLD', 'M-OLD'] } },
    synthesis: {
      type: 'array', minItems: 1, maxItems: 1,
      items: { type: 'object', additionalProperties: false, required: ['text', 'supportEvidenceIds'], properties: {
        text: { type: 'string' },
        supportEvidenceIds: { type: 'array', minItems: 1, maxItems: 3, items: { type: 'string', enum: ['E-OLD', 'M-OLD', 'E-NOW'] } },
      } },
    },
...
async function main(): Promise<void> {
  ...
  for (const seed of SEEDS) {
    ...
    const parsed = JSON.parse(raw.response) as StandingBoundPlan;
    try {
      const plan = bindCurrentStanding(parsed, CLAIM_KEY, standing);
      const rendered = renderStandingEnvelope(evidence, plan, standing);
      rows.push({ seed, status: 'rendered', rawPlan: parsed, rawPlanSha256: sha(raw.response),
        effectiveGround: plan.ground.map((x) => x.evidenceId),
        currentGrounded: plan.ground.some((x) => x.evidenceId === 'E-NOW'),
        currentInSynthesisSupport: plan.synthesis.every((x) => (x.supportEvidenceIds ?? []).includes('E-NOW')),
        rendered, usage: { promptEvalCount: raw.prompt_eval_count ?? null, evalCount: raw.eval_count ?? null } });
    } catch (error) {
  ...
  console.log(JSON.stringify({
    programme: 'FREE-SYNTHESIS-STRUCTURAL-STANDING-01', act: 'ACT 4 minimal standing-bound interface replay',
    rawReferenceArtifact: 'docs/programme/evidence/FREE_SYNTHESIS_STRUCTURAL_STANDING_ACT1_S4_2026-09-16.json',
    model: MODEL, temperature: TEMPERATURE, promptChars: prompt.length, promptSha256: sha(prompt), rows,
  }, null, 2));
}
