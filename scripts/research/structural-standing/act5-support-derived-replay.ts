...
const PLAN_SCHEMA = {
  ...
  properties: {
    synthesis: { type: 'array', minItems: 1, maxItems: 1, items: { type: 'object', additionalProperties: false, required: ['text', 'supportEvidenceIds'], properties: {
      text: { type: 'string' },
...
async function main(): Promise<void> {
  ...
  for (const seed of SEEDS) {
    ...
    const parsed = JSON.parse(raw.response) as SupportDerivedPlan;
    try {
      const plan = deriveSupportGround(parsed, CLAIM_KEY, standing);
      const rendered = renderStandingEnvelope(evidence, plan, standing);
      rows.push({ seed, status: 'rendered', rawPlan: parsed, rawPlanSha256: sha(raw.response), effectiveGround: plan.ground.map((x) => x.evidenceId),
  ...
  console.log(JSON.stringify({ programme: 'FREE-SYNTHESIS-STRUCTURAL-STANDING-01', act: 'ACT 5 support-derived-ground replay', model: MODEL,
