#!/usr/bin/env node
/**
 * R1-1B strict typecheck — strict + noUncheckedIndexedAccess + react-jsx over the read-only Review
 * seam, the flagship Review presentation files it lives in, the frozen witness fixtures it renders,
 * and the R1-1B suite. Inherited allowances are BLOB-PINNED to the R1-1B base: an allowance that
 * moves is a regression, not an allowance.
 */
import { execFileSync, spawnSync } from 'node:child_process';

const BASE = 'b867abebacbd15d3b19979fb6fc21372af26919e';
const CONFIG = 'tsconfig.ws-flagship-r1-1b.json';
const ALLOWED = [
...
console.log(`R1-1B TYPECHECK — ${CONFIG} (strict · noUncheckedIndexedAccess · react-jsx)\n`);
