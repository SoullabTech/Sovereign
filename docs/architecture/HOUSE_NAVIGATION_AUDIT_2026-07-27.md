...
> ## ⚠️ SUPERSESSION NOTE — added 2026-07-29 (Kelly ruling)
>
> **This document is preserved for provenance. Its conclusions no longer describe trunk.
> Do not cite it as current state.**
>
> **Accurate when written.** The audit's findings were correct against the tree it
> inspected. Nothing below is being retracted as mistaken.
>
> **What superseded it.** Commit `a787ec736` — *"feat(house): navigation contract — one
> destination model, typed dispatch, drift guard"* — plus eight follow-ups, all present on
> `clean-main-no-secrets` at `acb757f87`. It introduced `lib/navigation/houseDestinations.ts`
> as the single authoritative model for every place the House can open, with typed dispatch
> (`route` / `sheet` / `external`) replacing the uniform `router.push`.
>
> **Specifically no longer true:**
>
> | Claim below | Status on `acb757f87` |
> |---|---|
> | *"`MaiaHouseSheet` calls `router.push(route)` for everything with no native/`open-web` awareness"* | **False.** The sheet dispatches through `classifyReachability` / `dispatchHouseDestination` with an `isNative` context. |
> | *"The `/open-web` bridge exists but the House never uses it"* | **False.** Web-only destinations route through the bridge and render a `web ↗` mark. |
> | *"of 16 House destinations, only Journal and Settings … actually work"* | **Superseded.** Destinations not yet in the native bundle are now *withheld* on native rather than rendered as dead buttons. |
> | The three items under **Decisions needed before code** | **All ruled.** `houseDestinations.ts` cites *"Policy (founder ruling 2026-07-27)"* — bundle-vs-web decided per destination, Decisions removed from member grammar with Changes kept as a member-owned sheet, delivery split PR 1 / PR 2. |
>
> **Still true.** `MOBILE_MAIA_KEEP=()` remains empty — every `/maia/*` sub-route is still
> stripped from the iOS bundle. That is the PR 2 deliverable, tracked in the model as
> `nativePolicy: 'native', nativeReady: false`.
>
> **Preservation is not endorsement.** This note records supersession only. It does not
> reopen the House decisions ruled on 2026-07-27, and the audit's remediation plan
> ("What 'complete the House' means") should not be executed from this document.
