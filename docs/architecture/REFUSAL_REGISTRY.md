...
**Harness — current state (2026-07-02):** **19 passed · 0 failed · 0 warned (6 refusals: R01–R06).** R06 (Book Studio member-authored-only) landed its **A-half** (reader-absence) green while the mirror-safe surface is still unbuilt — a ratchet: the "no synthesis path in Book Studio" property held by absence, so it is now structurally *held*, and a future reader that pipes inferred themes into Book Studio must add a visible-diff import/read to defeat it. R06's **B-half** (member-act predicate per query in `lib/bookStudio/mirrorSources`) activates automatically when that module is built. *Note: R05 is in the harness but not yet given its own table row above — a documentation gap, not a coverage gap.*
