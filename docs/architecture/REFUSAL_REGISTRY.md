...
| **TTS provider qualification — production may not *select* a non-sovereign provider (Voice Lab switch ≠ production egress bypass)** | B | `assertProviderQualified(provider)` invoked in `synthesize()` before any dispatch; `QUALIFIED_PROVIDERS['production-maia']` is local-only (`auto,kokoro,sesame`) — excludes `openai` (cloud egress) and `pplex` (MLX, no x86 production backend); `getDeploymentContext()` **fails closed** to `production-maia`. Capability (the `pplex` dispatch branch + adapter) and gate ship together — the branch is reachable only past the guard | `lib/tts/ttsRouter.ts` (`assertProviderQualified`, `QUALIFIED_PROVIDERS`, `getDeploymentContext`, `pplex` dispatch); adapter `lib/tts/providers/personaplex.ts`. The archetype→OpenAI default (feminine voices) is a **separate open governance question**, tracked at `docs/adr/012-openai-tts-production-status.md` — this refusal does not resolve it | **Demonstrated (R15)** ✅ | Behavioural test: set `MAIA_TTS_PROVIDER=pplex` (or `openai`) with no `voice-quality-lab` context → assert `ProviderNotQualifiedError`; widen the production allow-list only via an ADR-referenced decision | Add `openai`/`pplex` to `QUALIFIED_PROVIDERS['production-maia']`, delete the `assertProviderQualified(provider)` call in `synthesize()`, or make `getDeploymentContext()` default to `voice-quality-lab` — all visible diffs |
