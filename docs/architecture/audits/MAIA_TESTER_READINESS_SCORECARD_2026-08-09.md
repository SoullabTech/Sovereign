...
## Post-Gate-1 update (2026-08-09, branch `feature/gate1-persistent-corrigibility` @ `dac99ebf1`)

Per the program's per-gate scoring requirement. Only dimensions Gate 1 touches are restated; scores remain honest to the closure standard (PROVEN requires production runtime evidence — implementation alone cannot reach it).

| # | Dimension | Was | Now | Basis |
|---|---|---|---|---|
| 7 | Corrigibility | BROKEN | **PARTIAL (implemented, not deployed)** | member_corrections + turns supersession + read-path enforcement + ledger authority model; 30 invariant tests green; 4 critical mutations caught; awaits merge → deploy → production-equivalent encounter for PROVEN |
| 10 | Authority integrity | BROKEN | **PARTIAL (implemented, not deployed)** | recurrence→authority structurally refused (DB CHECK + weight-0 promotion + routing filter); confirm/qualify member acts; laundering sites in CaseConsultation/MemoryBundle remain (Gate 2) |
| 5 | Temporal reasoning | ABSENT | **PARTIAL (narrow)** | supersession introduces "no longer current" as a first-class state on the highest-traffic substrate; general temporal architecture remains Gate 5 |

Unchanged this gate (correctly): semantic recall (4), salience (6), vault access (11), wisdom relevance (12), return experience (16) — Gates 2–6 territory. **Gate 1 closure chain remaining: merge → deploy (full lane, schema migration) → production-equivalent correction encounter → later-session recall verification.**
