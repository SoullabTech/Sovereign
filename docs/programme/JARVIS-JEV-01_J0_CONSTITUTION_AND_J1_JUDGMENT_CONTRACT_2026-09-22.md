...
⭐ **REPAIRED, as a separate act** (founder direction): the parent charter's `Current
state:` field is corrected and carries a **Status correction** section recording that this
document's false finding was generated by it. Superseded text preserved, ⛔ not deleted.

⚠️ **And the pathology is not unique to the charter.** Two adjacent records carry status
lines their own merge history contradicts — `J6_WORK_UNIT_I4_WITNESS` (*"not PR/merge/
deploy/provider-execution authorized"*, merged at `22a5f352`) and `R5B` (*"AWAITING FOUNDER
ADJUDICATION"*, merged at `f0063747`). ⛔ **Neither is asserted to be an unauthorized
merge** — the likelier reading is a founder act whose record lagged. ⚠️ But that cannot be
determined from the repository, which is the finding. ⛔ Observed, routed out, not
repaired; no lane opened.
