...
`#1239` is historical and closed. Future public-ledger publication runs open a new bounded PR only when an upstream claim-changing event triggers §11.

**Containment (founder ruling 2026-09-06, split record §7a).** Intended transfer point `c36d82ec`; founder-named containment `6ce59f82`; **final containment `a7b42f29`** (split record §7b — the R&D session acknowledged, switched to `claude/maia-human-experience-phase1-census`, and that branch's merge-base with #1239 is `a7b42f29` exactly; operational transfer CLOSED ~17:58Z). Every R&D commit up to that bound is PRESERVED — never rewritten, never reverted — and carried as R&D-owned historical content. From that containment point until #1239 merged, **this lane was the only writer on the PR branch**; further R&D writes were not authorized. The PR title/body were corrected to publication custody before merge.
...
| 2026-09-06 ~19:00Z | **CLOSED / ACCEPTED — publication run complete; page LIVE.** Closure condition (founder): deploy exact `ca5fdff4` → verify production runs it → verify the five-part page live → record. Evidence, each to its own instrument: (1) merge — canonical `ca5fdff4` (founder act 18:33Z at head `f61a1f2c`); (2) deploy — governed gate, immutable snapshot, container created 18:44:01Z; (3) container provenance — `GIT_COMMIT=ca5fdff44` on printenv and Config.Env; (4) **public-path provenance** — `https://soullab.life/api/health` → `"version":"ca5fdff44"` from the Mac Studio; (5) **public-path render** — `https://soullab.life/accounted-for` payload carries `"I"," · ","If"` · `"II"," · ","Why"` · `"III"," · ","How"` · `"IV"," · ","What"` · `"V"," · ","Who"` once each and `Why Soul Lab` (founder-run `curl`, 2026-09-06). The page may now be called **LIVE**. Note on the render witness: plain-text greps for `I · If` fail because the served HTML contains **no server-rendered markup** (zero `<p>`, no `<h1>`); the page ships as an RSC payload and renders on the client, so the part-marker line is serialized as three strings. The witness therefore matched the payload form. **Upstream observation, not this lane's and not a claim defect:** a reader without JavaScript, a text-only fetch, or a non-executing crawler receives an empty accounting; this is a property of the root layout's provider chain wrapping every page, routed to the owning lane as a note. Lane state after closure: the publication run is CLOSED / ACCEPTED; the lane **remains open as the public ledger lane** (§11) and reopens only on an upstream event that affects a public claim. |
| 2026-09-06 18:44Z | **DEPLOYED (founder act) — provenance witnessed; page render witness pending founder eyes.** Founder ran the governed quick path from the Mac Studio: `scripts/pre-deploy-gate.sh deploy-maia ca5fdff44` — deploy-lane lock acquired; immutable snapshot of `ca5fdff44` (subject: merge of PR #1239); compose carries `GIT_COMMIT` as build arg only; disk gate 396 GB free; Co-Lab boundaries 33 passed · 0 failed · 0 warned (floor 31; the script is `scripts/verify-constitution-colab.ts` — the `verify-colab-boundaries.ts` name in CLAUDE.md does not exist, known doc defect); build OK (`/accounted-for` prerendered static); built-image provenance `GIT_COMMIT=ca5fdff44` == asserted; rollback tags refreshed (`:previous`, `:current`, `:ca5fdff44`); container swapped; running provenance `ca5fdff44` (printenv) == `ca5fdff44` (Config.Env) == asserted. Post-swap: `printenv GIT_COMMIT` → `ca5fdff44`; `Created` → `2026-09-06T18:44:01.970767639Z`. Previous runtime was `50302f5d9` (created 14:05:07Z), which served the pre-manifesto page — the reason the founder saw no edits before this deploy. **Remaining witness before the page may be called LIVE:** a founder-visible open of `https://soullab.life/accounted-for` showing the five part-markers (I · If · Fire … V · Who · Aether) and the "Why Soul Lab" section; this session cannot reach production and does not assert it. Build-log notes, not this lane's: pre-existing `openai` dependency warning (already stated on the page as a guarded TTS fallback); prerender-time `ECONNREFUSED 127.0.0.1:5432` from `/api/ain/telemetry` static generation (non-blocking, pre-existing). |
| 2026-09-06 18:33Z | **MERGED (founder act).** Founder marked PR #1239 ready for review and merged it at the pinned head `f61a1f2c` → canonical **`ca5fdff4`** ("Merge pull request #1239"). All 8 GitHub checks green on `f61a1f2c`; no review threads; publication report posted as a PR comment. Merge is not deploy: the page was not live at this point. |
