...
config() (
  ...
  psql "$DB" -qtAX -c "SELECT 'LS0_RUN_BEGIN_$L'" >/dev/null
  ...
  sleep 2
  psql "$DB" -qtAX -c "SELECT 'LS0_RUN_END_$L'" >/dev/null
  ...
  RUNLOG=/tmp/ls0-e1-pg/run-ls1-$L.statements.log
  awk "/LS0_RUN_BEGIN_$L/{f=1} f{print} /LS0_RUN_END_$L/{f=0}" "$PGLOG" > "$RUNLOG"
