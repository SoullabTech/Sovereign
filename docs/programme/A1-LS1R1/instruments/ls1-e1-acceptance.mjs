...
// A1-LS1R1 · adds the controlled-lock A3 check `comparisonUnderLockRefusesChangedBody` (packet 38cf93d4 §3).
//
...
async function context(browser) {
  ...
  await ctx.addInitScript(() => {
    // Fault injection for the digest-ordering check only (A1-LS1R1: no clock).
    // When the witness installs a hold, every page-side SHA-256 computation is
    // held until the WITNESS releases it — not for a fixed time — so the order
    // "acknowledgements first, stale digests last" is established by the
    // witness, never by how fast the machine happens to be.
    window.__ls1DigestHold = null;
    window.__ls1HeldDigests = 0;
    window.__ls1LateDigests = 0;
    try {
      const origDigest = SubtleCrypto.prototype.digest;
      SubtleCrypto.prototype.digest = async function (...a) {
        const hold = window.__ls1DigestHold;
        if (hold) window.__ls1HeldDigests += 1;
        const r = await origDigest.apply(this, a);
        if (hold) { await hold; window.__ls1LateDigests += 1; }
        return r;
      };
...
await law('A3', async () => {
  ...
  // (7b) A1-LS1R1 · CONTROLLED-LOCK witness: the observed-body comparison is made UNDER the draft row lock.
  // (7) above is a stress check that can only expose an outside-the-lock comparison when the saves'
  // reads happen to interleave. This check forces the interleaving instead of hoping for it:
  //   the witness takes the draft row lock on its own connection → a stale-version save carrying the
  //   digest of the section's CURRENT body is started through the real route → the witness establishes
  //   that the save is WAITING ON A LOCK (else INSTRUMENT_FAILURE) → the competing writer's change to that
  //   same section is applied inside the lock-holding transaction and committed → the save completes.
  // Under the lock the save must see the competing body and refuse (409), leaving that body intact.
  const lockRounds = [];
  {
    const holder = new pg.Client({ connectionString: 'postgresql://soullab@127.0.0.1:55432/maia_consciousness' });
    await holder.connect();
    try {
      for (let r = 0; r < 3; r++) {
        const snapL = await api(`/api/writers-studio/rebuild/context?manuscriptId=${m.mid}`);
        const vL = Number(snapL.json.version); const bodyL = snapL.json.sections.find((x) => x.draftSectionId === D).body;
        const mk = MARK(`lock${r}`); const competing = ` LS1MARKcompeting${r}${crypto.randomBytes(3).toString('hex')}`;
        await holder.query('BEGIN');
        const draftRow = (await holder.query('SELECT id FROM manuscript_working_drafts WHERE manuscript_id = $1 FOR UPDATE', [m.mid])).rows[0];
        const pending = put(D, `${bodyL} ${mk}`, vL - 1, sha(bodyL));
        let waitingOnLock = false;
        for (let i = 0; i < 300 && !waitingOnLock; i++) {
          const w = (await q(`SELECT count(*)::int AS n FROM pg_stat_activity WHERE datname = 'maia_consciousness' AND wait_event_type = 'Lock' AND pid <> $1`, [holder.processID]))[0].n;
          waitingOnLock = w > 0; if (!waitingOnLock) await sleep(50);
        }
        if (!waitingOnLock) { await holder.query('ROLLBACK'); await pending; throw new Error(`controlled-lock precondition not reached: save not observed waiting on the draft lock (round ${r})`); }
        await holder.query('UPDATE manuscript_draft_sections SET text = text || $2, updated_at = now() WHERE id = $1', [D, competing]);
        await holder.query(`UPDATE manuscript_working_drafts d SET version = version + 1, content = (SELECT string_agg(text, '' ORDER BY position) FROM manuscript_draft_sections WHERE draft_id = d.id) WHERE id = $1`, [draftRow.id]);
        const competingDigest = sha((await holder.query('SELECT text FROM manuscript_draft_sections WHERE id = $1', [D])).rows[0].text);
        await holder.query('COMMIT');
        const res = await pending;
        const after = (await q('SELECT text FROM manuscript_draft_sections WHERE id = $1', [D]))[0].text;
        lockRounds.push({ waitingOnLock, status: res.status, competingStatePreserved: sha(after) === competingDigest, saveMarkPersisted: after.includes(mk), competingDigest: competingDigest.slice(0, 16), persistedDigest: sha(after).slice(0, 16) });
      }
  ...
  // Every page-side SHA-256 (the mount-time digests of the delivered bodies) is HELD until the witness
  // releases it, so two acknowledged saves of B land FIRST and the stale digest of B's delivered body
  // resolves LAST — by construction, whatever the machine's speed.
  const c3 = await context(browser);
  await c3.addInitScript(() => { let release; window.__ls1DigestHold = new Promise((r) => { release = r; }); window.__ls1ReleaseDigests = () => release(); });
  const t3 = await c3.newPage(); const p3 = putLog(t3);
  await openWrite(t3, m.mid, B);
  const heldAtMount = await waitFor(async () => ((await t3.evaluate(() => window.__ls1HeldDigests)) >= m.ids.length ? true : null), 30_000);
  if (!heldAtMount) throw new Error('ordering precondition not reached: mount-time digests were not observed held');
  ...
  const heldCount = await t3.evaluate(() => { window.__ls1ReleaseDigests(); return window.__ls1HeldDigests; });
  const lateResolved = await waitFor(async () => ((await t3.evaluate(() => window.__ls1LateDigests)) >= heldCount ? true : null), 30_000);
  if (!lateResolved) throw new Error('ordering precondition not reached: held digests did not resolve after release');
  await sleep(500);
  await q('BEGIN');
  ...
  const checks = {
    ...
    comparisonUnderLockRefusesChangedBody: lockRounds.length === 3 && lockRounds.every((x) => x.waitingOnLock && x.status === 409 && x.competingStatePreserved && !x.saveMarkPersisted),
