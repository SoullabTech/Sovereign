...
**Status of this document:** **EXECUTED** (2026-09-11). `MAC-COMPILE-01` was recorded on exactly `eef487422` (`KERNEL-00_MAC-COMPILE-01_2026-09-11.md`, founder), after which the founder authorized execution: P1–P8 plus the three compile-era fixes **C1** (the 60-second sliding-window test asserted per-fault-class budgets wrongly — test corrected, `RecoveryPolicy` untouched), **C2** (harness reached actor-isolated `kernel.recorder` — export now goes through `VoiceKernel.exportJournalJSONL()` / `journalEvents()`, recorder private), **C3** (`.allowBluetooth` → `.allowBluetoothHFP`). Applied on `claude/voice-2026-census-01`; the applying commit is the new SHA. The applying commit is **`488e0666c`**. **`MAC-COMPILE-02` (founder, exactly `488e0666c`, `KERNEL-00_MAC-COMPILE-02_2026-09-11.md`): `swift build` PASS · `swift test` 17/17 · source gate 13/13 · `xcodegen` PASS · unsigned iOS compile PASS · signed build against the paired iPhone PASS — all predeclared gates green, so the device witness HOLD is LIFTED by the standing rule recorded here before the compile.** Witness READY · UNSPENT: nothing installed, nothing launched. `.allowBluetoothHFP` compiles at the iOS 16 deployment target against the iOS 26.2 SDK; one non-blocking `try?`-unused warning is deliberately left unchanged.
...
eef487422
   ...
   ├── source gate + swift build/test + xcodegen + unsigned + signed compile → 488e0666c recorded (MAC-COMPILE-02)   ← DONE, all green
   └── DEVICE WITNESS under the ratified law (runbook §3), on 488e0666c   ← NEXT · HOLD LIFTED · UNSPENT
```
