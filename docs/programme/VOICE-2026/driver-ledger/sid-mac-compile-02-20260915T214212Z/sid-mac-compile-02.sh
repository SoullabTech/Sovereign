set -e
SHA=faf918b5c5b2cd85f8e8a6c9cbda8bc76df11ce8
STAMP=$(date -u +%Y%m%dT%H%M%SZ)
WT=/private/tmp/sid-mac-compile-02-$SHA
DD=$WT-derived
...
( cd "$P" && find . -type f | LC_ALL=C sort | xargs shasum -a 256 ) > "$OUT/SID-MAC-COMPILE-02.manifest.sha256"
shasum -a 256 "$OUT/SID-MAC-COMPILE-02.manifest.sha256" | tee -a "$OUT/identity.txt"
wc -l "$OUT/SID-MAC-COMPILE-02.manifest.sha256" | tee -a "$OUT/identity.txt"
...
echo "SID-MAC-COMPILE-02 $STAMP subject $SHA out $OUT"
