...
SHA=ec60321b741d9a2ec0ba35b8b7134e16a6cb1f7a
...
WT=/private/tmp/sid-first-install-04-$SHA
OUT=/private/tmp/sid-first-install-04-out-$STAMP
test ! -e "$WT"
test ! -e "$OUT"
test -d "$APP"
...
( cd "$OUT" && find . -type f | LC_ALL=C sort | xargs shasum -a 256 ) > "/private/tmp/sid-first-install-04-$STAMP.SHA256SUMS.install"
mv "/private/tmp/sid-first-install-04-$STAMP.SHA256SUMS.install" "$OUT/SHA256SUMS.install"
echo "SID-FIRST-INSTALL-04 $STAMP subject $SUBJECT_SHA instrument $SHA container $CONTAINER out $OUT"
