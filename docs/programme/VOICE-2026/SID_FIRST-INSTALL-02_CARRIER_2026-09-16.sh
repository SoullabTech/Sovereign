...
git checkout -b feature/sid-first-install-02-evidence-$STAMP
DEST=docs/programme/VOICE-2026/driver-ledger/VPIO-02-SID-FIRST-INSTALL-02-$STAMP
test ! -e "$DEST"
mkdir -p "$DEST"
cp -R "$OUT"/. "$DEST"/
...
ls -la /private/tmp/sid-first-install-02-transcript-*.log /private/tmp/sid-first-install-02.sh >> "$DEST"/OUT-listing.txt
ls -la "$PRIOR01" >> "$DEST"/OUT-listing.txt
( cd "$DEST" && grep -v ' \./SHA256SUMS.install$' SHA256SUMS.install | shasum -a 256 -c - > SHA256SUMS.install.check 2>&1 )
( cd "$DEST" && find . -type f ! -name SHA256SUMS.run ! -name RETURN.txt | LC_ALL=C sort | xargs shasum -a 256 ) > "/private/tmp/sid-first-install-02-carrier-$STAMP.SHA256SUMS.run"
mv "/private/tmp/sid-first-install-02-carrier-$STAMP.SHA256SUMS.run" "$DEST/SHA256SUMS.run"
printf 'ACT SID-FIRST-INSTALL-02\nSTAMP %s\nSUBJECT faf918b5c5b2cd85f8e8a6c9cbda8bc76df11ce8\nINSTRUMENT 3035c02353b3cc0dab0b0ce1823116d6712b8eb1\nBUNDLE life.soullab.voicekernel.vpio02sid\nCONTAINER %s (from container.txt = the install result line)\nOUT %s\nLAUNCH 0\nSAMPLE 0\nTRANSPORT Mac Studio terminal directly\nPRIOR FIRST-INSTALL-01\n  STOP · authority-precondition refusal · SPENT\n  device-verbs 0 · install 0 · container none\n  transcript prior-attempts/SID-FIRST-INSTALL-01-STOP-transcript-20260915T235951Z.log (not 02 evidence)\n' "$STAMP" "$SID_CONTAINER" "$OUT" > "$DEST"/RETURN.txt
shasum -a 256 "$DEST"/SHA256SUMS.run
git add "$DEST"
git commit -m "witness(voice-2026): return SID FIRST-INSTALL-02 evidence ($STAMP; container $SID_CONTAINER)"
git push -u origin feature/sid-first-install-02-evidence-$STAMP
git log -1 --format=%H
echo "SID-FIRST-INSTALL-02 CARRIER PUSHED"
