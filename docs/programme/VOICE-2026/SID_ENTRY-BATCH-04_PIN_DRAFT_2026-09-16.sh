...
WT=$(sed -n '1p' "$PTR")
PF=$(sed -n '2p' "$PTR")
case "$WT" in /private/tmp/sid-entry-04-$SHA-*) ;; *) exit 2;; esac
...
mv "/private/tmp/sid-entry-batch-04-$STAMP.SHA256SUMS.entry" "$L/SHA256SUMS.entry"
echo "SID-ENTRY-BATCH-04 $STAMP subject $SUBJECT_SHA instrument $SHA preflight $PFSTAMP ledger $L"
