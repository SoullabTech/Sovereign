...
OUT=/private/tmp/sid-fd05-out-$STAMP
...
xcrun devicectl device info apps --device "$DEV" --bundle-id "$BID" --json-output "$OUT/apps-before-$BID.json"
test "$(grep -c "\"bundleIdentifier\" : \"$BID\"" "$OUT/apps-before-$BID.json")" = 1
test "$(grep -c "$SID_C" "$OUT/apps-before-$BID.json")" -ge 1
...
xcrun devicectl device info processes --device "$DEV" --json-output "$OUT/processes-just-before-phase-a.json"
test "$(grep -ci VoiceKernelHarness "$OUT/processes-just-before-phase-a.json")" = 3
test "$(grep -c "$K00_C/VoiceKernelHarness.app" "$OUT/processes-just-before-phase-a.json")" = 1
test "$(grep -c "$V01_C/VoiceKernelHarness.app" "$OUT/processes-just-before-phase-a.json")" = 1
test "$(grep -c "$HIST_C/VoiceKernelHarness.app" "$OUT/processes-just-before-phase-a.json")" = 1
...
xcrun devicectl device info processes --device "$DEV" --json-output "$OUT/processes-just-before-vpio-01.json"
test "$(grep -ci VoiceKernelHarness "$OUT/processes-just-before-vpio-01.json")" = 2
test "$(grep -c "$K00_C/VoiceKernelHarness.app" "$OUT/processes-just-before-vpio-01.json")" = 0
test "$(grep -c "$V01_C/VoiceKernelHarness.app" "$OUT/processes-just-before-vpio-01.json")" = 1
test "$(grep -c "$HIST_C/VoiceKernelHarness.app" "$OUT/processes-just-before-vpio-01.json")" = 1
...
test "$(grep -c "\"bundleIdentifier\" : \"$BID\"" "$OUT/apps-after-$BID.json")" = 1
test "$(grep -c "$K00_C" "$OUT/apps-after-$BID_K00.json")" -ge 1
test "$(grep -c "$V01_C" "$OUT/apps-after-$BID_V01.json")" -ge 1
test "$(grep -c "$HIST_C" "$OUT/apps-after-$BID_HIST.json")" -ge 1
test "$(grep -c "$SID_C" "$OUT/apps-after-$BID.json")" -ge 1
( cd "$OUT" && find . -type f | LC_ALL=C sort | xargs shasum -a 256 ) > "/private/tmp/sid-fd05-$STAMP.SHA256SUMS.disposal"
mv "/private/tmp/sid-fd05-$STAMP.SHA256SUMS.disposal" "$OUT/SHA256SUMS.disposal"
echo "SID-FOREIGN-DISPOSAL-05 $STAMP runner $RUNNER_SRC targets $K00_C $V01_C preserved $HIST_C harness-after 1 out $OUT"
