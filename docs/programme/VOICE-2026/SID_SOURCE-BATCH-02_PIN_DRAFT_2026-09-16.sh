...
ACT_MARK=/private/tmp/sid-source-batch-02-invoked.txt
test ! -e "$ACT_MARK"
ACTSTAMP=$(date -u +%Y%m%dT%H%M%SZ)
printf 'SID-SOURCE-BATCH-02 INVOKED %s subject %s instrument %s\n' "$ACTSTAMP" "$SUBJECT_SHA" "$SHA" > "$ACT_MARK"
test -n "$K00_EXEC_AUTHORITY"
PTR=/private/tmp/sid-source-preflight-02-current.txt
test -f "$PTR"
WT=$(sed -n '1p' "$PTR")
PF=$(sed -n '2p' "$PTR")
case "$WT" in /private/tmp/sid-source-02-$SHA-*) ;; *) exit 2;; esac
...
BEFORE=/private/tmp/sid-source-batch-02-$ACTSTAMP.before-dirs
AFTER=/private/tmp/sid-source-batch-02-$ACTSTAMP.after-dirs
find "$BASE" -maxdepth 1 -type d -name 'VPIO-02-SID-SOURCE-2*' | LC_ALL=C sort > "$BEFORE"
STAMP=$(date -u +%Y%m%dT%H%M%SZ)
scripts/witness/k00-driver-batch.sh VPIO-02-SID-SOURCE 10 --act output --vp on --mode L --hold 15 --subject vpio-02-sid --stimulus sid-nearend-gated --cancel-at 1000 --settle 2 2>&1 | tee "/private/tmp/sid-source-batch-02-$STAMP.log"
find "$BASE" -maxdepth 1 -type d -name 'VPIO-02-SID-SOURCE-2*' | LC_ALL=C sort > "$AFTER"
NEW=$(comm -13 "$BEFORE" "$AFTER")
test "$(printf '%s\n' "$NEW" | sed '/^$/d' | wc -l | tr -d ' ')" = 1
L="$NEW"
test -d "$L"
cp "/private/tmp/sid-source-batch-02-$STAMP.log" "$L/batch-invocation.log"
...
( cd "$L" && find . -type f | LC_ALL=C sort | xargs shasum -a 256 ) > "/private/tmp/sid-source-batch-02-$STAMP.SHA256SUMS.source"
mv "/private/tmp/sid-source-batch-02-$STAMP.SHA256SUMS.source" "$L/SHA256SUMS.source"
echo "SID-SOURCE-BATCH-02 $STAMP subject $SUBJECT_SHA instrument $SHA preflight $PFSTAMP ledger $L"
