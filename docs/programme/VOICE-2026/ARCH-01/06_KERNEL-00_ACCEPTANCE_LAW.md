# KERNEL-00 Acceptance Law — RATIFIED · KERNEL-00 OPEN (founder act, 2026-09-11)

**Act:** ARCH-01 · artifact 6 · 2026-09-11 · **Status:** **RATIFIED — founder act, 2026-09-11** (`00_README.md` §2b item 4): six of seven thresholds ratified exactly as written; K00-06 amended to duplex physiology; K00-11 Bluetooth wording clarified. **`KERNEL-00` is OPEN** — by the separate founder act recorded in `00_README.md` §2c (2026-09-11): *"KERNEL-00 is OPEN under the ratified Voice 2026 acceptance law … No STT, no TTS, no Web audio, no canonical MAIA, no legacy voice components, and no network egress."* This header was stale between that act and PRE-WITNESS-01 (finding P1); corrected in place, the act itself unchanged. **The thresholds and the named set are unchanged by PRE-WITNESS-01** — only the clarifications marked below were added, each recorded as a clarification of what the obligation already meant, never a loosening.
...
> the lower path is a **new** witness, not a retry."* *(Layout note: commit `ebd9eef5d` moved this quotation here and removed it from beside the §5 bullet; the founder ruled on 2026-09-14 that the quotation stands beside the amended bullet — that layout preference is superseded, the quotation here is retained, `ebd9eef5d` is not rewritten.)*
...
| K00-02 | **One entry, one exit — CLARIFIED (PRE-WITNESS-01, P2).** `enterConversation()` establishes the session once; `leaveConversation()` releases it once. The journal shows exactly **one member-caused activation** (`session_activated`, cause `enter_conversation`) and **one member-caused deactivation** (`session_deactivated`, cause `leave_conversation`) per session. OS-forced re-activations are lawful **only** when stamped `interruption_recovery` (K00-12) or `media_services_reset_recovery` (K00-13) with a causal parent; any other activation is extra. This is what the obligation always meant — the earlier wording ("exactly one activation and one deactivation") contradicted K00-12/13's lawful recovery re-activation and is withdrawn. | an activation or deactivation with no member cause and no recovery stamp; a session left active after leave | D |
...
| K00-17 | **The trace replays.** The run's journal replays into the state machine (`03_STATE_MODEL.md` §2) with no orphan transition and answers *what observation caused this act, and which generation* for every automatic act. **Clarified (PRE-WITNESS-01, P8):** "answers" means a structural causal parent — every automatic act carries `causeSeq` naming an earlier record of the same or an earlier generation; a cause string alone does not discharge this. | an orphan; an unanswerable act; an act whose `causeSeq` is missing, later, absent, or from a later generation | D |
...
| Cancel window (K00-05) | ≤ 100 ms from `cancel(handle)` to last rendered frame — **measured** as `cancelToSilenceMs` in the `stream_cancel_measured` record (render-tap time of the last non-silent frame minus cancel-issue time; PRE-WITNESS-01 P5). The duration of the cancel call itself is not the metric. | **RATIFIED** (metric named; threshold unchanged) |
...
- Lower Voice I/O path — **AMENDED (founder act, 2026-09-14; trigger only)**: *the lower Voice Processing I/O path may be opened as a new KERNEL-00 subject if the higher-level `AVAudioEngine` path cannot pass K00-04, K00-06, or K00-11. A failure under one of those obligations must first be recorded on the higher-level subject; the lower path is a new witness, never a retry or silent fallback.* *(Superseded wording, kept for the record beside the rule it replaced — placement ruled by the founder 2026-09-14: "K00-11's Voice-I/O fallback: if the `AVAudioEngine` voice-processing path cannot pass K00-06 or K00-11, the run is recorded as FAIL on that path and a second run on the lower path is a **new** witness, not a retry.")* The run on the higher-level path is recorded as FAIL on that path; the lower-path run carries its own build identity and witness lineage.
