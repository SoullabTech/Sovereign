...
| 3 | **Play 3 s tone** ×5, letting each complete — on the built-in route AND again with output on the **Pi8** (E23 consequence) | floor `maiaSpeaking` → `listening`; stream `complete` with frames rendered = scheduled; **input callbacks continue during the tone and rms does not collapse to zero — on the Pi8 as well as built-in**; the journal has no session mutation | K00-03, K00-05, K00-06 |
...
| 9 | Route: **Speaker** / **System default** buttons (authority-owned `overrideOutputAudioPort(.speaker | .none)` → `session_output_override` in the journal), then hold to ear / away, then **the named sequence `built-in → Pi8 Bluetooth HFP → built-in → Pi8 again`** (founder ruling on E23: the Pi8 is a named K00-11 route, not "some Bluetooth device"), then any other Bluetooth device the OS admits | each transition: `route_changed`, `engine_configuration_changed`, `graph_rebuilt`, then `healthy` **with no manual tap**; the route line shows the new path and data source; unsupported topologies recorded as platform capability | K00-11 |
