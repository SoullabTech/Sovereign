...
WT=/private/tmp/sid-duplex-01-$SHA-$STAMP
...
printf 'SID-DUPLEX-PREFLIGHT-01 %s CLEAN subject %s instrument %s sid-container %s
' "$STAMP" "$SUBJECT_SHA" "$SHA" "$SID_CONTAINER" | tee "$PF/PREFLIGHT-CLEAN"
SEAL=/private/tmp/sid-duplex-preflight-01-$STAMP.SHA256SUMS.preflight
...
' "$WT" "$PF" > "$PTR.tmp"
mv "$PTR.tmp" "$PTR"
echo "SID-DUPLEX-PREFLIGHT-01 $STAMP CLEAN worktree $WT preflight $PF"
