...
WT=/private/tmp/sid-source-03-$SHA-$STAMP
...
printf 'SID-SOURCE-PREFLIGHT-03 %s CLEAN subject %s instrument %s entry %s sid-container %s\n' "$STAMP" "$SUBJECT_SHA" "$SHA" "$ENTRY_SHA" "$SID_CONTAINER" | tee "$PF/PREFLIGHT-CLEAN"
SEAL=/private/tmp/sid-source-preflight-03-$STAMP.SHA256SUMS.preflight
( cd "$PF" && find . -type f | LC_ALL=C sort | xargs shasum -a 256 ) > "$SEAL"
mv "$SEAL" "$PF/SHA256SUMS.preflight"
printf '%s\n%s\n' "$WT" "$PF" > "$PTR.tmp"
mv "$PTR.tmp" "$PTR"
echo "SID-SOURCE-PREFLIGHT-03 $STAMP CLEAN worktree $WT preflight $PF"
