...
| 2 (after reinstall; dylib must read `CC0D3604-…`) | 1 | `K00-ea8d431d` (`…_run6S2-1_K00-ea8d431d_vpON.jsonl`) | 61 | `5789fba77ebb578b9badad9e168977aed1c2af3368dbb9795899377ec4a3c3ee` | **gen-1 listen** | isRunning immediate TRUE · vp_enable 80 ms · first callback +96 ms · listening 439 ms · 10/10 running ticks · gen 1 held 24.4 s · no recovery · 13-step trace |
| 2 | 2 | — | | | | |
| 2 | 3 | — | | | | |
...
**Stage 2 reinstall — FOUNDER-STATED, NOT EVIDENCED IN THIS SESSION.** The founder states the identical artifact was reinstalled before `K00-ea8d431d`. The install output, the `dwarfdump --uuid` line (expected `CC0D3604-7902-373E-A2BB-2C093D9BF804`) and the post-install installed/running check were **not received by this session**; if they exist in the Studio session they are owed here for the record. Until received, Stage 2's separation of source behaviour from install/runtime state rests on the founder's statement, and the S2 rows carry that qualification.

**Stage 2 sequencing as ruled:** reinstall the identical signed P5-B0 artifact (the same DerivedData product; the dylib must still read `CC0D3604-7902-373E-A2BB-2C093D9BF804`), record the install output and the post-install `installed / running` state, then three cold VP-ON sessions.

### Manual phone witness — SUSPENDED (founder, 2026-09-12: "I'm not testing on my phone anymore")

State at suspension: Stage 1 closed (5 sessions) · Stage 2 at 1 of 3 (`K00-ea8d431d`; reinstall founder-stated, install/UUID evidence not received here) · Stage 3 not reached. Cumulative on `24a6fcfa1`: 7 of 9 VP-ON cold starts took in generation 1. The placement question for the six post-run-6 journals (reinstall before the first or after the fifth) is unresolved and is moot unless the phone witness resumes.

The manual loop was the failure: one human tapping force-quit / launch / Enter / wait / Export for every sample, exporting through a share sheet, ferrying files and terminal text into a remote session, and the record reconstructing stage boundaries afterwards. It cannot hold a 2-second Leave (W4), it cannot hold a 15-second wait reliably (Stage 1), and it cannot produce the sample sizes a nondeterminism question needs.

The instrument that removes the human from the loop is the one the founder already named for W4 and left unauthorized: a deterministic on-device driver (an XCUITest target on the Mac that cold-launches the harness by bundle id, taps Enter, waits a fixed time, taps Export or reads the journal from the app container, N times, with the reinstall and the dylib UUID captured by the same script). It is harness/test-target code, not kernel code, but it is a new build and a new subject, so it needs a founder act. **Not authorized. Not built.**
