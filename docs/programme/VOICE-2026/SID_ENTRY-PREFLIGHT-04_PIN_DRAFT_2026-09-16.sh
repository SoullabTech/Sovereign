...
WT=/private/tmp/sid-entry-04-$SHA-$STAMP
...
printf 'SID-ENTRY-PREFLIGHT-04 %s CLEAN subject %s instrument %s sid-container %s\n' "$STAMP" "$SUBJECT_SHA" "$SHA" "$SID_CONTAINER" | tee "$PF/PREFLIGHT-CLEAN"
SEAL="/private/tmp/sid-entry-preflight-04-$STAMP.SHA256SUMS.preflight"
( cd "$PF" && find . -type f | LC_ALL=C sort | xargs shasum -a 256 ) > "$SEAL"
mv "$SEAL" "$PF/SHA256SUMS.preflight"
printf '%s\n%s\n' "$WT" "$PF" > "$PTR.tmp"
mv "$PTR.tmp" "$PTR"
echo "SID-ENTRY-PREFLIGHT-04 $STAMP CLEAN worktree $WT preflight $PF"
