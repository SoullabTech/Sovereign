...
WT=/private/tmp/sid-entry-05-$SHA-$STAMP
...
printf 'SID-ENTRY-PREFLIGHT-05 %s CLEAN subject %s instrument %s sid-container %s\n' "$STAMP" "$SUBJECT_SHA" "$SHA" "$SID_CONTAINER" | tee "$PF/PREFLIGHT-CLEAN"
SEAL="/private/tmp/sid-entry-preflight-05-$STAMP.SHA256SUMS.preflight"
( cd "$PF" && find . -type f | LC_ALL=C sort | xargs shasum -a 256 ) > "$SEAL"
mv "$SEAL" "$PF/SHA256SUMS.preflight"
printf '%s\n%s\n' "$WT" "$PF" > "$PTR.tmp"
mv "$PTR.tmp" "$PTR"
echo "SID-ENTRY-PREFLIGHT-05 $STAMP CLEAN worktree $WT preflight $PF"
