...
## §18.80 — SOURCE-SB-01 prepared after ENTRY green: S-b-only N=10 successor pinned; read-only preflight next, population conditional on CLEAN + fresh authority (2026-09-16; implementation §10.54; qualification §10.98)

ENTRY-UNPERTURBED at `26116d4e4` satisfies the source design's prerequisite. S-b remains exactly the accepted gated 997 Hz source discriminator; S-a NOT OPEN. Preflight pin `f675d5d7…`/56 proves durable ENTRY prerequisite, instrument `9ed72a38…`, source-reader 24/24, fixture/player hashes, Mac Studio Speakers built-in / volume 69 / unmuted, SID container identity, harness 0, then seals CLEAN. Population pin `b43e3709…`/32 consumes that exact CLEAN marker within 300 s and invokes exactly one N=10 `--act output --vp on --mode L --hold 15 --cancel-at 1000 --settle 2 --subject vpio-02-sid --stimulus sid-nearend-gated`; requires 10 journals + output/source ledgers and seals custody. Voice gate 90/90. Preflight OPEN read-only; population execution CLOSED until CLEAN + fresh authority; no top-up/rerun; KERNEL/BRIDGE/migration/production remain downstream.


## §18.81 — SOURCE-SB-01 custody repaired before execution: old output-normalizing pins superseded; PRE-PLAY + JIT zero-harness S-b guards; new successor pins ready (2026-09-16; implementation §10.55; qualification §10.99)

The durable `306654d6a` S-b preparation is historical but its execution candidates are superseded before use: instrument `9ed72a38…` protected SID ENTRY only, while SID output still used global `harness_present → testTerminateOnly`; that subject-scoped cleanup is incompatible with the already-witnessed foreign DAS-prewarm class. Code-only repair `17c4ee524` bypasses cleanup for the exact S-b tuple and requires total-harness-zero twice per sample — PRE-PLAY before afplay, JIT after settle immediately before the phone driver; JIT refusal stops afplay; no phone launch/terminate occurs. New pins at `422485c34`: preflight `46f97f5d…`/78 (ENTRY ancestry + reader 24/24 + installed identities + harness 0 + fixture/player/output 69/unmuted) and batch `4430a8d0…`/48 (fresh sealed CLEAN ≤300 s + one-shot exact N=10 S-b invocation, counts recorded, one new ledger, sealed evidence). Voice gate **91/91**. PRELIGHT next; population authority unissued; S-a closed; KERNEL/BRIDGE/migration/production downstream.

## §18.80 — SOURCE-PREFLIGHT-01 refused on exact prewarm set 3; BATCH-01 spent pre-population by wrapper fall-through; fresh SOURCE-02 + separate exact-set clearance prepared (2026-09-16; implementation §10.56)

Source physiology remains unmeasured. PRELIGHT-01 found `.k00 + .vpio01 + .vpio02` harnesses and never wrote CLEAN/seal. A non-fail-fast outer terminal wrapper then invoked BATCH-01; its one-shot marker exists, but the pin refused on the incomplete pointer before any stimulus, phone launch, ledger or journal. BATCH-01 is spent; no top-up/rerun. Successors PRELIGHT-02 `9a335d34…`/78 and BATCH-02 `f2648c83…`/48 preserve the guarded N=10 S-b law on instrument `f67575797…`. Separate SOURCE-HARNESS-CLEAR-01 `298deba2…`/79 requires exact-set 3→2→1→0 native process custody and preserves all installed containers. Gate 93/93. Clearance needs fresh authority; PRELIGHT-02 follows only after zero; BATCH-02 only after CLEAN ≤300 s.

## §18.82 — source harness clearance PASS; SOURCE-PREFLIGHT-02 refuses only on Mac output state; source physiology still 0 (2026-09-16; implementation §10.57; qualification §10.100)

SOURCE-HARNESS-CLEAR-01 passed exact 3→2→1→0 at 13:48:03Z with SID container preserved. Three subsequent PRELIGHT-02 attempts all proved ENTRY ancestry, reader 24/24, installed identities, harness 0, fixture SHA/geometry, then refused because default Mac output was `B06Ultra` Bluetooth 44.1 kHz rather than `Mac Studio Speakers` built-in; CLEAN/seal absent. Current read-only volume = 13, unmuted. Refusals sealed together. BATCH-02 marker remains absent; source population/journals/source rows = 0. Next = set the Mac to the ruled output state, then fresh PRELIGHT-02; only CLEAN ≤300 s can open the unchanged N=10 guarded S-b population.

## §18.83 — SOURCE-BATCH-02 PARTIAL/STOP/SPENT at 5/10; sample 6 catches recurring DAS prewarm before stimulus; SOURCE-03 fresh-full-N=10 successor prepared (2026-09-16; implementation §10.58; qualification §10.101)
PRELIGHT-02 CLEAN at 14:01:10Z. BATCH-02 completed samples 1–5 with PRE-PLAY=0, JIT=0, VALID stimulus custody and successful readers; all five SOURCE-ROW outcomes are VALID / CHARACTERIZE-SRC, descriptive only. Sample 6 PRE-PLAY observed exact historical `.k00 + .vpio01 + .vpio02` = 3 and STOPped before stimulus/phone/terminate; 7–10 absent. BATCH-02 is spent and never topped up. Partial evidence sealed at `VPIO-02-SID-SOURCE-PARTIAL-02-20260916T140115Z`. Existing DAS evidence gives repeating prewarm interval 900 s; successor orchestration may exploit the post-prewarm window but validity never depends on that timing because all-sample PRE-PLAY + JIT zero guards remain. Fresh CLEAR-02 `5a7d2626…`/79, PRELIGHT-03 `f806e92a…`/78 and BATCH-03 `c2c33b55…`/48 preserve the same physiology and require a new full N=10. Gate 96/96.

## §18.84 — SOURCE-03 observer transport failed closed; natural harness zero observed; direct zero-gated SOURCE-03 wrapper supersedes observer path before execution (2026-09-16; implementation §10.59; qualification §10.102)
The DAS-log observer did not attach and therefore never opened CLEAR-02/PRELIGHT-03/BATCH-03. Independent device-state evidence later showed total harness 0. The observer process was retired before any Voice authority spend. Direct successor wrapper `a0e66598…` requires Mac output custody and total harness 0 at invocation, then PRELIGHT-03 CLEAN and the unchanged one-shot fresh full N=10 BATCH-03. It cannot invoke CLEAR-02, `idevicesyslog`, or any process termination. Gate **98/98**.

## §18.85 — SOURCE-03 NOW wrapper syntax refusal before witness boundary; repaired wrapper only, BATCH-03 still unspent (2026-09-16; implementation §10.60; qualification §10.103)
Direct-wrapper invocation failed in the Mac-output check because the Python heredoc terminator and `VOL=` shell assignment were joined; a second unreachable `tee`/`unset` join was found in review. No harness-zero read, PRELIGHT-03, batch marker, stimulus or phone launch occurred. This is orchestration syntax custody, not physiology. Repaired wrapper `a0e66598…`/57 preserves the exact PRELIGHT-03 and BATCH-03 delegates and gains a discriminating line-boundary gate. Voice gate **98/98**.

## §18.86 — SOURCE-BATCH-03 COMPLETE / SPENT; ten lawful rows but source attribution 10/10 UNMEASURED-SRC; K00-06 remains INCOMPLETE; closure-gap census next (2026-09-16; implementation §10.61; qualification §10.104)
PRELIGHT-03 CLEAN → one fresh full N=10 BATCH-03. All 10 PRE-PLAY and JIT process reads = harness 0; 10/10 stimulus custody VALID; 10/10 journals = gen-1 listen; population seal verifies. The SOURCE-ID-02 reader returns 10 × `UNMEASURED-SRC: stimulus_not_visible`; no source-attribution class is earned. Frozen companion output reading = K00-05 PASS evidence throughout and K00-06 9 PASS / 1 UNMEASURED / 0 FAIL. Under the predeclared law, source evidence cannot earn K00-06 PASS and a 9/10 companion reading cannot satisfy the frozen 10/10 K00-06 built-in condition. Therefore K00-06 remains CHARACTERIZE ONLY · INCOMPLETE; BATCH-03 is spent; no top-up or automatic rerun. SID custody is now 90 = ENTRY 75 + SOURCE 15. KERNEL-00 remains NOT ACCEPTED under ARCH-01 artifact 6's all-named-obligations-PASS rule. Next = read-only closure-gap adjudication/plan; no new device act is inherited.

## §18.87 — the SOURCE-03 null read explained from the journals: V1 (`B997 ≥ 10 × C_base`, 20 dB) fails on all ten rows at 9–15 dB (ratios 2.8–5.9), while the 2 Hz gate index `m2_997` is ≥ 0.9 in 6–10 of 10 baseline windows per journal (medians 1.09–1.26) — the stimulus reached the seam, 5–11 dB short of the predeclared visibility bar; stimulus and output-device custody VALID; design §132 falsifier fired at population scale → return, never adapt; threshold NOT to be re-ruled; proposed next act = `SOURCE LEVEL-CALIBRATION-01` (the missing S-a arm: level/geometry sweep read at rest, predeclared margin, new level pin), optional VP-off arm, then a fresh N=10 · remote lane adopts this branch as record of record (2026-09-20; implementation record §10.62)
