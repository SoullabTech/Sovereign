...
OUT=/private/tmp/sid-source-harness-clear-01-out-$STAMP
test ! -e "$OUT"
mkdir -p "$OUT"
...
SEAL=/private/tmp/sid-source-harness-clear-01-$STAMP.SHA256SUMS.clearance
( cd "$OUT" && find . -type f | LC_ALL=C sort | xargs shasum -a 256 ) > "$SEAL"
mv "$SEAL" "$OUT/SHA256SUMS.clearance"
echo "SID-SOURCE-HARNESS-CLEAR-01 $STAMP PASS k00 $K00_CONTAINER vpio01 $V1_CONTAINER vpio02 $V2_CONTAINER sid-preserved $SID_CONTAINER harness-after 0 out $OUT"
