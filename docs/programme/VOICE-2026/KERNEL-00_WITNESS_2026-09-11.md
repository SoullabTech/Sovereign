...
> **Correction (post-seal, 2026-09-11 ~20:55, evidence in `KERNEL-00_MAC-COMPILE-03_2026-09-11.md` §5):** `81D0C25C-…` is the UUID of the app's **stub executor**, which Xcode links identically for every local build of this project (`libPreviewsJITStubExecutor`, `-reproducible`); the kernel and harness code live in `VoiceKernelHarness.debug.dylib`, whose UUID changes with the code. The match between this UUID and the three crash reports therefore binds the crashes to the project's stub, **not to the CLI-built kernel specifically**, and does not discriminate the CLI artifact from a GUI build. The standing verdict — *binary replacement by Xcode NOT established* — is unchanged; the strength of the evidence cited for it is reduced from "binary identity" to "same project, same stub". The run-1 code identity is the `VoiceKernelHarness.debug.dylib` UUID inside the three `.ips` reports. **CLOSED 2026-09-12 — VERIFIED on the Mac Studio by parsing the 16:23:49 report's `usedImages` (report copied via `devicectl … --domain-type systemCrashLogs`, output verbatim): `81d0c25c-73c9-39be-b617-cf1020d0a9a4 arm64 VoiceKernelHarness` (the stub) · `dc6a68e9-f3a0-3427-871d-555ce7fafd7a arm64 VoiceKernelHarness.debug.dylib` (the run-1 code).** It differs from every later build (`1AEBEE45-…` run 2 · `F00F11D4-…` run 3 · `37A27138-…` run 4), which proves by inequality that each later run executed new code. The crash-log domain holds exactly the three run-1 reports (16:23:49 · 16:24:16 · 16:25:06) and no `VoiceKernelHarness` report after 19:54 on the 11th or on the 12th — runs 2–4 produced no death. Nothing else in this record depends on this line: the crash chain, the 0 Hz format, and the four reproductions were observed directly. Recorded in place; the sealed text above is left as written.
