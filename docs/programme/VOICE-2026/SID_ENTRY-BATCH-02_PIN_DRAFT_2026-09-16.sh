...
WT=/private/tmp/sid-entry-02-$SHA
test -d "$WT"
cd "$WT"
...
scripts/witness/k00-driver-batch.sh VPIO-02-SID-ENTRY 30 --vp on --mode L --hold 15 --subject vpio-02-sid 2>&1 | tee "/private/tmp/sid-entry-batch-02-$STAMP.log"
L=$(ls -d "$WT"/docs/programme/VOICE-2026/driver-ledger/VPIO-02-SID-ENTRY-2* | LC_ALL=C sort | tail -1)
test -d "$L"
cp "/private/tmp/sid-entry-batch-02-$STAMP.log" "$L/batch-invocation.log"
ls "$L"/journals/*.jsonl 2>/dev/null | wc -l | tr -d ' ' | tee "$L/journal-count.txt"
grep -c '^| ' "$L/ledger.md" | tee "$L/ledger-line-count.txt"
( cd "$L" && find . -type f | LC_ALL=C sort | xargs shasum -a 256 ) > "/private/tmp/sid-entry-batch-02-$STAMP.SHA256SUMS.entry"
mv "/private/tmp/sid-entry-batch-02-$STAMP.SHA256SUMS.entry" "$L/SHA256SUMS.entry"
echo "SID-ENTRY-BATCH-02 $STAMP subject $SUBJECT_SHA instrument $SHA preflight $PFSTAMP ledger $L"
