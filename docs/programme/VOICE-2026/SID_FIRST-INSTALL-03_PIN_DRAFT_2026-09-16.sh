...
WT=/private/tmp/sid-first-install-03-$SHA
OUT=/private/tmp/sid-first-install-03-out-$STAMP
test ! -e "$WT"
test ! -e "$OUT"
test -d "$APP"
...
( cd "$OUT" && find . -type f | LC_ALL=C sort | xargs shasum -a 256 ) > "/private/tmp/sid-first-install-03-$STAMP.SHA256SUMS.install"
mv "/private/tmp/sid-first-install-03-$STAMP.SHA256SUMS.install" "$OUT/SHA256SUMS.install"
echo "SID-FIRST-INSTALL-03 $STAMP subject $SUBJECT_SHA instrument $SHA container $CONTAINER out $OUT"
