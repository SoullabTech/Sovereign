...
- F-W1 **The entry axis does not move.** Predeclared reading of the first automated cold-launch stratum (N = 30, Mode L, VP ON): gen-1 `inputFlow = healthy` within 1500 ms on markedly more than the engine strata (A 14/29 · B 15/28 · C 16/29 · R1 13/30 — never pooled, laid beside) → the substrate substitution *may* matter (no mechanism claim); **about the same** → VPIO-01 has not demonstrated a more stable substrate and the lower-path question is answered NO on this device/runtime; **worse or infra-heavy** → characterize only. Which of these it is decides whether a second witness is worth a founder act; none of them authorizes a repair cycle.
...
- **VPIO-01 build identity:** its own compile record (`KERNEL-00_VPIO-01_MAC-COMPILE-01_<date>.md`), its own signed artifact, its own dylib UUID + SHA-256 + manifest recorded before any install. **Recommended, founder decides:** a distinct bundle identifier (e.g. `life.soullab.voicekernel.vpio01`) so the device can never hold two subjects under one id and the driver's `--subject vpio-01` maps to a bundle id in a subject table (C-D7 pattern). If the founder keeps `life.soullab.voicekernel.k00`, the reinstall custody must additionally record that the prior subject was removed.
...
- **Instrument reuse (no new instrument):** DRIVER-01 Mode L, `k00-driver-batch.sh`, the ledger classifier, `k00-reinstall.sh`, container archive/purge (a fresh archive-and-purge precedes VPIO-01 sample 1 — the container has accumulated exports since the 2026-09-13 purge: the pre-install batch, R1, LOG-CAL and both SEAM-01 blocks; the count is read by listing at archive time, never assumed).
