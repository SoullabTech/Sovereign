...
## §13.20 — ADJUDICATED (founder, 2026-09-14): VPIO-02 F-W1 PASS · CLOSED · K00-04 PASS on VPIO-02 · lower-path entry QUALIFIED · KERNEL-00 NOT ACCEPTED · K00-05/06 act NOT OPENED

Recorded verbatim in the witness record §9. §13.19 witness accepted as read: 29/30 gen-1 takes against the predeclared ≥24/30, Fisher < .05 vs A · B · C · R1 independently → **F-W1 SPENT · PASS**. Sample 1 = valid `failure then recovery` (not invalid, not a hidden take); its lone `willResignActive` preserved as observed / plausible inference / causation not established; no lifecycle experiment authorized. Established: the corrected VPIO-02 substrate demonstrates reliable healthy entry on the physical microphone path, repeatedly, through the whole probe → guard → configure → arm → initialize → `AudioOutputUnitStart` → running → callbacks → healthy-physiology chain — the question VPIO-01 left unmeasured is answered YES for the entry axis. C-D19 accepted as gate maintenance. Wording correction: the 1500 ms entry ceiling is **K00-04**, not K00-03 (witness §4 not rewritten; §9 governs); latencies descriptive only. Unmeasured on VPIO-02: K00-05 · 06 · 11 · 12 · 13 · 15.

**Standing:** VPIO-01 CLOSED · FAILED · 0/30 · **VPIO-02 F-W1 CLOSED · PASS · 29/30** · K00-04 PASS on VPIO-02 · lower-path entry QUALIFIED · new sample / route / interruption / reset / endurance / reinstall NOT AUTHORIZED · `.vpio01` FROZEN · K00/R1 UNTOUCHED · organism `ac12dedf4` · instrument `08483cfe4` · **KERNEL-00 NOT ACCEPTED** · KERNEL-01 / BENCH-01 / BRIDGE-01 / MIGRATE-01 CLOSED · JOP-04 UNTOUCHED. **Next programme decision (founder's, not opened here): whether to open the next bounded qualification act for K00-05 output/cancellation and K00-06 duplex physiology on the VPIO-02 subject.**

## §13.21 — K00-05 / K00-06 QUALIFICATION PLAN + INSTRUMENT COVERAGE CENSUS RETURNED (read-only; founder authorization 2026-09-14)

`KERNEL-00_VPIO-02_K00-05-06_QUALIFICATION_PLAN_2026-09-14.md`. Census: the frozen organism already journals every K00-05/06 seam (`stream_scheduled` with handle · `output_render_sample` progress · `stream_complete` · `stream_cancelled` · `stream_cancel_measured` 300 ms after cancel · per-second `input_health_sample` with `digitalZero/noiseFloor/signal`); the harness exposes `Play 3 s tone` / `Cancel active` by label; the driver taps neither, the entry classifier reads no output record, and **no journal in the repository carries `stream_cancel_measured` or `stream_complete` — K00-05/06 have never been exercised on any device on any subject.** Each obligation given its own falsifiable reading (§2.5 / §3.5), the three-way input vocabulary preserved, coupling descriptive only (post-AEC noted). Options: A manual (needs the suspension lifted) · B harness change REJECTED (changes the qualified subject) · **C driver-only RECOMMENDED** (one new test + two batch flags + one new output reader; organism and installed `.vpio02` untouched) · D unnecessary. Shape: N = 10 invocations, each one cancel row + one completion row + one duplex row + one coupling record; ceilings on every row, no rate comparison; validity / infra / abort / PASS / FAIL predeclared. Nothing implemented, compiled, installed, launched, played, cancelled or sampled. Returned for ruling (plan §7). Standing unchanged.

## §13.22 — K00-05/06 PLAN ACCEPTED WITH AMENDMENTS (founder, 2026-09-14) · OPTION C · INSTRUMENT PREPARATION OPEN · witness NOT YET AUTHORIZED

Ruling recorded prospectively in the qualification plan §8 (amendments govern over §§2–6): Option C only, exact surface (driver test · batch · NEW `k00-output-ledger.py` · gate · records); sequence values explicit (`--act output --cancel-at 1000 --settle 2`; historical invocations byte-identical in behaviour); K00-05 corrections (no pre-cancel-sample requirement; no completion latency ceiling; foreign input fault = non-evidence, never automatic FAIL-05); K00-06 pinned (full rendering windows; baselineRate median over ≥2 healthy pre-output windows; gap FAIL < 0.90×; collapse FAIL = `digitalZero == callbacks` or `input_dead`; partial zero = CHARACTERIZE; ≥2 full windows else UNMEASURED; 0.90 is a witness criterion, not a constitutional threshold); N = 10 population law (no top-up; FAIL decides, rows continue; EN/infra/not-a-cancel = no evidence, INCOMPLETE if < 10 valid); green = built-in duplex only, final K00-06 closure travels with K00-11; reader evidence-only with twelve pinned offline cases; sequence record → implement → self-test → gate → instrument SHA → driver-only compile → STOP. No device act authorized.

## §13.23 — K00-05/06 OPTION C INSTRUMENT LANDED at `8b111709b` (qualification plan §9) — NOT COMPILED · driver-only compile pinned as the next founder act · device witness NOT AUTHORIZED

Driver `testOutputSample` (ruled sequence, markers by `NSLog`, historical tests byte-identical) · batch `--act entry|output · --cancel-at 1000 · --settle 2` (historical invocation verbatim; output act forwards exactly two runner-env values and reads each journal a second time into `output-ledger.md`) · NEW evidence-only `k00-output-ledger.py` (four rows per journal; 100 ms ratified · 0.90 witness criterion; self-test 33/33; 60 real VPIO journals → never PASS/FAIL) · gate 64/64. Organism `ac12dedf4` and `k00-ledger.py`/`k00-reinstall.sh` `08483cfe4` byte-frozen, gate-pinned. One offline defect (handle mismatch first read as absence) corrected before the gate. Compile invocation pinned in plan §9; STOP after its record.
