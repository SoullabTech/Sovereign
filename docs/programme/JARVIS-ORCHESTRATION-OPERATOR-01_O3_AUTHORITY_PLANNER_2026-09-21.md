...
## O3R3 — proxy handling at the inert boundary

Independent review of O3R2 candidate `21b819c0e` reported two input-boundary failures. Both were reproduced against the candidate before repair:

- a live `Proxy` whose `getPrototypeOf` / `ownKeys` / `getOwnPropertyDescriptor` traps ran during snapshot inspection was **accepted** (`ok: true`, three traps executed);
- a revoked `Proxy` **threw** `TypeError: Cannot perform 'IsArray' on a proxy that has been revoked` out of `planAuthority` instead of returning a structured refusal.

Both are boundary defects and neither established an unauthorized grant: no authority was granted, no routing or execution occurred, and the plan produced for the live-proxy case carried the caller's own declared authorities.

O3R2 established that no caller code runs *between validation and use*. It did not establish that no caller code runs *during inspection itself*. `Array.isArray`, `Object.getPrototypeOf` and `Object.getOwnPropertyDescriptors` are each trap-invoking operations on a proxy, and each throws on a revoked one. The first of the three sat outside the existing `try`, which is why the revoked case escaped as an exception.

The repair places a proxy gate ahead of every reflective operation:

- nothing touches the value reflectively until it is established that no trap can run — `WeakSet` membership and `typeof` are trap-free and may precede the gate, nothing else may;
- proxy-backed input is refused as `INERT_SNAPSHOT_PROXY_REFUSED` at its own path, at any depth;
- `Array.isArray` is moved inside the existing `try`, and the whole walk is contained, so no input shape can propagate a throw out of the boundary;
- a proxy cannot be recognized from ordinary JavaScript — every reflective operation on one runs the trap. Only a host predicate reads the internal slot without invoking a trap, so O3 resolves `util.types.isProxy` at load;
- **where the host offers no such predicate, O3 refuses all object input** (`INERT_SNAPSHOT_DETECTOR_UNAVAILABLE`) rather than inspecting what it cannot establish to be inert. This is fail-closed by construction. O3 has no browser consumer today; the UMD global export would refuse every input under this rule until a detector is supplied.

The detector is a parameter of `makeInertSnapshotter(detector)`, not an ambient lookup, so the detector-absent path is itself falsifiable.

Extended invariant:

> **No caller-controlled code may execute at the O3 input boundary at all — not during validation, not during planning, and not during the inspection that produces the inert snapshot.**

### O3R3 falsifiers

- F45 — proxy-backed input is refused **before any trap can execute** (asserts `trapsRan === 0`; a refusal produced by the trap that was supposed to be prevented is not a refusal);
- F46 — a revoked proxy returns a structured refusal and never throws;
- F47 — a proxy nested inside otherwise plain data is refused at its own path (`graph.effects`), traps unexecuted;
- F48 — the boundary fails closed when no trap-free detector exists, and the detector this build actually runs with is asserted present rather than assumed.

### O3R3 lethality

Three defeat candidates were built and run against F45–F48:

| Candidate | Error embodied | Result |
|---|---|---|
| DC-A | catch-only repair: contain the throw, detect nothing | **4/4 fail** |
| DC-B | detect where possible, fail **open** where the detector is absent | **F48 fails** (3 pass) |
| DC-C | detect, but gate placed **after** reflective inspection | **F45/F46/F47 fail** (F48 passes) |

DC-B is the discriminating case: it fails exactly the one law it breaks. DC-A's and DC-C's wider collateral is irreducible — a candidate that refuses a proxy only after inspecting it cannot name the proxy in its refusal, so the proxy-specific codes are unreachable for it. Narrowing either candidate further would stop it embodying its error.

### O3R3 witness

Run in-session against the reconciled tree:

- O3 Authority Planner suite: **49 passed · 0 failed** (45 carried + 4 new);
- full `jarvis-desktop` suite with O3 present: **295 tests · 280 passed · 6 failed · 9 skipped**;
- same suite with the three O3 files withheld (canonical baseline): **246 tests · 231 passed · 6 failed · 9 skipped**;
- **0 attributable regressions** — the six failures are identical in both runs and pre-existing at canonical (`CONTROL 3 — C3 automatic execution is absent`, two `r5b-execution-authorization` cases, two `wire-local-native` cases, and one nested tripwire pair);
- module loads under CommonJS; `PROXY_DETECTION_AVAILABLE === true` in this environment.

### O3R3 freshness

Reconciled onto canonical `fa5274fd`. O0/O1/O2 dependency blobs are byte-identical between the candidate base `7c58ad533` and canonical:

- `src/operator-constitution.js` — `5038741ee73a2a565f4486f30881bf8e7196bdea`;
- `src/operator-intent-contract.js` — `d46b63f67714ed2d5df6fd08f76beebc9cc7b2c7`;
- `src/operator-work-graph.js` — `8535566fab70ceaad453738d4405543009597b08`.

The O3R2 candidate touched three files, none of which exists at canonical, so reconciliation carried no content conflict.
