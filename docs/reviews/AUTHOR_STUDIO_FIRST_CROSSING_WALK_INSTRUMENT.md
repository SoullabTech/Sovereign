...
## 1ter. Governing authority, run conditions, and the two paths

✅ **G1 RATIFIED 2026-08-05** — this instrument is the **governing measurement authority** for this
evidence event. Record: `docs/design/author-studio/WRITERS_FIELD_RECONCILIATION_DECISION_RECORD_2026-08-05.md` §10.1b.

⭐⭐⭐ **G1 opens an evidence lane, ⛔ not a build lane.** Governing question:

> **Can the existing room preserve authorship through the crossing?**

⛔ **Non-goals — none of these is under test, and no result here bears on any of them:**
Writer's Field authorization · corpus adoption · Canvas decisions · W8 validation · House redesign ·
A0 validation.

### ⛔ Deployment guard — the experiment boundary
```
at C1:  verify GIT_COMMIT = f46a4fde4
at C9:  verify GIT_COMMIT = f46a4fde4
```
⚠️ A mid-walk deployment is ⛔ **not** a product failure — it is a **failure of the experiment
boundary**, and voids the run. (Trunk is at `c0a133059` / #957, **undeployed** as of 2026-08-05
01:5x — so the pin is live but not idle.)

### ⭐⭐ Two legitimate beginnings — record which path each step ran on

```
Import path:   Source  →  Working Draft  →  revisions  →  return
Blank path:    Nothing →  Working Draft  →  revisions  →  return
```

- **C3 (source preservation) applies to the Import path only.** ⛔ A blank page **cannot fail** a
  source-preservation test — there is no source. It is a different path, not a missing one.
- **C5 must run on the Blank path.** ⭐⭐⭐ The blank path is ⛔ **not a shortcut — it is the actual
  test**: the question is not *"can the system import a document?"* but

  > **Can a writer begin writing without the system demanding a prior object, structure, or
  > management decision?**

### Recording rule during the walk
Report: **what you expected · what happened · hesitation · surprise · any moment the software became
the object of attention instead of the writing.**
⛔ **No interpretation during the walk.** Interpretation belongs after the evidence exists.
