...
> ⚠️⚠️ **Interpretation amended 2026-08-09 — Ruling 1. ⛔ Every measurement above is unchanged and must
> not be edited; this measurement is what Ruling 1 itself rests on.**
>
> ⛔ **Blocker 1 is no longer *"operational adoption."*** Ruling 1 §2: `practitioner_clients`
> population or member association *"does not establish the constitutional commitment and does not
> confer relational authority."* §2.1 **prohibits** backfilling the commitment from it. ⭐ So linking
> the remaining 12 rows would resolve **identity reference** and ⛔ clear **nothing** constitutional.
>
> ⭐⭐⭐ **Restated:** the blocker is **absent bilateral constitution**, ⛔ not unpopulated identity.
>
> ✅ **Blocker 2 is RULED.** *"Competing referents"* is superseded: the commitment is the bilaterally
> constituted relationship; `relationship_spaces` at the constituted threshold is its **present
> expression** (⛔ not the constitution itself — §6); `practitioner_clients` remains a legitimate
> **contact/operational record** that ⛔ confers no authority. ⚠️ Ruled ⛔ does not mean satisfied —
> `relationship_spaces` still holds **0 rows**.
