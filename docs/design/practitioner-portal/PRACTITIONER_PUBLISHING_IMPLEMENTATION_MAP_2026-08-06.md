...
| **Commitment** | The `relationship_space` — `active` + `consent_status='accepted'` between **the acting person and the recipient** | Permissions §1, §3 | ✅ Consistent. Synthesis stated it abstractly; portal grounds it in `20260701000001` | `commitment_ref` → `relationship_spaces.id` ⭐ **at the constituted threshold** (both parties identified · `active` · `consent_status='accepted'`) — ✅ affirmed by Ruling 1 as the **present implementation referent**, ⛔ not as the constitution itself (§6 definitional primacy). ⛔ Authority is bounded by [Permissions §3.1](PRACTITIONER_PUBLISHING_PERMISSIONS_2026-08-06.md) |
...
|---|---|---|---|
| **1** | ✅ **Commitment Authority — RATIFIED 2026-08-09** | ⭐ the **constitutional container**. ⛔ The disjunction *"`relationship_spaces` **or** `practitioner_clients`"* is **superseded**: see the holding below | — |
...
> ⭐⭐⭐ **RULING 1 HOLDING (ratified 2026-08-09) — supersedes the disjunction in row 1.**
> [`FOUNDER_RULING_1_COMMITMENT_AUTHORITY_2026-08-09.md`](FOUNDER_RULING_1_COMMITMENT_AUTHORITY_2026-08-09.md)
>
> The commitment is a **bilaterally constituted relationship between two governed member identities**.
> Its **present schema expression** is `relationship_spaces` where **both parties are identified**,
> `status = 'active'`, and `consent_status = 'accepted'`.
>
> ⭐ `practitioner_clients` *"may remain a legitimate contact and operational record"* — ⛔ but it is
> **not** the container, and its population or association with a member ⛔ **confers no relational
> authority**.
>
> ⭐⭐ **Definitional primacy (§6):** the table is the **current implementation referent**, ⛔ not the
> constitution. A schema redesign ⛔ would not change constitutional law.
>
> ⛔ **Boundary (§2, §3):** the commitment ⛔ confers no ownership of either member's identity,
> history, data, memories, or activity outside it — canonical text at
> [Permissions §3.1](PRACTITIONER_PUBLISHING_PERMISSIONS_2026-08-06.md).
>
> 📌 **Narrower dependency than row 2 states:** Ruling 1 Amendment 3 settled the `practitioners`
> identity question **inside Ruling 1**, expressly so it would ⛔ not be carried into Ruling 2.
...
| 1 — **Identity linkage** (⛔ ~~operational adoption~~ — **reclassified, see below**) | ⛔ **no** — 12 of 13 relationships still name no member |
| 3 — **Eligible Work corpus** (product lifecycle) | ⛔ **no** — 0 of 2228 Works ratified |

> ⚠️⚠️ **BLOCKER 1 RECLASSIFIED — Ruling 1 §2, §2.1 (ratified 2026-08-09).** ⭐ The measurement stands
> (**12 of 13** name no member); ⛔ its **interpretation** does not.
>
> ⛔ **Populating `member_id` on all 13 rows would not clear this blocker.** Under Ruling 1,
> `practitioner_clients` population or member association *"does not establish the constitutional
> commitment and does not confer relational authority."*
>
> ⭐ **What linkage does do:** resolve **identity / reference**. ⛔ **What it does not do:** establish a
> **constituted practitioner–member commitment**.
>
> ⭐⭐⭐ **The blocker is not unpopulated identity — it is absent bilateral constitution.** It clears
> only when the governed bilateral process establishes the authoritative commitment: both parties
> identified · `active` · `consent_status='accepted'`. ⛔ §2.1 **prohibits** backfilling it from the
> contact record where the member's constituting act did not occur.

⭐ So after the four rulings, Placement **design** is constitutionally grounded and may proceed. ⛔ Placement in **use** still waits on 1 and 3. ⚠️ **Blocker 3 is a lifecycle problem; blocker 1 is now partly constitutional** — ⛔ it is not an adoption task and ⛔ is not backfillable. Building the table does not populate a governed subject or ratify a single Work.
