...
Ruled the same day: `docs/architecture/FIELD_OBJECT_PROMOTION_RULING_2026-08-02.md`.

⛔ The Workbench does **not** ingest capsules. Capsules are **not** silently atoms.
The capsule stays the **review/refinement artifact**; the member performs an **explicit
act** — *"Keep in my Field"* — that mints the canonical atom with member-authored
provenance. The Shelf keeps reading canonical Field Objects only.

⭐⭐⭐ `Field = medium · Work = object` does **not** require every medium artifact to become
a Field Object. **A Field Object exists when the member performs the act that says "this
belongs in my enduring Field." That act is the authority boundary** — and it may be reached
from multiple source surfaces without making those surfaces ontologically identical.

Corrections (2) and (3) are independent of the ontology ruling and proceed on their own,
as **separate PRs or clearly separate commits** — each has its own acceptance and revert
shape.
