...
- This is the same defect the other two lanes fixed in March 2026 (`542b43a2f` build-ios.sh, `256513b65` deploy-testflight.sh — "eliminate JS redirect loop"). The canonical `scripts/ios/build.sh` (last touched 2026-03-03) predates the fix and never received it. **Implication recorded honestly: the May 2026 TestFlight builds (2491–2496) cannot have been produced by an unmodified `scripts/ios/build.sh` Step 3**; the exact lane/local state that produced them is not reconstructable from the repo.
