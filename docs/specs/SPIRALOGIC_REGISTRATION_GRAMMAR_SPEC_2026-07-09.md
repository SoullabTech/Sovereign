...
**Canonical home:** `docs/specs/SPIRALOGIC_REGISTRATION_GRAMMAR_SPEC_2026-07-09.md` on `feature/now-what-maia-presence` (main worktree) until merged to `clean-main-no-secrets`, then main is home. Every other appearance of this spec — pasted blocks, other branches, summaries — is a pointer, not an editable copy. Edits happen here only; ratifications land here first.
**Status:** **FULLY RATIFIED (Kelly, 2026-07-10)** — Q0/Q2 authored; Q3/Q7 resolved by consequence; **Q6 RATIFIED = Option C (2026-07-09)**; **Q1/Q4/Q5 RATIFIED as written (2026-07-10)**; **SH-10/11/12 RATIFIED as written (2026-07-10)**; **SQ-1/2/3 RATIFIED (2026-07-10)**. No open questions remain at the grammar layer. Still outstanding (outside this spec's jurisdiction): the wiring crossing (Kelly-gated, carries SQ-1's export) and the interpretive-layer dominance rule (`interpretation_version`, designed against the null-rate table). **Finding 6 RULED (Kelly, 2026-07-10): PHASE NAMES** — vector/circle/spiral = interpretive-layer display vocabulary for phases 1/2/3, modality-keyed per the Elemental Alchemy manuscript; grammar schema stays {1,2,3}; house-keying dies at the engine refit (see conformance report Finding 6 row for evidence + ruling).
...
## Spec-hole resolutions — RATIFIED (authored 2026-07-09 · ratified as written, Kelly, 2026-07-10; the contract the build implements)

- **SH-10 — `ChartPositions` input contract:** `{ bodies: Record<Q1Body, number>; mode: 'timed' | 'noon'; moonBranches?: [number, number] }`. Longitudes are ecliptic degrees in `[0, 360)`, full precision. `registerChart` resolves signs **internally** via half-open 30° arithmetic on the raw longitude (INV-5 becomes testable at the grammar layer; the function never consumes an upstream rounded degree). `moonBranches` is present **iff** `mode:'noon'` and the Moon is sign-ambiguous that day — it replaces `bodies.Moon`, each branch contributing weight 0.5 (`moonUncertain: true` emitted). **✅ RATIFIED as written (Kelly, 2026-07-10)** — the grammar owns sign resolution end-to-end; no pre-decided signs accepted.
- **SH-11 — Out-of-vocabulary input: THROW.** Missing Q1 body, non-finite longitude, or longitude outside `[0, 360)` → typed `RegistrationInputError`. The grammar refuses rather than repairs; the Q4 Moon split (explicit, flagged) remains the sole sanctioned degradation. This specifies the fix-shape for conformance Finding 9's silent-drop class. **✅ RATIFIED as written (Kelly, 2026-07-10)** — canonical instance of the refuse-not-repair pattern.
- **SH-12 — Distribution serialization + input fingerprint:** distribution carries **raw weights, never percentages** (Finding 8 lesson), all 12 keys always present, key naming `fire_1 … air_3` (matches the live `element_N` runtime vocabulary). Canonical byte representation for INV-1: JSON with lexicographically sorted keys, weights via `Number.prototype.toString()`. `input_fingerprint` = SHA-256 over the canonical serialization of `{bodies (sorted), mode, moonBranches}`. **✅ RATIFIED as written (Kelly, 2026-07-10)** — determinism is byte-testable; every result carries provable input provenance.

## Post-build standings (Kelly, 2026-07-09, after the 38/38 fresh build)

- **Refuse-not-repair RATIFIED AS A PATTERN, not just two behaviors** (covers SQ-2, SQ-3, and future input-contract questions at this layer): the registration layer *throws* on any input it cannot register totally — silent repair was the old engine's deepest sin (a 9-body chart masquerading as total registration). "Throws" is the registration-layer expression of sacred-refusal-vs-system-failure. The Q4 Moon split remains the sole sanctioned degradation because it is explicit and flagged. **✅ SQ-2 and SQ-3 RATIFIED as written (Kelly, 2026-07-10)** — out-of-Q1-set body throws; `moonBranches` iff-violations throw. Both confirmed as instances of the ratified refuse-not-repair pattern.
- **SQ-1 (export `SIGNS_TO_ELEMENT`) is BUNDLED INTO THE WIRING CROSSING**, not done loose now — the fix touches live `spiralogicEngine.ts`, and live-file edits are licensed only inside that crossing. Until then the read-source equivalence pin stands (Built-unwired discipline applied to a test). **✅ RATIFIED (Kelly, 2026-07-10)** — source-parsing pin now, export bundled into the wiring crossing, nothing loose in between.
