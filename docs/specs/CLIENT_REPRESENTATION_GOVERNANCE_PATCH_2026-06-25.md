...
## Follow-up — Parallel Client Representation Stores Governance Pass

Deliberately **not** folded into this patch (keeps the first patch coherent). A separate, focused pass.

**Scope:** `maia_prep` (on `practitioner_sessions`) + `spiralogic_reports` — generate / persist / surface gates, body-auth, and private-mode edge handling (*existing* representations non-surfaceable under private).

**Already inherited from this patch:** ✓ `stellium/maia/prepare` auth closed + maia_prep generate gate. **Remaining:** maia_prep **surface** gate (`sessionPrep.ts`); full `spiralogic_reports` governance (8+ read/write routes). The `representationRefusal` / `maySurfaceRepresentation` guards already exist and are **7/7-proven** — this pass is *wiring*, not new policy.
