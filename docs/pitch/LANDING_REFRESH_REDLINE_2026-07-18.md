...
**Section restructure (Kelly directive, same day):** the "Earlier work / Sites we've built for others" list is REMOVED from the page (Old Head Plaster, Rudeboy, JL Master Handyman, Loralee Starweaver, Audiobook, White-Label Demo no longer appear anywhere on the landing). The section is now "Fields built with clients" — heading *"Every field begins with a person"* — carrying the two practitioner cards, the line *"And more fields are taking shape."*, and a closing vision block: **"Where this is going / Becoming an open field"** — voiced strictly as preparation ("We are preparing this platform to become an open-source field… will be able to author… so that one day it can be given"), consistent with the open-source strategy candidate's steward-not-layer framing; zero released-capability claims. The `PORTFOLIO_PROJECTS` 'sites' data entries remain in `lib/data/portfolio.ts` (unrendered) for history.
