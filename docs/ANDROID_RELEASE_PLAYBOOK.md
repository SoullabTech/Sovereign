...
This runs `bash scripts/build-android.sh debug`, which:

1. Syncs Capacitor (`npx cap sync android`) — copies `out/` into the native project. You must have already run `npm run build` (with `CAPACITOR_BUILD=1` if you also build for iOS).
2. `./gradlew assembleDebug` — produces `android/app/build/outputs/apk/debug/app-debug.apk`.
3. Copies to repo root as `maia-android-debug.apk` for easy access.

**Expected duration:** ~3–6 min on first run, ~1–2 min incremental.
...
This runs `./gradlew assembleRelease`, which applies ProGuard minification + resource shrinking, then signs with your release keystore.

**Output:** `android/app/build/outputs/apk/release/app-release.apk` (signed) — the script copies it to repo root as `maia-android-release.apk`.

Note: the script's `APK_PATH` variable in [scripts/build-android.sh:36](../scripts/build-android.sh) says `app-release-unsigned.apk`. That is the **unsigned** filename Gradle produces when `ANDROID_KEYSTORE_PATH` is unset. When you've sourced `.env.android` correctly, the actual signed output lives at `app-release.apk`. Verify which file Gradle produced:

```bash
ls -la android/app/build/outputs/apk/release/
```

If you see `app-release-unsigned.apk` and not `app-release.apk`, the signing config didn't pick up your env vars — re-check that `source .env.android` ran in the same shell as the build command.
