...
// Export factory function instead of singleton to avoid circular dependency issues
export function createAlchemicalDetector() {
  return new AlchemicalOperationDetector(new PsychospiritualMetricsEngine());
}

// Legacy export for backward compatibility (lazy initialization)
let _alchemicalDetector: AlchemicalOperationDetector | null = null;
export const alchemicalDetector = {
  get instance() {
    if (!_alchemicalDetector) {
      _alchemicalDetector = createAlchemicalDetector();
    }
    return _alchemicalDetector;
  }
};