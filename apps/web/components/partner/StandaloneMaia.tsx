...
export function StandaloneMaia({ onProfileUpdate, className = '' }: StandaloneMaiaProps) {
  ...
  useEffect(() => {
    const initializePartner = async () => {
      const existingProfile = typeof window !== 'undefined' ? localStorage.getItem('maia_partner_profile') : null;
...
export function enhanceWithPartnerMode(maiaProps: any) {
  const partnerProfile = typeof window !== 'undefined' ? localStorage.getItem('maia_partner_profile') : null;
