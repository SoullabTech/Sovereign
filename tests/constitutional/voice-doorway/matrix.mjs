...
async function laws(make) {
  ...
  { const io = makeIO(); const d = make(io, { sttEndpoint: 'https://example.invalid/nonlocal-stt' }); d.pressToTalk(); for (const f of frames(2)) d.frame(f); await d.release(); if (io.calls.stt.some((/** @type {string} */ ep) => ep !== STT_ENDPOINT) || d.view().stt_endpoint !== STT_ENDPOINT) broken.add('V-F2'); }
  // V-F3 no retention
