 ...
 *       R-S2 true Git first-add ancestry resolves it next · R-S3 a same-commit set stays UNVERIFIED / CONFLICT · R-C1 complete is never forced (one leftover blocks it) ·
...
const GIT_A = 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa';
const GIT_B = 'bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb';
const GIT_C = 'cccccccccccccccccccccccccccccccccccccccc';
const GIT_D = 'dddddddddddddddddddddddddddddddddddddddd';
const FAKE_GIT_LOG = [
  // newest-first listing is incidental. THETA B is a DAG descendant of A; IOTA A/B share one commit.
  GIT_D, 'docs/programme/KAPPA-10_B_2026-09-09.md', '',
  GIT_C, 'docs/programme/IOTA-09_A_2026-09-08.md', 'docs/programme/IOTA-09_B_2026-09-08.md', '',
  GIT_B, 'docs/programme/THETA-08_B_2026-09-07.md', '',
  GIT_A, 'docs/programme/THETA-08_A_2026-09-07.md', 'docs/programme/KAPPA-10_A_2026-09-09.md', '',
].join('\n');
const fakeGit = (/** @type {string[]} */ args) => {
  if (args[0] === 'log') return FAKE_GIT_LOG;
  if (args[0] === 'merge-base' && args[1] === '--is-ancestor') {
    const [, , older, newer] = args;
    if (older === GIT_A && newer === GIT_B) return '';
    const e = Object.assign(new Error('not ancestor'), { status: 1 }); throw e;
  }
...
function makeRepoR1() {
  ...
  w('docs/programme/KAPPA-10_A_2026-09-09.md', '**Standing:** KAPPA A\n');
  w('docs/programme/KAPPA-10_B_2026-09-09.md', '**Standing:** KAPPA B\n');
...
check(byId(R, 'THETA-08').standing === 'THETA B' && byId(R, 'THETA-08').precedence.succession?.rule.startsWith('R-S2'), 'R-S2', 'true git ancestry resolves the next same-day set');
...
check(byId(R, 'KAPPA-10').evidence_state === 'UNVERIFIED / CONFLICT' && byId(R, 'KAPPA-10').precedence.conflict.length === 2, 'R-S2', 'distinct but incomparable first-add commits stay UNVERIFIED / CONFLICT');
