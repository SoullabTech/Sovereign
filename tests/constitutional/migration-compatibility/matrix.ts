#!/usr/bin/env tsx
import { FALSIFIERS, STRICT_COMPATIBILITY } from "./falsifiers";
...
let failures=0;
console.log("MIGRATION COMPATIBILITY — EXECUTION MATRIX");
console.log("  "+FALSIFIERS.length+" falsifiers · "+CANDIDATES.length+" defeat candidates\n");
const ref=run(STRICT_COMPATIBILITY);
...
if (failures===0) console.log("REFERENCE  STRICT_COMPATIBILITY satisfies "+FALSIFIERS.length+"/"+FALSIFIERS.length+"\n");
...
if (failures===0) {
  console.log("COMPATIBILITY MATRIX: LETHAL + DISCRIMINATING");
  console.log("  "+CANDIDATES.length+"/"+CANDIDATES.length+" candidates died on their named falsifier");
  console.log("  STRICT_COMPATIBILITY satisfies "+FALSIFIERS.length+"/"+FALSIFIERS.length+" laws");
  console.log("  all collateral CLASSIFIED");
  process.exit(0);
}
console.error("COMPATIBILITY MATRIX FAILED — "+failures+" defect(s)");
process.exit(1);
