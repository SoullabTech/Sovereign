/** C1B LETHALITY MATRIX — `npm run matrix:ws-flagship-c1b`. ⛔ A surviving candidate repairs the SUITE. */
import { runC1BLaws, type LawResult } from './laws';
...
line(''); line('── FLAGSHIP-RUNTIME-CONVERGENCE-01 / C1B · LIVE WRITE HOST MOUNT ──'); line('');
const ref = runC1BLaws(REFERENCE);
...
for (const c of DEFEAT_CANDIDATES) {
  const named = NAMED_KILL[c.name]; const dl = failed(runC1BLaws(c)).map((r) => r.id);
  ...
  line(`  DEAD        ${c.name.padEnd(38)} → ${named}${collateral.length ? `   collateral: ${collateral.join(', ')}${un.length ? ' ⛔ UNCLASSIFIED' : ' (classified)'}` : ''}`);
...
line(''); line('  C1B mounts the flagship Write composition on the proven authorship substrate.');
line('  ⛔ No Develop · Review · MAIA bridge · Pure Canvas · toolbar authority is conferred.'); line('');
process.exit(exit);
