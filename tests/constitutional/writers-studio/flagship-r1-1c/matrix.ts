/** R1-1C LETHALITY MATRIX — `npm run matrix:ws-flagship-r1-1c`. ⛔ A surviving candidate repairs the SUITE. */
import { runR11CLaws, type LawResult } from './laws';
...
async function main() {
  line(''); line('── FLAGSHIP-RUNTIME-CONVERGENCE-01 / R1-1C · REVIEW NAVIGATION SUCCESSION + GOVERNED RE-FREEZE ──'); line('');
  const ref = await runR11CLaws(REFERENCE);
  ...
  for (const c of DEFEAT_CANDIDATES) {
    const named = NAMED_KILL[c.name]; const dl = failed(await runR11CLaws(c)).map((r) => r.id);
  ...
  line(''); line('  R1-1C makes Review a visible flagship destination; entry into a particular Review is still one explicit human choice → reading=<id> → R1-1B.');
  line('  ⛔ No automatic selection · no aggregation · no commission · no cognition · no write · no legacy bridge · read-only capabilities unchanged.'); line('');
  process.exit(exit);
}
void main();
