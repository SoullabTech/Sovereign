...
//
// Electron 44 uses `WebContentsView`; `BrowserView` has been deprecated since
// Electron 29. The containment properties remain the same — an independent
// webContents, session partition and frozen webPreferences — while the host now
// stays on Electron's supported view lifecycle.
...
/**
 * @param deps.WebContentsView Electron's WebContentsView constructor
...
function createPlatformShell({
  WebContentsView, sessionApi, shellApi, window, credential, onPlace,
} = {}) {
  ...
  function build() {
    ...
    view = new WebContentsView({ webPreferences: PLATFORM_WEB_PREFERENCES });
