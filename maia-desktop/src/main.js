...
// (`MAIADesktop/...`), so pin the governed marker explicitly and early.
const DESKTOP_UA_MARKER = `maia-desktop/${app.getVersion()}`;
if (!app.userAgentFallback.includes(DESKTOP_UA_MARKER)) {
  app.userAgentFallback = `${app.userAgentFallback} ${DESKTOP_UA_MARKER}`.trim();
}
...
const { PLATFORM_ENTRY_PATH, navigationDecision } = require('./shell-policy');
...
function witnessDirectory() {
  const dir = path.join(app.getPath('userData'), 'witness');
  fs.mkdirSync(dir, { recursive: true });
  return dir;
}
function witnessPath() {
  return path.join(witnessDirectory(), `d01-witness-${Date.now()}.jsonl`);
}
...
ipcMain.handle('maia:status', async () => ({
  app: 'maia-desktop',
  unit: 'MAIA-DESKTOP-BETA-01',
  version: app.getVersion(),
  channel: app.getVersion().includes('-beta.') ? 'beta' : 'stable',
...
function buildMenu() {
  ...
  Menu.setApplicationMenu(Menu.buildFromTemplate([
    ...
    { role: 'windowMenu' },
    {
      role: 'help',
      submenu: [{
        label: 'Open Beta Diagnostics Folder',
        click: async () => {
          const error = await shell.openPath(witnessDirectory());
          if (error) console.warn('[Desktop diagnostics] could not open folder');
        },
      }],
    },
  ]));
...
function showPlace() {
  ...
  mainWindow.setTitle('MAIA Desktop');
  buildMenu();
}
...
async function goTo(id) {
  ...
  const path = id === MAIA ? PLATFORM_ENTRY_PATH : null;
  if (!path) return;
