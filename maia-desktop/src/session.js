...
/**
 * ⛔ Same rule. A missing or malformed tier is `null` — unknown — never `free`.
 * Defaulting would make Electron the origin of an entitlement fact, and `free`
 * is a real entitlement, not an absence of one. Fail closed syntactically;
 * never fill a semantic blank.
 */
 ...
 */
/**
 * ⛔ IDENTITY CUSTODY, NOT IDENTITY SEMANTICS. HOUSE-RECONCILE-01.
 *
 * Desktop may REMOVE malformed claims. It may not ADD missing ones.
 *
 * This function used to answer a missing or malformed roles field with
 * `['member']`. That is the host originating an entitlement fact — deciding
 * that someone canonical said nothing about is an ordinary member. A House may
 * safely CARRY canonical identity; it may not COMPLETE it.
 *
 * `[]` means "no asserted roles", which is not the same claim as "ordinary
 * member" and must never be read as one. A missing role stays missing until the
 * canonical identity layer resolves it.
 */
 ...
 *
 * ⛔ Re-sanitizing is NOT re-defaulting. An upgraded install lands on `null`
 * tier and `[]` roles — unknown, not `free`/`['member']`. Reading an old file
 * is not an occasion for the host to decide what canonical never said.
 */
...
function createSession({ app, safeStorage, fetchImpl, onSignedOut } = {}) {
  ...
  async function signIn(username, password) {
    ...
    if (typeof AbortSignal !== 'undefined' && typeof AbortSignal.timeout === 'function') {
      request.signal = AbortSignal.timeout(SIGN_IN_TIMEOUT_MS);
    }

    let res;
    try {
      res = await doFetch(`${baseUrl}/api/members/signin`, request);
    } catch (error) {
      ...
      return {
        ...
        error: timedOut
          ...
          : 'Unable to reach MAIA. Check your connection and try again.',
      };
    }
    let data = null;
