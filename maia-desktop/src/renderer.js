...
window.addEventListener('DOMContentLoaded', async () => {
  $('signin').onsubmit = async (e) => {
    e.preventDefault();
    const submit = $('signin').querySelector('button[type="submit"]');
    submit.disabled = true;
    $('autherr').textContent = 'Signing in…';
    try {
      const out = await window.maia.signIn($('u').value.trim(), $('p').value);
      $('autherr').textContent = out && out.ok ? '' : ((out && out.error) || 'Sign-in failed.');
    } catch {
      $('autherr').textContent = 'Unable to reach MAIA. Check your connection and try again.';
    } finally {
