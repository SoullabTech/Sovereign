...
const INJECTED_ELECTRON = {
  'shell.js': {
    names: ['WebContentsView', 'webContents'],
    why: 'Every Electron surface arrives as a constructor parameter — WebContentsView, sessionApi, shellApi, window. `webContents` is only ever read off the injected view, never imported. ds01 drives the real logic with fakes precisely because none of it is bound to Electron.',
  },
};
