...
test('U — Desktop opens on canonical MAIA; the House is her in-page overlay', () => {
  assert.equal(PLATFORM_ENTRY_PATH, '/maia');
  assert.equal(platformEntryUrl(), `${PLATFORM_ORIGIN}/maia`);
  assert.equal(navigationDecision(`${PLATFORM_ORIGIN}/house`).action, 'block');
...
function fakeElectron() {
  ...
  const window = {
    ...
    getContentSize: () => [900, 700], isDestroyed: () => false, setTitle() {},
  };
  return { log, sessionApi, shellApi: { openExternal: (u) => log.external.push(u) }, window, WebContentsView: FakeWebContentsView };
}
...
test('U — navigate() opens a place, and refuses one the House does not name', async () => {
  const { shell, log } = await shellFor();
  const ok = await shell.navigate('/writers-studio');
  assert.equal(ok.ok, true, `navigate(/writers-studio) failed: ${ok.error}`);
  assert.equal(log.loaded[log.loaded.length - 1], `${PLATFORM_ORIGIN}/writers-studio`);
...
test('U — SHELL-DOUBLE-LOAD-01: opening a place loads that place, once', async () => {
  ...
  const studio = await shell.navigate('/writers-studio');
  assert.equal(studio.ok, true, `navigate(/writers-studio) failed: ${studio.error}`);
  assert.deepEqual(e.log.loaded, [`${PLATFORM_ORIGIN}/writers-studio`],
    'opening Writer\'s Studio went through MAIA on the way');

  const maia = await shell.navigate('/maia');
  assert.equal(maia.ok, true);
  assert.deepEqual(e.log.loaded, [`${PLATFORM_ORIGIN}/writers-studio`, `${PLATFORM_ORIGIN}/maia`],
