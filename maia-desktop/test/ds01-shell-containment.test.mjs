...
function fakeElectron() {
  ...
  const window = {
    ...
    title: null,
    contentView: {
      addChildView(v) { window.current = v; log.attached.push('view'); },
      removeChildView() { window.current = null; log.attached.push(null); },
    },
  ...
  return { log, sessionApi, shellApi, window, WebContentsView: FakeWebContentsView };
}
...
async function signedInShell(token = TOKEN) {
  ...
  const shell = createPlatformShell({
    WebContentsView: e.WebContentsView, sessionApi: e.sessionApi, shellApi: e.shellApi,
...
test('F1 — an unreachable sign-in returns a retryable result instead of throwing', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ds01-'));
  tempDirs.push(dir);
  let request;
  const credential = createSession({
    app: { getPath: () => dir },
    safeStorage: { isEncryptionAvailable: () => false },
    fetchImpl: async (_url, init) => { request = init; throw new TypeError('offline'); },
  });

  const out = await credential.signIn('kelly', 'pw');
  assert.deepEqual(out, {
    ok: false,
    error: 'Unable to reach MAIA. Check your connection and try again.',
  });
  assert.equal(credential.state().signedIn, false);
  assert.ok(request.signal, 'sign-in has no bounded network timeout');
});
...
test('F5 — the native menu returns to canonical MAIA; current IA stays in-page', () => {
  ...
  assert.ok(!/PLATFORM_HOUSE_PATH|Alt\+CmdOrCtrl\+J/.test(mainJs),
    'the native menu still points at the retired /house route');
});
...
test('ADVERSARIAL — the view is sized to the window and follows a resize', async () => {
  ...
  assert.equal(shell._view().bounds.width, 900);
  assert.equal(shell._view().bounds.height, 700);
...
test('BETA — privacy-safe diagnostics are reachable from the native menu', () => {
  assert.ok(mainJs.includes("label: 'Open Beta Diagnostics Folder'"));
  assert.ok(/shell\.openPath\(witnessDirectory\(\)\)/.test(mainJs));
});
