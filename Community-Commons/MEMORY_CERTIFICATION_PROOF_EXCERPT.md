...
### 2) Memory injection is transparent (view trust details)

Each response includes **metadata** that shows what MAIA actually used from memory.
...
**Why this matters:** the system doesn't just *claim* it remembered — metadata shows whether memory was truly present.
...
1. **Your conversation history persists** across restarts.
2. **Metadata shows whether memory was actually injected** into a reply.
...
* **Metadata:** the system's transparency layer that reports what happened, not what's implied.
