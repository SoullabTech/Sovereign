...
const TEST_ENDPOINT = 'http://localhost:3003/api/between/chat';
  ...
  try {
    ...
    const response = await fetch(TEST_ENDPOINT, {
      ...
      body: JSON.stringify({
        ...
        sessionId: `test-port3003-${Date.now()}-${Math.random()}`
      }),
    ...
    if (data.route?.safeMode) {
      ...
      console.log(`⚠️  Safe mode not detected - mode: ${data.route?.mode || 'unknown'}`);
    }
...
async function runTests(): Promise<void> {
  console.log('🚀 Testing Rupture Detection Integration (Safe Mode Server)\n');
  console.log(`📍 Testing endpoint: ${TEST_ENDPOINT}`);
  console.log(`🛡️  Using dedicated SAFE_MODE server to bypass Ollama performance issues\n`);
  ...
  console.log('- Verify safe mode server works properly');
}
